...
`pre-commit` is disabled by default via the `SKIP` environment variable.
If you would prefer to use `pre-commit`, then you can unset the variable in your shell.
To make it persist, you can create a `.env` file and unset it there or if
you use `direnv` in some form, you can unset it after the nix shell is loaded
(i.e: unset it after the `use nix` / `use flake` command in `.envrc`).

(NOTE: The reason why we use the `SKIP` flag is because due to `nix flakes`
there is no easy way to customise the usage of `pre-commit`. Flakes restricts
`builtins.getEnv` and the local `.pre-commit-config.yaml` is generated by the flake
and symlinked to the nix store. If you need more fine-grained control over the config,
you could potentially rename the `.pre-commit-config.yaml` symlink to something else
and use your own local `.pre-commit-config.yaml`.)
...
You can also run things manually via `pre-commit`. If you previosly unset `SKIP`
you can just run `pre-commit run --all-files`, if you didn't unset it and only wish
to unset it for one run, assuming you are using `bash` do the following:
`SKIP= pre-commit run --all-files`.
