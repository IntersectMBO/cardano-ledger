...
\begin{figure}[htb]
  ...
  \begin{equation*}
    ...
    = \left(
      \begin{array}{r@{~\in~}lr}
        \var{rups} & \UPropId \mapsto (\ProtVer \times \ProtParams)
...
\item The type $\type{Rups}$ is equal to
  $\UPropId \mapsto (\ProtVer \times \ProtParams)$, we use this local
  ...
  \begin{align*}
    & \BVREnv
      ...
      \begin{array}{r@{~\in~}lr}
        ...
        \var{b_n} & \BkNr & \text{current block number}\\
        (\var{pv}, \var{pps}) & \ProtVer \times \ProtParams & \text{current protocol parameters}\\
        \var{cps} & \UPropId \mapsto \BkNr & \text{confirmed proposals}\\
        \var{rups} & \type{Rups}
                             & \text{registered update proposals}\\
      \end{array}\right)
  ...
  \begin{align*}
    & \BVRState
      ...
      \begin{array}{r@{~\in~}lr}
        ...
        & \text{candidate protocol parameters}\\
