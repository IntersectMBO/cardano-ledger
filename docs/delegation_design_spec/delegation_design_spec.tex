...
the list of key registration certificates in the transaction\todo{Or
  rather in the list of \emph{all} certifications in the transaction?}. This
...
\item posting a delegation certificate
\end{itemize}
...
\item a staking key is de-registered -- this should release the
  deposit for the key itself, as well as for the delegation
  certificate for this key if there is one
...
operator will include a list\footnote{Allowing a list of owner keys
  ...
  control over their funds.} of stake key hashes
\(\mathcal{H}(vks_\text{owner})\)
...
Since the actual amount of stake that the stake pool owners contribute
to the pool might change at any point in time, the ordering of pools
will use the amount of stake that the operator \emph{pledged} when
registering the pool, not the amount of stake that is currently
delegated to the pool from registered owner keys. However, pools where
this amlount is smaller than the amount pledged are expected to give
zero rewards, and will end up at the end of the list.
...
registered, and the registration certificate is in the immutable part
of the chain (\(k\)
blocks deep), newly created addresses can be pointer addresses to the
