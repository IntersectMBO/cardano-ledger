...
desirability, as described in \cref{pool-desirability-and-ranking}.
Since this ranking depends not only on the costs and margin set
...
When calculating the desirability of a pool, the wallet will use the
best data available:
\begin{itemize}
\item The cost, margin, and pledged stake will be taken from the most
  recent stake pool registration certificate of the pool.
\item The ratio of published blocks and elected slots will be taken
  from the previous epoch.
\item The stake of the pool, and the amounn of stake that the owners
  ...
  wallet performs the ranking).
\end{itemize}

In order to prevent a slight difference in the desirability to result
in people conglomerating to a single stake pool, filling it rapidly, a
wallet should jitter the list of stake pools slightly when displaying
it to a user\todo{Needs discussion with the researchers}.
...
\]\todo{This is based on the maximal pool rewards. Should we not
...
desirable pool gets rank 2 and so on.\todo{Is it really \(\hat{f}\)
  here, or rather \(\tilde{f}\)?}
