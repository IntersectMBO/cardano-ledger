...
1. **Terms** consisting of 
   ...
   - Applications: `elem_ "abc" xs`, `x ==. y`.  
	   ...
	   - Infix application the function symbol lies between its two Term arguments .(i.e. `abc ==. y`)
...
1. Every function symbol has `Syntax`, `Semantics`, and `Logic` properties.
...
3. Associated with some types `t`, is another type called the `TypeSpec t` . This an associated type family
   ...
   constraints for a variable of type `t`.
...
4. A set of constraints about a single variable of type `t` is collected into a `Spec t`, The type `Spec t`
   ...
   embed non type-specific constraint properties.
...
6. We solve a set of constraints one variable at a time. The order in which we solve the variables is called
   ...
   from the plan. Choosing a random value for that variable that meets the constraints on it. Then
...
7. We can extract a random value from a `Spec t`, by using the function `genFromSpec :: Spec t -> QuickCheck.Gen t`
...
class HasSpec a where
  ...
  -- | The `TypeSpec` that has no constraints.
  emptySpec :: TypeSpec a
  
  -- | A function, akin to the `Semigroup(<>)`,  that combines two `TypeSpec`. It is different
...
In a later section we present `HasSpec` instances for several types: `Bool`, `Integer`, `Set a`, pairs `(a,b)` and `(Either a b)`
...
## The Spec datatype
 ...
 ```
  data Spec a where
	...
	-- ^ The Spec encodes a FOTL statement
...
3. The method `guardTypeSpec` returns an `ErrorSpec` if its `Range` input is inconsistent.
   This happens only when a `(Just lo)` bound is greater than a `(Just hi)` bound. If a `Range` has
...
4. The method `genFromTypeSpec` generates a random value in the `GenT` monad (a richer version of the QuickCheck
   `Gen` monad that as mechanisom for handling errors, which the `Gen` monad does not). We extract the `QuickCheck.Gen`
...
It says `x` must be bewteen 9(6+3) and 5(12+3), but cannot be 10(7+3). Which correponds to the code.
...
2. It is the first type that has we will see that has a `Container` instance.
3. Introduces a rather complex `TypeSpec` to account for the first two.

Probably best to start with the `Container` class. A type can have a `(Container t e)` instance
if the `t` stores elements of type `e`, and we can enumerate over the `e` in `t`. The operation
...
We will use this type `SetSpec` as the `TypeSpec` for `Set` . Like many `TypeSpec`s it
...
`rewriteRules` over the function symbols. Here are the rewrite rules in a more relaxed notation.
