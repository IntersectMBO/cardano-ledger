...
The following transition system is used in
Properties~\ref{prop:header-only-validation} and .
...
block headers without having to download the block bodies. The
...
\begin{property}[Header only validation]\label{prop:header-only-validation}
  ...
  for all and chain extensions $E$ with headers $H$ such that:
  %
  $$
  t_E - t  \leq 2*k
  $$
...
$e$ and initial state $s$, assume that a sequence of headers $H$ corresponding
to blocks in $E$ is valid according to the $\stslabel{pbft+bhead}$ transition
system:
%
$$
...
%
$$
e \vdash s \transtar{CHAIN}{E \shortuparrow j} s_j
$$
But by Property~\ref{prop:body-only-validation} we know that
%
$$
e_{h_j} \vdash s_{h_j} \trans{pbft+bhead}{H.j} s_{h_{j+1}}
$$
...
block bodies where valid.

\begin{property}[Body only validation]\label{prop:body-only-validation}
  For all environments $e$, states $s$ with slot number ($\var{s_{last}}$) $t$;
  for all and chain extensions $E$, $n = \size{E}$, with headers $H$ such that:
  $$
  t_E - t  \leq 2*k
  $$
...
\begin{property}[Existence of roll back function]
  There exists a function $f$ such that for all chains $C = C_0 ; b; C_1$ with
  corresponding headers $H = H_0; h; H_1$ we have that if

$$
s_0 \transtar{CHAIN}{C_0;b} s_1 \transtar{CHAIN}{C_1} s_2
$$

then

$$
f (slot b) s_2 \transtar{PBFT+BHEAD}{H_1} s_h
$$
\end{property}
