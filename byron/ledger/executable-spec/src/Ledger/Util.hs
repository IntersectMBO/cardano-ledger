 ...
 where
  ...
  body = do
    ...
    let listName    = mkName ("goblinGens" <> stsNameStr)
        ...
        listDataDec = ValD (VarP listName)
  ...
  sigGenTy :: Type
  sigGenTy  = AppT (ConT (mkName "SignalGenerator"))
                   (ConT stsName)
...
mkGoblinDecls stsNameStr pfNameStr = do
 ...
 where
  ...
  goblinDataDecs = do
    ...
    let sigDec  = SigD gdName (AppT (ConT (mkName "GoblinData"))
                                    (ConT (mkName "Bool")))
  ...
  goblinGenDecs = do
    body <- [| tinkerWithSigGen @Bool @ $(pure (ConT stsName))
                                $(pure (VarE gdName)) |]
    let sigDec  = SigD ggName (AppT (ConT (mkName "SignalGenerator"))
                                    (ConT stsName))
