...
-- | This is a simple lookup into the state for the values of current
-- protocol parameters. These values can change on the epoch boundary.
-- Use 'queryFuturePParams' to see if we are aware of any upcoming
-- changes.
...
-- | This query will return values for protocol parameters that are
-- likely to be adopted at the next epoch boundary. It is only when we
-- passed 2 stability windows before the end of the epoch that users can
-- rely on this query to produce stable results.
