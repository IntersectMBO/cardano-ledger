...
-- | Report stake per pool per snapshot as well as total active stake
-- per snapshot.
--
-- Each snapshot is taken at an epoch boundary. The mark snapshot is
-- the most recent, taken immediately before the start of the current
-- epoch. The set snapshot is one epoch older, and the go snapshot is
-- the oldest (two epochs back), used for reward calculation and leader
-- election.
--
-- /Note/ - The behavior of this function depends on the protocol
-- version:
--
-- __Protocol Version < 11:__
--
-- * Empty 'Set': collects pools that have at least one delegator in
-- any of the three snapshots. Pools with delegators but zero stake are
-- still included (with zero-valued entries).
--
-- * Specific pool IDs: results are returned for exactly those pools,
-- even if they have no stake or are not registered (with zero-valued
-- entries).
--
-- __Protocol Version >= 11:__
--
-- * Empty 'Set': collects pools with non-zero stake in any of the three
-- snapshots.
--
-- * Specific pool IDs: pools with zero stake across all three snapshots
-- are filtered out and will not appear in the result, even if
