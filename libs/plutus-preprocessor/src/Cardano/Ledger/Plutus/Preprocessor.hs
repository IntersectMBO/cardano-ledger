...
display h = do
  ...
  forM_ allTestScripts $ \(scriptName, scriptLangLookup, haddockFirst :| haddockRest) -> do
    hPutStr h $
      unlines $
        ...
        , "-- | " ++ haddockFirst
          ...
          ++ ["-- " ++ l | l <- haddockRest]
          ++ [ scriptName ++ " :: SLanguage l -> Maybe (Plutus l)"
             , scriptName ++ " ="
             , indent "fmap (decodeHexPlutus . mconcat) . \\case"
             ]
    forM_ [minBound .. maxBound] $ \lang -> case scriptLangLookup lang of
      ...
      Nothing -> hPutStr h . indent . indent $ "S" ++ show lang ++ " -> Nothing\n"
...
allTestScripts =
  ...
  ,
    ...
    , \case
        PlutusV1 -> Nothing
        PlutusV2 -> Just V2.inputsIsSubsetOfRefInputsBytes
        PlutusV3 -> Just V3.inputsIsSubsetOfRefInputsBytes
