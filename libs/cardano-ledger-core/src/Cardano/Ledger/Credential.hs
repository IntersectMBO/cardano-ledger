...
credentialDecoder :: Typeable kr => Decode (Closed Dense) (Credential kr)
credentialDecoder = Summands "Credential" $ \case
  0 -> SumD KeyHashObj <! From
  1 -> SumD ScriptHashObj <! From
  k -> Invalid k

instance Typeable kr => DecCBOR (Credential kr) where
  decCBOR = decode credentialDecoder
  {-# INLINE decCBOR #-}
...
instance Typeable kr => FromCBOR (Credential kr) where
  fromCBOR = toPlainDecoder Nothing shelleyProtVer $ decode credentialDecoder
  {-# INLINE fromCBOR #-}
