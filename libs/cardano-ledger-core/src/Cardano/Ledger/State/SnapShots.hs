...
instance DecShareCBOR SnapShot where
  ...
  decSharePlusCBOR = decodeRecordNamedT "SnapShot" (const 2) $ do
...
emptySnapShot = SnapShot (Stake VMap.empty) (knownNonZeroCoin @1) mempty
...
calculatePoolDistr' includeHash SnapShot {ssTotalActiveStake, ssStakePoolsSnapShot} =
  poolDistr
  where
    ...
    poolDistr =
      PoolDistr
        { unPoolDistr = VMap.toMap $ VMap.mapMaybeWithKey toIndividualPoolStake ssStakePoolsSnapShot
        , pdTotalActiveStake = ssTotalActiveStake
        }
