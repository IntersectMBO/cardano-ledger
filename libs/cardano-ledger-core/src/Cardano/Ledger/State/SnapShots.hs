...
data StakePoolSnapShot = StakePoolSnapShot
  ...
  -- the `StakePoolState` is `spsRewardAccount`.
  }
...
data SnapShot = SnapShot
  ...
  -- ^ Total active stake is primarily used in denominator, therefore it cannot be zero and is
  ...
  deriving (NoThunks) via AllowThunksIn '["ssDelegations", "ssPoolParams"] SnapShot
