...
selectLeiosCommittee committeeSize candidates =
  ...
  where
    ...
    toTuple lk = (unLeiosPubKey (leiosPubKey lk), unLeiosPossessionProof (leiosPossessionProof lk))
...
instance EncCBOR LeiosSeat where
  encCBOR (LeiosSeat weight vkey) =
    encode $
      Rec LeiosSeat
        !> To weight
        !> E (encodeStrictMaybe encodeVerKeyDSIGN) vkey
...
instance DecCBOR LeiosSeat where
  decCBOR =
    decode $
      RecD LeiosSeat
        <! From
        <! D (decodeStrictMaybe decodeVerKeyDSIGN)
