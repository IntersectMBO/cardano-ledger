...
import Cardano.Prelude (NFData, Typeable, Word16, Word32, Word64, Word8, fromMaybe)
...
nonZeroM x
  | isZero x = pure $ NonZero x
  | otherwise = fail "Encountered zero while trying to decode a NonZero value"
...
mulNonZero :: NonZero Integer -> NonZero Integer -> NonZero Integer
mulNonZero (NonZero x) (NonZero y) = NonZero $ x * y
