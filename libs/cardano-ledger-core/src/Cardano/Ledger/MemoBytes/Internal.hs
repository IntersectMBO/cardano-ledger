...
mbpackedByteCount :: MemoBytes t -> Int
mbpackedByteCount = packedByteCount . mbBytes

mbpackM :: MemoBytes t -> Pack s ()
mbpackM = packM . mbBytes
...
mbunpackM v = unpackM >>= decodeMemoBytes v
...
lensMemoRawType ::
  ...
  Lens t t a b
lensMemoRawType v getter setter =
  lens (getter . getMemoRawType) (\t b -> mkMemoized v $ setter (getMemoRawType t) b)
