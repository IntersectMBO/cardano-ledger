...
instance DSIGNAlgorithm r => ToExpr (VerKeyDSIGN r) where
  toExpr vk =
    Rec "VerKeyDSIGN" $
      OMap.fromList [("VerKeyDSIGN (rawBytes)", toExpr $ rawSerialiseVerKeyDSIGN vk)]

instance DSIGNAggregatable r => ToExpr (PossessionProofDSIGN r) where
  toExpr proof =
    Rec "PossessionProofDSIGN" $
      OMap.fromList [("PossessionProofDSIGN (rawBytes)", toExpr $ rawSerialisePossessionProofDSIGN proof)]
...
instance ToExpr LeiosKey

instance ToExpr LeiosPubKey

instance ToExpr LeiosPossessionProof
