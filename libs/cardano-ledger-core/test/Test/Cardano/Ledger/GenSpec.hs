      ...
      describe "Coin" $ do
        it @DefaultImpSpec "has appropriate statistical behavior" $ do
          ...
          impAnn "Mean and standard deviation are reasonable" $ do
            impAnn "mocs" $ (mocs, 7_000) `shouldSatisfy` relativelyClose 0.1
            impAnn "socs" $ (socs, 37_000) `shouldSatisfy` relativelyClose 0.1
            impAnn "mbcs" $ (mbcs, 7_000_000) `shouldSatisfy` relativelyClose 0.1
            impAnn "sbcs" $ (sbcs, 37_000_000) `shouldSatisfy` relativelyClose 0.1
