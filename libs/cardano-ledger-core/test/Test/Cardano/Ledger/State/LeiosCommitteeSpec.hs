...
candidateWith :: Int -> Word64 -> StrictMaybe LeiosKey -> LeiosCandidate
...
validKey :: Int -> LeiosKey
validKey i =
  LeiosKey
    { leiosPubKey = LeiosPubKey (deriveVerKeyDSIGN signKey)
    , leiosPossessionProof =
        LeiosPossessionProof (createPossessionProofDSIGN leiosSignContext signKey)
    }
  where
...
spec = describe "selectLeiosCommittee" $ do
  ...
  prop "equal stakes are seated by ascending pool id" $
    \(Positive poolCount) ->
      let n = 1 + poolCount `mod` 8
       ...
       in seated === [unLeiosPubKey (leiosPubKey key) | (_, key) <- keyed]
  ...
  it "a key whose proof of possession does not verify is seated keyless" $
    ...
    let mismatched =
          (validKey 1) {leiosPossessionProof = leiosPossessionProof (validKey 2)}
