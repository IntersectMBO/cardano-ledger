...
module Constrained.Experiment.Examples.Map where

import Constrained.Experiment.API
import Constrained.Experiment.Examples.Basic
import Constrained.Experiment.Syntax hiding (name)
import Constrained.Experiment.TheKnot (printPlan)
import Data.Map (Map)
...
elemSpec = constrained' $ \ [var|key|] [var|val|] [var|map|] ->
  [ assert $ key `member_` dom_ map
  , forAll' map $ \ [var|k'|] [var|v'|] ->
