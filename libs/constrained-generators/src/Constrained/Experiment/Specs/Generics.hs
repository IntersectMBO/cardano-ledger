    ...
    case spec of
      ...
      MemberSpec es -> case [a | SumLeft a <- NE.toList es] of
        [] ->
          Left $
            pure $
              "propagate(left HOLE) on (MemberSpec es) with no SumLeft in es: " ++ show (NE.toList es)
        (x : xs) -> Right $ MemberSpec (x :| xs)
      _ ->
        Left
          (NE.fromList ["InjLeftW[sumleft_]", "Unreachable context, should be covered by propagate", show ctx])
  simplepropagate ctx _ = Left (NE.fromList ["InjLeftW[sumleft_]", "Unreachable context, wrong number of args", show ctx])
  ...
  simplepropagate ctx@(Context _ InjRightW (HOLE End)) spec =
    case spec of
      ...
      _ ->
        Left
          ( NE.fromList
              ["InjRightW[sumright_]", "Unreachable context, should be covered by propagate", show ctx]
          )
  simplepropagate ctx _ =
    Left (NE.fromList ["InjRightW[sumright_]", "Unreachable context, wrong number of args", show ctx])
