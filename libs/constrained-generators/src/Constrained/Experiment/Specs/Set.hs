...
data SetW (c :: Constraint) (s :: Symbol) (d :: [Type]) (r :: Type) where
  SingletonW :: SetW (Ord a) "singleton_" '[a] (Set a)
  UnionW :: SetW (Ord a) "union_" '[Set a, Set a] (Set a)
  SubsetW :: SetW (Ord a) "subset_" '[Set a, Set a] Bool
  MemberW :: SetW (Ord a) "member_" '[a, Set a] Bool
  DisjointW :: SetW (Ord a) "disjoint_" '[Set a, Set a] Bool
  FromListW :: SetW (Ord a) "fromList_" '[[a]] (Set a)

deriving instance Eq (SetW c s dom rng)

instance Show (SetW c s ds r) where
...
setSem :: c => SetW c s ds r -> FunTy ds r
...
instance (HasSpec a, Ord a) => FunSym (Ord a) "subset_" SetW '[Set a, Set a] Bool where
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence SubsetW (HOLE :<> x :<| End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App SubsetW (v' :> Lit x :> Nil)) (v :-> ps)
  propagate (Context Evidence SubsetW (x :|> HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App SubsetW (Lit x :> v' :> Nil)) (v :-> ps)
  propagate (Context Evidence SubsetW ctx) spec
  ...
  propagate ctx _ = ErrorSpec $ pure ("FunSym instance for SubsetW with wrong number of arguments. " ++ show ctx)
...
instance (HasSpec a, Ord a) => FunSym (Ord a) "member_" SetW '[a, Set a] Bool where
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence MemberW (HOLE :<> x :<| End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App MemberW (v' :> Lit x :> Nil)) (v :-> ps)
  propagate (Context Evidence MemberW (x :|> HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App MemberW (Lit (x :: a) :> v' :> Nil)) (v :-> ps)
  propagate ctx (SuspendedSpec _ _) = ErrorSpec $ pure ("FunSym instance for MemberW with wrong number of arguments. " ++ show ctx)
  propagate (Context Evidence MemberW ctx) spec
  ...
  propagate ctx _ = ErrorSpec $ pure ("FunSym instance for MemberW with wrong number of arguments. " ++ show ctx)
...
instance (HasSpec a, Ord a) => FunSym (Ord a) "union_" SetW '[Set a, Set a] (Set a) where
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence UnionW (HOLE :<> x :<| End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App UnionW (v' :> Lit x :> Nil)) (v :-> ps)
  propagate (Context Evidence UnionW (x :|> HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App UnionW (Lit x :> v' :> Nil)) (v :-> ps)
  propagate (Context Evidence UnionW ctx) spec
    | (s :|> HOLE :<> End) <- ctx =
        propagate (Context Evidence UnionW (HOLE :<> s :<| End)) spec
    | (HOLE :<> (s :: Set a) :<| End) <- ctx
  ...
  propagate ctx _ = ErrorSpec $ pure ("FunSym instance for UnionW   with wrong number of arguments. " ++ show ctx)
...
instance (HasSpec a, Ord a) => FunSym (Ord a) "disjoint_" SetW '[Set a, Set a] Bool where
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence DisjointW (HOLE :<> x :<| End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App DisjointW (v' :> Lit x :> Nil)) (v :-> ps)
  propagate (Context Evidence DisjointW (x :|> HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App DisjointW (Lit x :> v' :> Nil)) (v :-> ps)
  propagate (Context Evidence DisjointW ctx) spec
    | (HOLE :<> (s :: Set a) :<| End) <- ctx =
        propagate (Context Evidence DisjointW (s :|> HOLE :<> End)) spec
    | ((s :: Set a) :|> HOLE :<> End) <- ctx
    , Evidence <- prerequisites @(Set a) = caseBoolSpec spec $ \case
  ...
  propagate ctx _ = ErrorSpec $ pure ("FunSym instance for DisjointW with wrong number of arguments. " ++ show ctx)
...
instance (HasSpec a, Ord a) => FunSym (Ord a) "fromList_" SetW '[[a]] (Set a) where
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence FromListW (HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App FromListW (v' :> Nil)) (v :-> ps)
  propagate (Context Evidence FromListW (HOLE :<> End)) spec
  ...
  propagate ctx _ = ErrorSpec $ pure ("FunSym instance for FromListW with wrong number of arguments. " ++ show ctx)
...
fromList_ :: (Ord a, HasSpec a) => Term [a] -> Term (Set a)
