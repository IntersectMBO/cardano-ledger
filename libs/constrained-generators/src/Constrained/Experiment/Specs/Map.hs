  ...
  genFromTypeSpec (MapSpec mHint mustKeys mustVals size (simplifySpec -> kvs) foldSpec) = do
    ...
    let haveVals = map snd mustMap
        ...
        foldSpec' = case foldSpec of
          NoFold -> NoFold
          FoldSpec fn@(Fun Evidence symbol) sumSpec -> FoldSpec fn $ propagate (Context Evidence theAddFn (HOLE :<> mustSum :<| End)) sumSpec
            where
...
data MapW (c :: Constraint) (sym :: Symbol) (dom :: [Type]) (rng :: Type) where
...
deriving instance Eq (MapW c s dom rng)

mapSem :: MapW c s d r -> FunTy d r
mapSem DomW = Map.keysSet
mapSem RngW = Map.elems
mapSem LookupW = Map.lookup

instance Witness MapW where
  semantics = mapSem

instance Show (MapW c s d r) where
  show DomW = "dom_"
  show RngW = "rng_"
...
instance
  ...
  FunSym () "dom_" MapW '[Map k v] (Set k)
  where
  propagate ctxt (ExplainSpec [] s) = propagate ctxt s
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence DomW (HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App DomW (v' :> Nil)) (v :-> ps)
  propagate (Context Evidence DomW (HOLE :<> End)) spec =
    case spec of
      MemberSpec (s :| []) ->
        typeSpec $
  ...
  propagate ctx _ =
    ErrorSpec $ pure ("FunSym instance for DomW with wrong number of arguments. " ++ show ctx)
...
instance
  (HasSpec k, HasSpec v, Ord k, IsNormalType v, IsNormalType k) =>
  FunSym () "rng_" MapW '[Map k v] [v]
  where
  propagate ctxt (ExplainSpec [] s) = propagate ctxt s
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence RngW (HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App RngW (v' :> Nil)) (v :-> ps)
  propagate (Context Evidence RngW (HOLE :<> End)) spec
  ...
  propagate ctx _ =
    ErrorSpec $ pure ("FunSym instance for RngW with wrong number of arguments. " ++ show ctx)
...
instance
  ...
  FunSym () "lookup_" MapW '[k, Map k v] (Maybe v)
  where
  propagate ctxt (ExplainSpec [] s) = propagate ctxt s
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence LookupW (HOLE :<> x :<| End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App LookupW (v' :> Lit x :> Nil)) (v :-> ps)
  propagate (Context Evidence LookupW (x :|> HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App LookupW (Lit x :> v' :> Nil)) (v :-> ps)
  propagate (Context Evidence LookupW (k :|> HOLE :<> End)) spec
    | Evidence <- prerequisites @(Map k v) =
        constrained $ \m ->
  ...
  propagate (Context Evidence LookupW (HOLE :<> m :<| End)) spec =
  ...
  propagate ctx _ =
    ErrorSpec $ pure ("FunSym instance for LookupW with wrong number of arguments. " ++ show ctx)
