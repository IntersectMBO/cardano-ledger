...
module Constrained.Experiment.Specs.Tree (BinTree (..), TreeW (..), rootLabel_, TreeSpec (..)) where
...
import Constrained.Experiment.Specs.ListFoldy (FoldSpec (..), ListSpec (..))
import Constrained.Experiment.Specs.SumProd (PairSpec (Cartesian), match)
import Constrained.Experiment.Syntax (forAll, genHint)
import Constrained.Experiment.TheKnot (genFromSpecT, shrinkWithSpec)
...
data TreeW (c :: Constraint) (sym :: Symbol) (dom :: [Type]) (rng :: Type) where
  RootLabelW :: TreeW () "rootLabel_" '[Tree a] a

deriving instance Eq (TreeW c s d r)
deriving instance Show (TreeW c s d r)

instance Witness TreeW where
  semantics RootLabelW = \(Node a _) -> a
  inFix _ = False

instance HasSpec a => FunSym () "rootLabel_" TreeW '[Tree a] a where
  propagate ctxt (ExplainSpec [] s) = propagate ctxt s
  propagate ctxt (ExplainSpec es s) = ExplainSpec es $ propagate ctxt s
  propagate _ TrueSpec = TrueSpec
  propagate _ (ErrorSpec msgs) = ErrorSpec msgs
  propagate (Context Evidence RootLabelW (HOLE :<> End)) (SuspendedSpec v ps) =
    constrained $ \v' -> Let (App RootLabelW (v' :> Nil)) (v :-> ps)
  propagate (Context Evidence RootLabelW (HOLE :<> End)) spec =
