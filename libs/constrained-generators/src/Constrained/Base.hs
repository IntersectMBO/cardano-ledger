...
appendFn :: forall fn a. (HasSpec fn a, Show a, Typeable a) => fn '[[a], [a]] [a]
...
instance Pretty (Pred fn) where
  pretty = \case
    ...
    FalsePred {} -> "False"
    Monitor {} -> "monitor"
    Explain es p -> "ExplainPred" <+> viaShow (NE.toList es) <+> "$" /> pretty p
...
checkPredsE msgs env ps =
  -- and <$> mapM (checkPred env) ps
  case catMaybes (fmap (checkPredE env msgs) ps) of
    [] -> Nothing
    (x : xs) -> Just (NE.nub (foldr (<>) x xs))
  ...
  ForAll t (x :-> p) -> case runTermE env t of
    ...
    Right set ->
      let answers =
       ...
       in case answers of
            [] -> Nothing
            (x : xs) -> Just (msgs <> NE.nub (foldr (<>) x xs))
