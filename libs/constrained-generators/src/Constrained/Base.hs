...
toCtx ::
  ...
  Term fn a ->
  m (Ctx fn v a)
toCtx termstring v = go . simplifyTerm
  where
    go :: forall b. Term fn b -> m (Ctx fn v b)
    go (Lit i) = fatalError ["toCtx (Lit " ++ show i ++ ")"]
    go (App f as) = CtxApp f <$> toCtxList termstring v as
    go (V v')
...
toCtxList termstring v = prefix
  where
    ...
    suffix (_ :> _) = fatalError ["toCtxList: the variable: " ++ show v ++ "appears more than once in term: " ++ termstring ++ ", so we can't make a context."]
...
--   It is not unusual for instances of sizeOfTypeSpec to call sizeOfSpec, some (TypeSpec fn t)'s contain (Spec fn s)
sizeOfSpec :: forall fn t. Spec fn t -> SizeSpec
sizeOfSpec TrueSpec = mempty
sizeOfSpec (MemberSpec xs) = atMostInt (length xs)
...
specSizeBound spec = TypeSpec (sizeOfSpec spec) []
...
class
  ...
  --  If the type 't' does not have a (Sized t) instance we use the default mempty, which palces no constraint on the Size.
...
computeSpec x p = fromGE ErrorSpec $ case simplifyPred p of
  ...
  Assert es t -> explain es $ propagateSpec (equalSpec True) <$> toCtx (show t) x t
  ForAll (Lit s) b -> pure $ foldMap (\val -> computeSpec x $ unBind val b) (forAllToList s)
  ForAll t b -> propagateSpec (forAllSpec $ computeSpecBinder b) <$> toCtx (show t) x t
  ...
  Case t branches ->
    let branchSpecs = mapList computeSpecBinder branches
     in propagateSpec (caseSpec branchSpecs) <$> toCtx (show t) x t
...
-- | Like specSizeBound, but slightly more accurate, since it can analyze the 'cant' part of TypeSpec for Set
sizeOfSet :: forall fn a. (Ord a, BaseUniverse fn) => Spec fn (Set a) -> Spec fn Size
sizeOfSet t@(TypeSpec (SetSpec _ _ _) cant) = specSizeBound t <> cantSpec
  where
    cantSpec
      | mempty `elem` cant = notEqualSpec (Size 0)
      | otherwise = TrueSpec
sizeOfSet x = specSizeBound x
...
instance (Ord a, HasSpec fn a) => HasSpec fn (Set a) where
  ...
  toPreds s (SetSpec m es _size)
    | Evidence :: Evidence (HasSpec gn a) <- prerequisites @fn @(Set a) =
...
instance (BaseUniverse fn, Member (FunFn fn) fn) => Functions (FunFn fn) fn where
  ...
  -- Lam x t | NilCtx HOLE <- ctx -> fromGE ErrorSpec $ propagateSpec spec <$> toCtx (show t) x t
...
instance {-# OVERLAPPABLE #-} NumLike fn a => Num (Term fn a) where
  (+) = app addFn
  negate = app negateFn
...
toPredsSizeSpec v (SizeSpec (NumSpecInterval Nothing (Just u))) = assert (v <=. Lit (Size u))
toPredsSizeSpec v (SizeSpec (NumSpecInterval (Just u) Nothing)) = assert (Lit (Size u) <=. v)
toPredsSizeSpec v (SizeSpec (NumSpecInterval (Just a) (Just b))) = assert [Lit (Size a) <=. v, v <=. Lit (Size b)]
