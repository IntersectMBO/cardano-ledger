...
instance Monad m => Applicative (GenT m) where
  pure a = GenT (\_ -> pure @Gen (pure @m a))
  (<*>) = ap

-- Honestly, I don't understand this, ask Max.
-- This might be an inlined use of the Gen monad transformer?
...
-- | Similar to (concat :: [[x]] -> [x]) for lists
concatNE :: NonEmpty (NonEmpty t) -> NonEmpty t
concatNE (x :| []) = x
concatNE (x :| (y : ys)) = x <> concatNE (y :| ys)
...
--   We use 'reverse' since messages are accumulated backwards.
catMessages :: (NonEmpty (NonEmpty String)) -> String
catMessages xs = unlines (reverse (NE.toList (catMessageList xs)))

-- | Turn each inner (NonEmpty String) into a String
catMessageList :: (NonEmpty (NonEmpty String)) -> NonEmpty String
catMessageList xs = fmap sh xs
  where
    sh x = unlines (NE.toList x)
...
resizeT f (GenT gm) = GenT $ \pair -> sized $ \sz -> resize (f sz) (gm pair)
...
getMess :: Applicative m => GenT m [NonEmpty String]
getMess = GenT $ \(_, msgs) -> pure (pure msgs)
...
oneofT gs = do
  ...
  r <-
    explain (pure "suchThatT in oneofT") $
      pureGen (oneof [runGenT g (mode, mess) | g <- gs]) `suchThatT` isOk
  runGE r
...
frequencyT gs = do
  ...
  r <-
    explain (pure "suchThatT in oneofT") $
      pureGen (frequency [(f, runGenT g (mode, mess)) | (f, g) <- gs]) `suchThatT` isOk
  runGE r
...
inspect (GenT f) = GenT g
  where
    g pair = do result <- f pair; pure @Gen (pure @m result)
...
push [] m = dropGen m
push (x : xs) m = dropGen $ explain (x :| xs) m
