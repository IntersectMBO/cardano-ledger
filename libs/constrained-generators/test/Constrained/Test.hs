...
conformsToSpecETest a speca =
  if conformsToSpec a speca
    then case (conformsToSpecE a speca (pure ("ConformsToSpecETest " ++ show a ++ " " ++ show speca))) of
      Nothing -> property (collect (specType speca ++ " both conform") True)
      Just xs -> counterexample (unlines (NE.toList xs)) False
    else case (conformsToSpecE a speca (pure ("ConformsToSpecETest " ++ show a ++ " " ++ show speca))) of
      Nothing ->
