...
spec =
  ...
  where
    inputs = [1 .. 10]
    ctx = TRC ((), 0, inputs)
    withSum = runReader (applySTS @GSUM ctx) (Ops (Foldable.foldl' (+) 0))
    withLblSum vp =
      right fst $
        runReader (applySTSOptsEither @GSUM lblOpts ctx) (Ops (Foldable.foldl' (+) 1))
      where
        lblOpts =
          ApplySTSOpts
    ...
    withProduct = runReader (applySTS @GSUM ctx) (Ops (Foldable.foldl' (*) 1))
    withLazyEventsSum =
      right fst $
        runReader (applySTSOptsEither @GSUM evtOpts ctx) (Ops (Foldable.foldl' (+) 0))
      where
        evtOpts =
          ApplySTSOpts
            { asoAssertions = AssertionsOff
