...
aggProp agg0 aggregate test trace = test firstState lastState (List.foldl' aggregate agg0 sigs)
  where
...
consistentUtxoSizeProp proof trace = aggProp agg0 aggregate makeprop trace
  where
    agg0 = 0
    aggregate count (MockBlock _ _ txs) = List.foldl' aggTx count txs
    aggTx count tx =
      count
        + ( if valid
