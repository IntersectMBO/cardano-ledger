...
import Cardano.Protocol.HeaderCrypto as HC
import qualified Cardano.Protocol.HeaderKeys as HC
...
defaultLedgerExamples proof value txBody auxData translationContext =
  ...
  where
    tx = exampleTx @era @hc proof txBody auxData
    ...
    resultExamples =
      ShelleyResultExamples
        ...
        , srePoolDistr = examplePoolDistr @(EraCrypto era) @hc
...
exampleLedgerBlock ::
  forall era hc.
  (Reflect era, PraosCrypto (EraCrypto era) hc) =>
  Proof era ->
  Tx era ->
...
exampleKeys :: forall c hc r. (Crypto c, HeaderCrypto hc) => AllIssuerKeys c hc r
exampleKeys =
  AllIssuerKeys
    coldKey
...
mkVRFKeyPair ::
  ...
  Proxy c ->
  Word8 ->
  (HC.SignKeyVRF c, HC.VerKeyVRF c)
...
exampleNewEpochState proof spendvalue ppp pp =
  NewEpochState
    ...
    , nesPd = examplePoolDistr @(EraCrypto era) @hc
...
examplePoolDistr :: forall c hc. PraosCrypto c hc => PoolDistr c
examplePoolDistr =
  PoolDistr $
    Map.fromList
      [
        ...
        , IndividualPoolStake
            1
            (toPoolStakeVRF $ hashVerKeyVRF (snd (vrf (exampleKeys @c @hc))))
        )
      ]
...
exampleTx proof txBody auxData =
  ...
  where
    ...
    keyPairWits =
      ...
      , asWitness $ cold (exampleKeys @(EraCrypto era) @hc @'StakePool)
      ]
...
exampleTxBody proof spendval =
  genericTxBody
    ...
    , Just (Certs $ exampleCerts @(EraCrypto era) @hc)
    , Just (Withdrawals' (exampleWithdrawals @era @hc proof))
...
exampleCerts :: forall c hc. (Crypto c, HeaderCrypto hc) => StrictSeq (DCert c)
exampleCerts =
  StrictSeq.fromList
    [ DCertDeleg (RegKey (keyToCredential exampleStakeKey))
    , DCertPool (RegPool $ examplePoolParams @c @hc)
    , DCertMir $
        MIRCert ReservesMIR $
          StakeAddressesMIR $
            Map.fromList
...
examplePoolParams :: forall c hc. (Crypto c, HeaderCrypto hc) => PoolParams c
examplePoolParams =
  PoolParams
    { ppId = hashKey $ vKey $ cold poolKeys
    , ppVrf = toPoolStakeVRF $ hashVerKeyVRF $ snd $ vrf poolKeys
    , ppPledge = Coin 1
    , ppCost = Coin 5
...
exampleWithdrawals proof =
  case proof of
    Shelley _ -> Withdrawals $ Map.fromList [(ppRewardAcnt $ examplePoolParams @(EraCrypto era) @hc, Coin 100)]
    Allegra _ -> Withdrawals $ Map.fromList [(ppRewardAcnt $ examplePoolParams @(EraCrypto era) @hc, Coin 100)]
    Mary _ -> Withdrawals $ Map.fromList [(ppRewardAcnt $ examplePoolParams @(EraCrypto era) @hc, Coin 100)]
    Alonzo _ ->
      Withdrawals $
        Map.singleton
...
ledgerExamplesShelley :: ShelleyLedgerExamples (ShelleyEra StandardCrypto) StandardCrypto
ledgerExamplesShelley =
  defaultLedgerExamples
    (Shelley Standard)
    exampleCoin
    (exampleTxBody @(ShelleyEra StandardCrypto) @StandardCrypto (Shelley Standard) (inject (Coin 100000)))
...
ledgerExamplesAllegra :: ShelleyLedgerExamples (AllegraEra StandardCrypto) StandardCrypto
ledgerExamplesAllegra =
  defaultLedgerExamples
    (Allegra Standard)
    exampleCoin
    (exampleTxBody @(AllegraEra StandardCrypto) @StandardCrypto (Allegra Standard) (inject exampleCoin))
...
ledgerExamplesMary :: ShelleyLedgerExamples (MaryEra StandardCrypto) StandardCrypto
ledgerExamplesMary =
  defaultLedgerExamples
    (Mary Standard)
    (exampleMultiAssetValue 1)
    ( exampleTxBody @(MaryEra StandardCrypto) @StandardCrypto
        (Mary Standard)
...
ledgerExamplesAlonzo :: ShelleyLedgerExamples (AlonzoEra StandardCrypto) StandardCrypto
ledgerExamplesAlonzo =
  defaultLedgerExamples
    (Alonzo Standard)
    (exampleMultiAssetValue 1)
    ( exampleTxBody @(AlonzoEra StandardCrypto) @StandardCrypto
        (Alonzo Standard)
...
ledgerExamplesBabbage :: ShelleyLedgerExamples (BabbageEra StandardCrypto) StandardCrypto
ledgerExamplesBabbage =
  defaultLedgerExamples
    (Babbage Standard)
    (exampleMultiAssetValue 1)
    ( exampleTxBody @(BabbageEra StandardCrypto) @StandardCrypto
        (Babbage Standard)
        (exampleMultiAssetValue @StandardCrypto 2)
    )
    exampleAlonzoTxAuxData
...
ledgerExamplesConway :: ShelleyLedgerExamples (ConwayEra StandardCrypto) StandardCrypto
ledgerExamplesConway =
  defaultLedgerExamples
    (Conway Standard)
    (exampleMultiAssetValue 1)
    ( exampleTxBody @(ConwayEra StandardCrypto) @StandardCrypto
        (Conway Standard)
        (exampleMultiAssetValue @StandardCrypto 2)
    )
    exampleAlonzoTxAuxData
...
ledgerExamples ::
  ...
  Proof era ->
  ShelleyLedgerExamples era StandardCrypto
