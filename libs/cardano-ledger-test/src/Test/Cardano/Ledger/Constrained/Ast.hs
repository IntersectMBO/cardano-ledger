...
import Cardano.Ledger.Core (Era (EraCrypto), EraPParams, EraScript (..), hashScript)
...
var :: EraPParams era => String -> Rep era t -> Term era t
var s r = Var (V s r No)

unVar :: EraPParams era => Term era t -> V era t
unVar (Var v) = v
unVar x = error ("Non Var in unVar: " ++ show x)

fieldToTerm :: EraPParams era => Field era rec field -> Term era field
...
select :: (Eq t, EraPParams era) => Term era t -> Term era big -> Lens' big t -> Pred era
...
select2 :: EraPParams era => Term era t -> Term era big -> Lens' big t -> Pred era
...
instance EraPParams era => Show (Term era t) where
...
instance EraPParams era => Show (Sum era c) where
...
instance EraPParams era => Show (Pred era) where
...
showAllTarget :: EraPParams era => RootTarget era r t -> [Char]
showAllTarget tar = "   forall " ++ showL show " " (HashSet.toList (varsOfTarget HashSet.empty tar)) ++ ". "

instance EraPParams era => Show (RootTarget era r t) where
...
showT :: forall era t r. EraPParams era => RootTarget era r t -> String
...
ppTarget :: EraPParams era => RootTarget era r t -> PDoc
ppTarget x = targetRecord x []

targetRecord :: EraPParams era => RootTarget era r t -> [(Text, PDoc)] -> PDoc
...
nameOf :: EraPParams era => RootTarget era r t -> Text
...
targetPair :: EraPParams era => RootTarget era r t -> (Text, PDoc)
...
varsOfTerm :: EraPParams era => HashSet (Name era) -> Term era t -> HashSet (Name era)
...
vars :: EraPParams era => Term era t -> HashSet (Name era)
vars x = varsOfTerm HashSet.empty x

varsOfTarget :: EraPParams era => HashSet (Name era) -> RootTarget era r t -> HashSet (Name era)
...
varsOfPred :: forall era. EraPParams era => HashSet (Name era) -> Pred era -> HashSet (Name era)
...
varsOfTrips :: EraPParams era => HashSet (Name era) -> [(Int, RootTarget era r t2, [Pred era])] -> HashSet (Name era)
...
varsOfPairs :: EraPParams era => HashSet (Name era) -> [(RootTarget era r t2, [Pred era])] -> HashSet (Name era)
...
varsOfPats :: EraPParams era => HashSet (Name era) -> [(Pat era t2, [Pred era])] -> HashSet (Name era)
...
varsOfSum :: EraPParams era => HashSet (Name era) -> Sum era r -> HashSet (Name era)
...
expandSum :: EraPParams era => Sum era c -> [Int] -> [Sum era c]
...
substToEnv :: EraPParams era => Subst era -> Env era -> Typed (Env era)
...
findV :: EraPParams era => Subst era -> V era t -> Term era t
...
singleSubst :: EraPParams era => V era t -> Term era t -> Subst era
singleSubst (V n r access) expr = Subst (Map.insert n (SubstElem r expr access) Map.empty)

substFromNames :: forall era. EraPParams era => HashSet (Name era) -> Subst era
...
instance EraPParams era => Show (SubItem era) where
  show (SubItem x y) = "(SubItem " ++ show x ++ " " ++ show y ++ ")"

itemsToSubst :: forall era. EraPParams era => [SubItem era] -> Subst era
...
substTerm :: EraPParams era => Subst era -> Term era t -> Term era t
...
substPred :: EraPParams era => Subst era -> Pred era -> Pred era
...
substPredWithVarTest :: EraPParams era => Subst era -> Pred era -> Pred era
...
substFromTarget :: EraPParams era => RootTarget era r t -> Subst era
substFromTarget tar = substFromNames (varsOfTarget HashSet.empty tar)

substFromPat :: EraPParams era => Pat era t -> Subst era
substFromPat pat = substFromNames (varsOfPat HashSet.empty pat)

substSum :: EraPParams era => Subst era -> Sum era t -> Sum era t
...
substTarget :: EraPParams era => Subst era -> RootTarget era r t -> RootTarget era r t
...
simplify :: EraPParams era => Term era t -> Typed t
...
simplifySum :: EraPParams era => Sum era c -> Typed c
...
simplifyTarget :: forall era t root. EraPParams era => RootTarget era root t -> Typed t
...
runTerm :: EraPParams era => Env era -> Term era t -> Typed t
...
runTarget :: EraPParams era => Env era -> RootTarget era x t -> Typed t
...
getTarget :: forall era root t. EraPParams era => root -> RootTarget era root t -> Env era -> Env era
...
targetMaybeEnv :: forall era root t. EraPParams era => root -> RootTarget era root t -> Env era -> Maybe (Env era)
...
runPred :: EraPParams era => Env era -> Pred era -> Typed Bool
...
extendableSumsTo :: EraPParams era => Pat era t -> Pred era -> Bool
...
runPreds :: EraPParams era => Env era -> [Pred era] -> Typed Bool
runPreds env ps = do
  bs <- mapM (runPred env) ps
  pure (and bs)

bind :: EraPParams era => RootTarget era r t -> t -> Env era -> Env era
...
runComp :: EraPParams era => Env era -> s -> AnyF era s -> Typed Bool
...
termRep :: EraPParams era => Term era t -> Rep era t
termRep (Lit r _) = r
...
runSum :: EraPParams era => Env era -> Sum era c -> Typed c
...
makeTest :: EraPParams era => Env era -> Pred era -> Typed (String, Bool, Pred era)
makeTest env c = do
  b <- runPred env c
  pure (show c ++ " => " ++ show b, b, c)

displayTerm :: EraPParams era => Env era -> Term era a -> IO ()
...
patt :: EraPParams era => Rep era t1 -> Term era t2 -> Pat era t1
patt rep term = Pat rep [arg rep term]

instance EraPParams era => Show (Pat era t) where
  show (Pat r xs) = "Pat " ++ show r ++ " " ++ showL show ", " xs
instance EraPParams era => Show (Arg era t) where
...
varsOfField :: EraPParams era => HashSet (Name era) -> Field era s t -> HashSet (Name era)
varsOfField l (Field n r rx l2) = HashSet.insert (Name $ V n r (Yes rx l2)) l
varsOfField l (FConst _ _ _ _) = l

varsOfPat :: EraPParams era => HashSet (Name era) -> Pat era t -> HashSet (Name era)
varsOfPat ans (Pat _ qs) = List.foldl' varsOfArg ans qs

varsOfArg :: EraPParams era => HashSet (Name era) -> Arg era t -> HashSet (Name era)
varsOfArg ans (ArgPs f qs) = List.foldl' varsOfPat (varsOfField ans f) qs
varsOfArg ans (Arg f) = varsOfField ans f

substField :: EraPParams era => Subst era -> Field era rec fld -> Field era rec fld
...
substPat :: EraPParams era => Subst era -> Pat era t -> Pat era t
substPat sub (Pat r as) = Pat r (map (substArg sub) as)

substArg :: EraPParams era => Subst era -> Arg era t -> Arg era t
substArg sub (ArgPs r as) = ArgPs (substField sub r) (map (substPat sub) as)
substArg sub (Arg r) = Arg (substField sub r)

bindPat :: EraPParams era => t -> Env era -> Pat era t -> Env era
bindPat t env (Pat _ as) = List.foldl' (bindArg t) env as

bindArg :: EraPParams era => t -> Env era -> Arg era t -> Env era
...
argP :: EraPParams era => Rep era s -> Term era t -> [Pat era t] -> Arg era s
...
arg :: EraPParams era => Rep era s -> Term era t -> Arg era s
