...
unVar :: AlonzoEraTest era => Term era t -> V era t
unVar (Var v) = v
...
select2 :: AlonzoEraTest era => Term era t -> Term era big -> Lens' big t -> Pred era
...
instance AlonzoEraTest era => Show (Term era t) where
...
instance AlonzoEraTest era => Show (Sum era c) where
...
instance AlonzoEraTest era => Show (Pred era) where
...
instance AlonzoEraTest era => Show (RootTarget era r t) where
...
showT :: forall era t r. AlonzoEraTest era => RootTarget era r t -> String
...
ppTarget :: AlonzoEraTest era => RootTarget era r t -> Expr
ppTarget x = targetRecord x []

targetRecord :: AlonzoEraTest era => RootTarget era r t -> [(Text, Expr)] -> Expr
...
nameOf :: AlonzoEraTest era => RootTarget era r t -> Text
...
targetPair :: AlonzoEraTest era => RootTarget era r t -> (Text, Expr)
...
expandSum :: AlonzoEraTest era => Sum era c -> [Int] -> [Sum era c]
...
instance AlonzoEraTest era => Show (SubstElem era) where
...
instance AlonzoEraTest era => Show (Subst era) where
...
substToEnv :: AlonzoEraTest era => Subst era -> Env era -> Typed (Env era)
...
instance AlonzoEraTest era => Show (SubItem era) where
...
substPredWithVarTest :: AlonzoEraTest era => Subst era -> Pred era -> Pred era
...
simplify :: AlonzoEraTest era => Term era t -> Typed t
...
simplifySum :: AlonzoEraTest era => Sum era c -> Typed c
...
simplifyTarget :: forall era t root. AlonzoEraTest era => RootTarget era root t -> Typed t
...
runPred :: AlonzoEraTest era => Env era -> Pred era -> Typed Bool
...
runPreds :: AlonzoEraTest era => Env era -> [Pred era] -> Typed Bool
runPreds env ps = do
  bs <- mapM (runPred env) ps
  pure (and bs)

bind :: AlonzoEraTest era => RootTarget era r t -> t -> Env era -> Env era
...
makeTest :: AlonzoEraTest era => Env era -> Pred era -> Typed (String, Bool, Pred era)
makeTest env c = do
  b <- runPred env c
  pure (show c ++ " => " ++ show b, b, c)

displayTerm :: AlonzoEraTest era => Env era -> Term era a -> IO ()
...
patt :: AlonzoEraTest era => Rep era t1 -> Term era t2 -> Pat era t1
patt rep term = Pat rep [arg rep term]

instance AlonzoEraTest era => Show (Pat era t) where
  show (Pat r xs) = "Pat " ++ show r ++ " " ++ showL show ", " xs

instance AlonzoEraTest era => Show (Arg era t) where
...
argP :: AlonzoEraTest era => Rep era s -> Term era t -> [Pat era t] -> Arg era s
...
arg :: AlonzoEraTest era => Rep era s -> Term era t -> Arg era s
