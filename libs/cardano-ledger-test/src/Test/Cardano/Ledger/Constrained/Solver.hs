...
simplifyAtType :: (Era era, ToExprs era) => Rep era s -> Term era t -> Typed s
...
simplifySet ::
  (Ord rng, Era era, ToExprs era) => Rep era rng -> Term era y -> Typed (HasConstraint Ord (Set rng))
...
simplifyList ::
  (Era era, ToExprs era) => Rep era rng -> Term era y -> Typed (HasConstraint Eq [rng])
...
solveMap ::
  forall dom rng era.
  (Era era, ToExprs era) => V era (Map dom rng) -> Pred era -> Typed (MapSpec era dom rng)
...
solveMaps ::
  (Era era, Ord dom, ToExprs era) => V era (Map dom rng) -> [Pred era] -> Typed (MapSpec era dom rng)
...
solveSet ::
  forall era a. (Era era, ToExprs era) => V era (Set a) -> Pred era -> Typed (SetSpec era a)
...
solveSets :: (Era era, ToExprs era) => V era (Set a) -> [Pred era] -> Typed (SetSpec era a)
...
solveSum ::
  forall era t. (Adds t, Era era, ToExprs era) => V era t -> Pred era -> Typed (AddsSpec t)
...
solveSums :: (Adds t, Era era, ToExprs era) => V era t -> [Pred era] -> Typed (AddsSpec t)
...
summandAsInt :: (Adds c, ToExprs era) => Sum era c -> Typed Int
...
genSum :: (Adds x, ToExprs era) => Sum era x -> Rep era x -> x -> Subst era -> Gen (Subst era)
...
summandsAsInt :: (Adds c, Era era, ToExprs era) => [Sum era c] -> Typed Int
...
intSumWithUniqueV :: (Adds c, ToExprs era) => V era t -> [Sum era c] -> Typed (Int, Bool)
...
--   which describes the constraints implied by the Pred 'predicate'
solveList :: (Era era, ToExprs era) => V era [a] -> Pred era -> Typed (ListSpec era a)
...
solveLists :: (Era era, ToExprs era) => V era [a] -> [Pred era] -> Typed (ListSpec era a)
...
genCount :: (Count t, ToExprs era) => V era t -> [Pred era] -> Typed (Gen t)
...
anyToUpdate :: ToExprs era => Rep era t1 -> (AnyF era t2) -> Typed (Update t1)
...
dispatch ::
  forall t era. (HasCallStack, Era era, ToExprs era) => V era t -> [Pred era] -> Typed (Gen t)
...
genDependGraph ::
  ToExprs era => Bool -> Proof era -> DependGraph era -> Gen (Either [String] (Subst era))
...
solveOneVar :: (Era era, ToExprs era) => Subst era -> ([Name era], [Pred era]) -> Gen (Subst era)
...
toolChainSub ::
  (Era era, ToExprs era) => Proof era -> OrderInfo -> [Pred era] -> Subst era -> Gen (Subst era)
...
toolChain ::
  (Era era, ToExprs era) => Proof era -> OrderInfo -> [Pred era] -> Subst era -> Gen (Env era)
