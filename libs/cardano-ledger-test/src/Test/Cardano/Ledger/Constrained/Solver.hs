...
simplifyAtType :: AlonzoEraTest era => Rep era s -> Term era t -> Typed s
...
simplifySet ::
  (Ord rng, AlonzoEraTest era) => Rep era rng -> Term era y -> Typed (HasConstraint Ord (Set rng))
...
simplifyList ::
  AlonzoEraTest era => Rep era rng -> Term era y -> Typed (HasConstraint Eq [rng])
...
solveMap ::
  forall dom rng era.
  AlonzoEraTest era => V era (Map dom rng) -> Pred era -> Typed (MapSpec era dom rng)
...
solveMaps ::
  (Ord dom, AlonzoEraTest era) => V era (Map dom rng) -> [Pred era] -> Typed (MapSpec era dom rng)
...
solveSet ::
  forall era a. AlonzoEraTest era => V era (Set a) -> Pred era -> Typed (SetSpec era a)
...
solveSets :: AlonzoEraTest era => V era (Set a) -> [Pred era] -> Typed (SetSpec era a)
...
solveSum ::
  forall era t. (Adds t, AlonzoEraTest era) => V era t -> Pred era -> Typed (AddsSpec t)
...
solveSums :: (Adds t, AlonzoEraTest era) => V era t -> [Pred era] -> Typed (AddsSpec t)
...
summandAsInt :: (Adds c, AlonzoEraTest era) => Sum era c -> Typed Int
...
genSum :: (Adds x, AlonzoEraTest era) => Sum era x -> Rep era x -> x -> Subst era -> Gen (Subst era)
...
summandsAsInt :: (Adds c, AlonzoEraTest era) => [Sum era c] -> Typed Int
...
intSumWithUniqueV :: (Adds c, AlonzoEraTest era) => V era t -> [Sum era c] -> Typed (Int, Bool)
...
--   which describes the constraints implied by the Pred 'predicate'
solveList :: AlonzoEraTest era => V era [a] -> Pred era -> Typed (ListSpec era a)
...
solveLists :: AlonzoEraTest era => V era [a] -> [Pred era] -> Typed (ListSpec era a)
...
genCount :: (Count t, AlonzoEraTest era) => V era t -> [Pred era] -> Typed (Gen t)
...
anyToUpdate :: AlonzoEraTest era => Rep era t1 -> AnyF era t2 -> Typed (Update t1)
...
dispatch ::
  forall t era. (HasCallStack, AlonzoEraTest era) => V era t -> [Pred era] -> Typed (Gen t)
...
-- TODO: revisit
genDependGraph ::
  AlonzoEraTest era => Bool -> DependGraph era -> Gen (Either [String] (Subst era))
...
solveOneVar :: AlonzoEraTest era => Subst era -> ([Name era], [Pred era]) -> Gen (Subst era)
...
toolChainSub ::
  AlonzoEraTest era => Proof era -> OrderInfo -> [Pred era] -> Subst era -> Gen (Subst era)
...
toolChain ::
  AlonzoEraTest era => Proof era -> OrderInfo -> [Pred era] -> Subst era -> Gen (Env era)
