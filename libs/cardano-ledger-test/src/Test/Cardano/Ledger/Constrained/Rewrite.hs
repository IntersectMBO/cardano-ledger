...
mkNewVar :: forall era d r. AlonzoEraTest era => Term era (Map d r) -> Term era (Set d)
...
removeTrivial :: AlonzoEraTest era => [Pred era] -> [Pred era]
...
rewrite :: AlonzoEraTest era => [Pred era] -> [Pred era]
...
extendSum :: AlonzoEraTest era => SubItems era -> Sum era c -> [Sum era c]
...
extendSums :: AlonzoEraTest era => SubItems era -> [Pred era] -> [Pred era]
extendSums _ [] = []
...
rewritePred :: AlonzoEraTest era => Int -> Pred era -> Gen ([Pred era], Int)
{-
...
removeExpandablePred ::
  AlonzoEraTest era => ([Pred era], Int) -> [Pred era] -> Gen ([Pred era], Int)
...
rewriteGen :: AlonzoEraTest era => (Int, [Pred era]) -> Gen (Int, [Pred era])
rewriteGen (m, cs0) = do
...
compileGenWithSubst ::
  AlonzoEraTest era => OrderInfo -> Subst era -> [Pred era] -> Gen (Int, DependGraph era)
...
instance AlonzoEraTest era => Show (DependGraph era) where
...
initialOrder :: forall era. AlonzoEraTest era => OrderInfo -> [Pred era] -> Typed [Name era]
...
-- | Construct the DependGraph
compile :: AlonzoEraTest era => OrderInfo -> [Pred era] -> Typed (DependGraph era)
compile info cs = do
