...
mkNewVar :: forall era d r. ToExprs era => Term era (Map d r) -> Term era (Set d)
...
removeTrivial :: (Era era, ToExprs era) => [Pred era] -> [Pred era]
...
rewrite :: (Era era, ToExprs era) => [Pred era] -> [Pred era]
...
extendSum :: ToExprs era => SubItems era -> Sum era c -> [Sum era c]
...
extendSums :: (Era era, ToExprs era) => SubItems era -> [Pred era] -> [Pred era]
extendSums _ [] = []
...
rewritePred :: (Era era, ToExprs era) => Int -> Pred era -> Gen ([Pred era], Int)
{-
...
removeExpandablePred ::
  (Era era, ToExprs era) => ([Pred era], Int) -> [Pred era] -> Gen ([Pred era], Int)
...
rewriteGen :: (Era era, ToExprs era) => (Int, [Pred era]) -> Gen (Int, [Pred era])
rewriteGen (m, cs0) = do
...
compileGenWithSubst ::
  (Era era, ToExprs era) => OrderInfo -> Subst era -> [Pred era] -> Gen (Int, DependGraph era)
...
instance (Era era, ToExprs era) => Show (DependGraph era) where
...
initialOrder :: forall era. (Era era, ToExprs era) => OrderInfo -> [Pred era] -> Typed [Name era]
...
-- | Construct the DependGraph
compile :: (Era era, ToExprs era) => OrderInfo -> [Pred era] -> Typed (DependGraph era)
compile info cs = do
