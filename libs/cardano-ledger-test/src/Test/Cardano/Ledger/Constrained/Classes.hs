...
data TxF era = TxF (Proof era) (Tx era)
  deriving (Generic)
...
instance Reflect era => Show (PlutusPurposeF era) where
  show (PlutusPurposeF _p x) = case reify @era of
    Shelley -> error "Impossible. `PlutusPurpose AsIxItem` doesn't exist pre-Alonzo."
    Allegra -> error "Impossible. `PlutusPurpose AsIxItem` doesn't exist pre-Alonzo."
    Mary -> error "Impossible. `PlutusPurpose AsIxItem` doesn't exist pre-Alonzo."
    Alonzo -> show $ toExpr x
    Babbage -> show $ toExpr x
    Conway -> show $ toExpr x

instance Reflect era => Show (PlutusPointerF era) where
  show (PlutusPointerF _p x) = case reify @era of
    Shelley -> error "Impossible. `PlutusPurpose AsIx` doesn't exist pre-Alonzo."
    Allegra -> error "Impossible. `PlutusPurpose AsIx` doesn't exist pre-Alonzo."
    Mary -> error "Impossible. `PlutusPurpose AsIx` doesn't exist pre-Alonzo."
    Alonzo -> show $ toExpr x
    Babbage -> show $ toExpr x
    Conway -> show $ toExpr x
...
instance ToExpr (CertState era) => Show (CertStateF era) where
  show (CertStateF _p c) = show $ toExpr c
...
instance Reflect era => Show (ScriptsNeededF era) where
  show (ScriptsNeededF _p t) = case reify @era of
    Conway -> show $ toExpr t
    Babbage -> show $ toExpr t
    Alonzo -> show $ toExpr t
    Mary -> show $ toExpr t
    Allegra -> show $ toExpr t
    Shelley -> show $ toExpr t
