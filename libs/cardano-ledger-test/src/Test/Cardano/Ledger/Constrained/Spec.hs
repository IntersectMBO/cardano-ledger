...
instance AlonzoEraTest era => Monoid (RelSpec era dom) where
  mempty = RelAny

instance AlonzoEraTest era => Semigroup (RelSpec era dom) where
  (<>) = mergeRelSpec

instance AlonzoEraTest era => Show (RelSpec era dom) where
  show = showRelSpec
...
showRelSpec :: AlonzoEraTest era => RelSpec era dom -> String
...
mergeRelSpec :: AlonzoEraTest era => RelSpec era d -> RelSpec era d -> RelSpec era d
...
maybeSynopsis :: AlonzoEraTest e => Rep e t -> Maybe t -> String
maybeSynopsis r (Just x) = format r x
maybeSynopsis _ _ = ""

synSet :: (Ord t, AlonzoEraTest era) => Rep era t -> Set t -> String
...
relOper ::
  (Ord d, AlonzoEraTest era) => Rep era d -> Set d -> Maybe (Set d) -> Set d -> Typed (RelSpec era d)
relOper r must may cant =
...
genFromRelSpec ::
  forall era t.
  (Ord t, AlonzoEraTest era) => [String] -> Gen t -> Int -> RelSpec era t -> Gen (Set t)
...
genRelSpec ::
  (Ord dom, AlonzoEraTest era) => [String] -> Gen dom -> Rep era dom -> Int -> Gen (RelSpec era dom)
...
genConsistentRelSpec ::
  AlonzoEraTest era => [String] -> Gen dom -> RelSpec era dom -> Gen (RelSpec era dom)
...
testConsistentRel :: forall era. AlonzoEraTest era => Gen Property
...
instance AlonzoEraTest era => Show (RngSpec era t) where
  show = showRngSpec

instance (Era era, AlonzoEraTest era) => Monoid (RngSpec era rng) where
  mempty = RngAny

instance (Era era, AlonzoEraTest era) => Semigroup (RngSpec era rng) where
  (<>) = mergeRngSpec
...
showRngSpec :: AlonzoEraTest era => RngSpec era t -> String
...
mergeRngSpec :: forall r era. AlonzoEraTest era => RngSpec era r -> RngSpec era r -> RngSpec era r
...
genFromRngSpec ::
  forall era r. AlonzoEraTest era => [String] -> Gen r -> Int -> RngSpec era r -> Gen [r]
...
genRngSpec ::
  forall w era.
  (Ord w, Adds w, AlonzoEraTest era) =>
  Gen w ->
  Rep era w ->
  -- Rep era c ->
  SomeLens era w ->
...
instance (Ord d, AlonzoEraTest w) => Show (MapSpec w d r) where
  show = showMapSpec

instance (Ord dom, Era era, AlonzoEraTest era) => Semigroup (MapSpec era dom rng) where
  (<>) = mergeMapSpec

instance (Ord dom, Era era, AlonzoEraTest era) => Monoid (MapSpec era dom rng) where
...
showMapSpec :: AlonzoEraTest era => MapSpec era dom rng -> String
...
mergeMapSpec ::
  (Ord dom, AlonzoEraTest era) => MapSpec era dom rng -> MapSpec era dom rng -> MapSpec era dom rng
...
genMapSpec ::
  forall era dom w.
  (Ord dom, Ord w, Adds w, AlonzoEraTest era) =>
  Gen dom ->
  Rep era dom ->
  Rep era w ->
  -- Rep era c ->
...
remove :: (Eq a, AlonzoEraTest era) => PairSide -> String -> Rep era a -> a -> [a] -> [a]
...
instance AlonzoEraTest era => Show (SetSpec era a) where show = showSetSpec

instance (Ord a, AlonzoEraTest era) => Semigroup (SetSpec era a) where
  (<>) = mergeSetSpec

instance (Ord a, AlonzoEraTest era) => Monoid (SetSpec era a) where
...
showSetSpec :: AlonzoEraTest era => SetSpec era a -> String
showSetSpec (SetSpec s r) = sepsP ["SetSpec", show s, show r]
showSetSpec (SetNever _) = "SetNever"

mergeSetSpec :: (Ord a, AlonzoEraTest era) => SetSpec era a -> SetSpec era a -> SetSpec era a
...
setSpec :: (Ord t, AlonzoEraTest era) => Size -> RelSpec era t -> Typed (SetSpec era t)
...
genSetSpec ::
  (Ord s, AlonzoEraTest era) => [String] -> Gen s -> Rep era s -> Int -> Gen (SetSpec era s)
genSetSpec msgs genS repS size = do
  r <- genRelSpec ("from genSetSpec" : msgs) genS repS size
  pure (SetSpec (SzExact size) r)

genFromSetSpec ::
  forall era a. AlonzoEraTest era => [String] -> Gen a -> SetSpec era a -> Gen (Set a)
...
instance AlonzoEraTest era => Show (ElemSpec era a) where
  show = showElemSpec

instance (Era era, AlonzoEraTest era) => Semigroup (ElemSpec era a) where
  (<>) = mergeElemSpec

instance (Era era, AlonzoEraTest era) => Monoid (ElemSpec era a) where
  mempty = ElemAny
...
showElemSpec :: AlonzoEraTest era => ElemSpec era a -> String
...
mergeElemSpec :: AlonzoEraTest era => ElemSpec era a -> ElemSpec era a -> ElemSpec era a
...
instance AlonzoEraTest era => Show (ListSpec era a) where
  show = showListSpec

instance (Era era, AlonzoEraTest era) => Semigroup (ListSpec era a) where
  (<>) = mergeListSpec

instance (Era era, AlonzoEraTest era) => Monoid (ListSpec era a) where
...
showListSpec :: AlonzoEraTest era => ListSpec era a -> String
showListSpec (ListSpec s xs) = sepsP ["ListSpec", show s, show xs]
showListSpec (ListNever _) = "ListNever"

mergeListSpec :: AlonzoEraTest era => ListSpec era a -> ListSpec era a -> ListSpec era a
...
listSpec :: AlonzoEraTest era => Size -> ElemSpec era t -> Typed (ListSpec era t)
...
aMap :: AlonzoEraTest era => Gen (MapSpec era Int Word64)
...
solveSumsTo :: AlonzoEraTest era => Pred era -> AddsSpec DeltaCoin
...
instance AlonzoEraTest era => Monoid (PairSpec era a b) where
  mempty = PairAny

instance AlonzoEraTest era => Semigroup (PairSpec era dom rng) where
  (<>) = mergePairSpec

instance AlonzoEraTest era => Show (PairSpec era dom rng) where
  show = showPairSpec
...
showPairSpec :: AlonzoEraTest era => PairSpec era dom rng -> String
...
mergePairSpec :: AlonzoEraTest era => PairSpec era a b -> PairSpec era a b -> PairSpec era a b
...
genFromPairSpec ::
  forall era dom rng.
  (Ord dom, AlonzoEraTest era) => [String] -> PairSpec era dom rng -> Gen (Map dom rng)
