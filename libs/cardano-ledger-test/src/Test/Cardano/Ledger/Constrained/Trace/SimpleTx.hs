...
getSTSLedgerEnv ::
  (Reflect era, EraTest era) => Proof era -> TxIx -> Env era -> Typed (LedgerEnv era, LedgerState era)
...
simpleTxBody ::
  (Reflect era, AlonzoEraTest era) => Proof era -> Coin -> TraceM era [TxBodyField era]
...
simpleTx :: (Reflect era, AlonzoEraTest era) => Proof era -> Coin -> TraceM era (Tx era)
...
--   Only works if the internal Env of the TraceM monad stores variables capable of creating the LedgerState
completeTxBody ::
  (Reflect era, AlonzoEraTest era) => Proof era -> Coin -> TxBody era -> TraceM era (Tx era)
