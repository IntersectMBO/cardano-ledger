...
import Cardano.Ledger.Core (EraRule, Tx)
...
pipeToGraph :: AlonzoEraTest era => Stage era -> HashSet (Name era) -> TraceM era (DependGraph era)
...
mergePipeline ::
  AlonzoEraTest era =>
  Pipeline era -> HashSet (Name era) -> DependGraph era -> TraceM era (DependGraph era)
...
solvePipeline2 :: AlonzoEraTest era => Pipeline era -> TraceM era (Env era, DependGraph era)
...
genSig :: (Reflect era, AlonzoEraTest era) => Proof era -> (a, Subst era, b) -> p -> Gen (Tx era)
