...
getTerm :: AlonzoEraTest era => Term era a -> TraceM era a
getTerm term = do
  env <- getEnv
...
getTarget :: AlonzoEraTest era => RootTarget era r a -> TraceM era a
getTarget tar = do
  env <- getEnv
...
fromMapTerm :: AlonzoEraTest era => Term era (Map k a) -> TraceM era (k, a)
fromMapTerm term = do
  u <- getTerm term
...
fromMapTermSuchThat ::
  AlonzoEraTest era => Term era (Map k a) -> ((k, a) -> Bool) -> TraceM era (k, a)
...
fromSetTerm :: AlonzoEraTest era => Term era (Set b) -> TraceM era b
fromSetTerm term = do
  u <- getTerm term
...
updateVar :: AlonzoEraTest era => Term era t -> (t -> t) -> TraceM era ()
...
setVar :: AlonzoEraTest era => Term era t -> t -> TraceM era ()
...
compileTraceWithSubst ::
  AlonzoEraTest era => OrderInfo -> Subst era -> [Pred era] -> TraceM era (DependGraph era)
...
toolChainTrace ::
  AlonzoEraTest era => Proof era -> OrderInfo -> [Pred era] -> Subst era -> TraceM era (Subst era)
...
universeTrace ::
  (Reflect era, AlonzoEraTest era) => Proof era -> Subst era -> TraceM era (Subst era)
universeTrace proof subst = liftGen (universeStage def proof subst)

pparamsTrace :: (Reflect era, AlonzoEraTest era) => Proof era -> Subst era -> TraceM era (Subst era)
pparamsTrace proof subst = liftGen (pParamsStage proof subst)

utxoTrace :: (Reflect era, AlonzoEraTest era) => Proof era -> Subst era -> TraceM era (Subst era)
utxoTrace proof subst = liftGen (utxoStage def proof subst)

pstateTrace :: (Reflect era, AlonzoEraTest era) => Proof era -> Subst era -> TraceM era (Subst era)
pstateTrace proof subst = liftGen (pstateStage proof subst)

vstateTrace :: AlonzoEraTest era => Proof era -> Subst era -> TraceM era (Subst era)
vstateTrace proof subst = liftGen (vstateStage proof subst)

dstateTrace :: (Reflect era, AlonzoEraTest era) => Proof era -> Subst era -> TraceM era (Subst era)
...
genLedgerStateEnv :: (Reflect era, AlonzoEraTest era) => Proof era -> TraceM era (Env era)
...
--   by chaining smaller pieces together.
genNewEpochStateEnv :: (Reflect era, AlonzoEraTest era) => Proof era -> TraceM era (Env era)
...
mockChainProp ::
  forall era.
  (Reflect era, STS (MOCKCHAIN era), AlonzoEraTest era) =>
  Proof era ->
  Int ->
  (Proof era -> TraceM era (Tx era)) ->
