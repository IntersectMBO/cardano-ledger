...
import Test.Cardano.Ledger.Constrained.Conway.ParametricSpec (EraOut (irewardSpec))
...
dsX :: forall era. EraLedger era ConwayFn => Gen (DState era)
dsX = do
...
csX :: forall era. EraLedger era ConwayFn => Gen (CertState era)
csX = do
  acct <- genFromSpec @ConwayFn @AccountState accountStateSpec
  epoch <- genFromSpec @ConwayFn @EpochNo epochNoSpec
  genFromSpec @ConwayFn @(CertState era) (certStateSpec (lit acct) (lit epoch))

utxoX :: forall era. EraLedger era ConwayFn => Gen (UTxO era)
utxoX = do
  cs <-
    genFromSpec
      @ConwayFn
...
utxostateX :: forall era. EraLedger era ConwayFn => PParams era -> Gen (UTxOState era)
utxostateX pp = do
  certstate <- csX @era
...
lsX :: forall era. EraLedger era ConwayFn => PParams era -> Gen (LedgerState era)
lsX pp = do
  acct <- genFromSpec @ConwayFn @AccountState accountStateSpec
  epoch <- genFromSpec @ConwayFn @EpochNo epochNoSpec
  genFromSpec @ConwayFn @(LedgerState era) (ledgerStateSpec pp (lit acct) (lit epoch))

esX :: forall era. EraLedger era ConwayFn => PParams era -> Gen (EpochState era)
esX pp = do
  epoch <- genFromSpec @ConwayFn @EpochNo epochNoSpec
  genFromSpec @ConwayFn @(EpochState era) (epochStateSpec pp (lit epoch))

nesX :: forall era. EraLedger era ConwayFn => PParams era -> Gen (NewEpochState era)
...
snapsX :: forall era. EraLedger era ConwayFn => PParams era -> Gen (SnapShots (EraCrypto era))
snapsX pp = do
  acct <- genFromSpec @ConwayFn @AccountState accountStateSpec
  epoch <- genFromSpec @ConwayFn @EpochNo epochNoSpec
...
instanRewX :: forall era. EraOut era ConwayFn => Gen (InstantaneousRewards (EraCrypto era))
instanRewX = do
  acct <- genFromSpec @ConwayFn @AccountState accountStateSpec
...
instance (EraPP era, EraLedger era ConwayFn) => WellFormed (DState era) era where
  wff = dsX
  wffWithPP _ = dsX
...
instance (EraPP era, EraLedger era ConwayFn) => WellFormed (CertState era) era where
  wff = csX

instance (EraPP era, EraLedger era ConwayFn) => WellFormed (UTxO era) era where
  wff = utxoX

instance (EraPP era, EraLedger era ConwayFn) => WellFormed (UTxOState era) era where
...
instance (EraPP era, EraLedger era ConwayFn) => WellFormed (ShelleyGovState era) era where
  wffWithPP pp = genFromSpec @ConwayFn @(ShelleyGovState era) (shelleyGovStateSpec pp)

instance (EraPP era, EraLedger era ConwayFn) => WellFormed (LedgerState era) era where
  wffWithPP = lsX

instance (EraPP era, EraLedger era ConwayFn) => WellFormed (EpochState era) era where
  wffWithPP = esX

instance (EraPP era, EraLedger era ConwayFn) => WellFormed (NewEpochState era) era where
  wffWithPP = nesX

instance (EraPP era, c ~ EraCrypto era) => WellFormed (SnapShot c) era where
  wff = snapX @era

instance (EraPP era, EraLedger era ConwayFn, c ~ EraCrypto era) => WellFormed (SnapShots c) era where
  wffWithPP = snapsX @era

instance (EraPP era, c ~ EraCrypto era, EraOut era ConwayFn) => WellFormed (InstantaneousRewards c) era where
  wff = instanRewX @era
