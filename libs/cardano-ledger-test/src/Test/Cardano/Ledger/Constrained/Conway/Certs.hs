...
-- At some point the Spec will change so that only the keyhash credentials need be delegated to a DRep
...
bootstrapDStateSpec withdrawals =
  let withdrawalPairs = Map.toList (Map.mapKeys snd (Map.map coinToWord64 withdrawals))
...
certsEnvSpec = constrained $ \ce ->
  match ce $ \tx pp _ _ _ _ -> [satisfies pp pparamsSpec, assert $ tx ==. lit txZero]
