...
certsEnvSpec ::
  (EraPP era, HasSpec fn (Tx era), IsConwayUniv fn) =>
...
noSameKeys :: EraCert era => [TxCert era] -> [TxCert era]
noSameKeys [] = []
...
listSeqCertPairSpec ::
  (EraCert era, IsConwayUniv fn, HasSpec fn (TxCert era)) =>
  CertEnv era ->
  CertState era ->
...
listSeqCertPairSpec env state =
  constrained' $ \list seqs ->
    ...
    , forAll list $ \x -> satisfies x (txCertSpec env state)
