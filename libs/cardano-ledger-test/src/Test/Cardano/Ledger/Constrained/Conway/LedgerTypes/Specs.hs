...
-- | The class (EraLedger era) supports Era parametric Specs over types that appear in the Cardano Ledger.223
...
--   and the EraLedger class has methods that asbtract over those changes.
class
  ( EraOut era fn
...
instance IsConwayUniv fn => EraLedger Shelley fn where
  govStateSpec = shelleyGovStateSpec
  newEpochStateSpec = newEpochStateSpecUTxO

instance IsConwayUniv fn => EraLedger Allegra fn where
  govStateSpec = shelleyGovStateSpec
  newEpochStateSpec = newEpochStateSpecUnit

instance IsConwayUniv fn => EraLedger Mary fn where
  govStateSpec = shelleyGovStateSpec
  newEpochStateSpec = newEpochStateSpecUnit

instance IsConwayUniv fn => EraLedger Alonzo fn where
  govStateSpec = shelleyGovStateSpec
  newEpochStateSpec = newEpochStateSpecUnit

instance IsConwayUniv fn => EraLedger Babbage fn where
  govStateSpec = shelleyGovStateSpec
  newEpochStateSpec = newEpochStateSpecUnit

instance IsConwayUniv fn => EraLedger Conway fn where
...
-- ================================================================================
-- Specifications for types that appear in the EraLedger Ledger
...
shelleyGovStateSpec ::
  forall era fn. EraLedger era fn => PParams era -> Specification fn (ShelleyGovState era)
...
-- The 'newEpochStateSpec' method (of (EraLedger era fn) class) in the Shelley instance
newEpochStateSpecUTxO ::
  forall era fn.
  (EraLedger era fn, StashedAVVMAddresses era ~ UTxO era) =>
  PParams era ->
...
-- The 'newEpochStateSpec' method (of (EraLedger era fn) class) in the instances for (Allegra,Mary,Alonzo,Babbage,Conway)
newEpochStateSpecUnit ::
  forall era fn.
  (EraLedger era fn, StashedAVVMAddresses era ~ ()) =>
  PParams era ->
  Specification fn (NewEpochState era)
