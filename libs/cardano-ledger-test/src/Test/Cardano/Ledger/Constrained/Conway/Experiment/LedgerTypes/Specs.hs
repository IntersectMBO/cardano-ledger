...
module Test.Cardano.Ledger.Constrained.Conway.Experiment.LedgerTypes.Specs where
...
import Test.Cardano.Ledger.Constrained.Conway.Experiment.Deleg (EraSpecDeleg (hasGenDelegs))
import Test.Cardano.Ledger.Constrained.Conway.Experiment.Gov (govProposalsSpec)
import Test.Cardano.Ledger.Constrained.Conway.Experiment.Instances
import Test.Cardano.Ledger.Constrained.Conway.Experiment.ParametricSpec (
  EraSpecTxOut (..),
  txOutSpec,
 )
...
newEpochStateSpecUTxO ::
  forall era.
  (EraSpecLedger era, StashedAVVMAddresses era ~ UTxO era, CertState era ~ ShelleyCertState era) =>
  PParams era ->
  WitUniv era ->
...
newEpochStateSpecUnit ::
  forall era.
  (EraSpecLedger era, StashedAVVMAddresses era ~ (), CertState era ~ ShelleyCertState era) =>
  PParams era ->
  WitUniv era ->
  Specification (NewEpochState era)
