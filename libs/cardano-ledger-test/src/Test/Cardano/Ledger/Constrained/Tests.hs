  ...
  where
    ...
    sumsToC
      | sumBeforeParts (gOrder env) =
          -- Known sum
          withValue (genTerm' env CoinR (> Coin 10) (Lit CoinR . (<> Coin 10) <$> arbitrary) KnownTerm) $ \sumTm val env' -> do
                ...
                genParts (n : ns) env0 = do
                  (t, env1) <-
                    genTerm'
                      ...
                      (MapR CoinR CoinR)
                      ((== n) . fold)
                      (Lit (MapR CoinR CoinR) <$> mapWithSum n)
                      (VarTerm d)
...
showPred (HasDom m d) = "HasDomain " ++ showTerm m ++ " " ++ showTerm d
showPred pr = show pr
