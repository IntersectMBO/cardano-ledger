...
data Decode (w :: Wrapped) t where
  ...
  FromGroup :: (EncCBORGroup t, DecCBORGroup t) => Proxy t -> Decode w t
...
hsize (KeyedD _) = 0
hsize From = 1
hsize (FromGroup proxy) = fromIntegral $ listLenBound proxy
hsize (D _) = 1
...
decodeCount (FromGroup _) n = (n,) <$> decCBORGroup
...
decodeClosed (FromGroup _) = decCBORGroup
