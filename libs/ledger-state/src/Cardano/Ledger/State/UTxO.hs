...
-- Need to answer:
-- - HeapWords size of UTxO
-- - Number of unique TxID, total number duplicates. What % are duplicates
-- - Number of unique Address by type
--    payment addresses  counts of duplicates
--    staking addresses    "
--    ptr addresses        "
-- - how many non-coin (Token) values in TxOut, what percent of TxOut have Tokens
--    what amount of sharing in AssetName
--    what amount of sharing in PolicyID

--  withSourceFile fp $ \c -> runConduit $ c .| (sinkParser prefixParser *> sinkUTxO) .| lengthC
-- where
--   reportStats = foldM

-- loadUTxO :: FilePath -> IO (Map.Map (TxIn C) (Alonzo.TxOut CurrentEra))
-- loadUTxO fp = do
--   UTxO utxo <- withSourceFile fp $ \c ->
--     runConduit $ do
--       Object hm <- c .| sinkParser json'
--       sourceList (HM.toList hm) .| parseObject

-- loadUTxO :: FilePath -> IO (Map.Map (TxIn C) (Alonzo.TxOut CurrentEra))
-- loadUTxO fp = do
--   eitherDecodeFileStrict' fp >>= \case
--     Left err -> mempty <$ putStrLn err
--     Right (UTxO m) -> pure m
...
--   Map (TxId (Crypto era)) (IntMap (TxOut era))
--   , 
