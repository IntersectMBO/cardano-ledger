...
import Cardano.Base.Bytes (byteArrayToByteString, byteStringToByteArray, splitsAt)
...
instance VRFAlgorithm FakeVRF where
  ...
  rawSerialiseCertVRF (CertFakeVRF k s (OutputVRF ba)) =
    writeBinaryWord64 k <> writeBinaryWord16 s <> byteArrayToByteString ba
