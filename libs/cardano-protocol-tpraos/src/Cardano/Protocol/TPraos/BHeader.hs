...
data BHeaderRaw c = BHeaderRaw
  { bhrBody :: !(BHBody c)
  , bhrSig :: !(KES.SignedKES (KES c) (BHBody c))
  }
...
newtype BHeader c = BHeaderConstr (MemoBytes (BHeaderRaw c))
  deriving (Generic)
  deriving newtype (Eq, Show, NoThunks, Plain.ToCBOR, DecCBOR)
...
bhHash bh@(BHeader bhb _) = HashHeader . Hash.castHash . hashEncCBOR version $ bh
  where
    version = pvMajor (bprotver bhb)
