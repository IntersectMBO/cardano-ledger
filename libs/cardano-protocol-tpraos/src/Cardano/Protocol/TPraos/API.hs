...
-- | Preserves old semantics: does not force fields (especially the lazy
-- @lvExtraEntropy@ which may be @error@ in Babbage).
...
instance DecCBOR ChainDepState where
  decCBOR = decode (RecD ChainDepState <! From <! From <! From)
  {-# INLINE decCBOR #-}
  ...
  toCBOR
    ChainDepState
      ...
      mconcat
        ...
        , toCBOR csProtocol
        , toCBOR csTickn
        , toPlainEncoding shelleyProtVer (encCBOR csLabNonce)
        ]
