...
\begin{description}
\item[Byron/Ouroboros] In the Byron/Ouroboros era, the Ouroboros\todo{Reference}
...
\item[Handover] In the handover era, blocks will be issued according to
  Ouroboros BFT\todo{Reference}. The Byron payload will be retained, although
  parts of will be superfluous.
\item[Shelley/Praos] In the Shelley/Praos era, blocks will be issued according
  to the Ouroboros Praos\todo{Reference} protocol, with stake distribution
  determined according to the new delegation design in \todo{Reference}.
\end{description}
...
\begin{enumerate}
 \item We check that the current block is being issued for a slot later than
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
  \caption{Progressive BFT types and functions}
  \label{fig:defs:pbft}
\end{figure}

\begin{figure}[ht]
  \emph{Progressive BFT environments}
  \begin{equation*}
    \PBFTEnv =
    \left(
      \begin{array}{r@{~\in~}lr}
