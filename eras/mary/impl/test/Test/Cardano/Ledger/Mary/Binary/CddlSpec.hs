...
spec =
  describe "CDDL" $ do
    ...
    describe "Ruby-based" $ beforeAllCddlFile 3 readMaryCddlFiles $ do
      ...
      cddlRoundTripAnnCborSpec @(TxBody TopTx MaryEra) v "transaction_body"
      cddlRoundTripCborSpec @(TxBody TopTx MaryEra) v "transaction_body"
      ...
      describe "DecCBOR instances equivalence via CDDL" $ do
        cddlDecoderEquivalenceSpec @(TxBody TopTx MaryEra) v "transaction_body"
