...
module Test.Cardano.Ledger.Conway.Rules.Chain (
  CHAIN,
  ChainState (..),
...
import Cardano.Ledger.Conway.Rules.Bbody (ConwayBbodyPredFailure, ConwayBbodyState (..))
...
type instance EraRule "TICKN" (ConwayEra c) = TICKN
...
initialConwayState ::
  ...
  Nonce ->
  ChainState era
initialConwayState lab e utxo reserves genDelegs pp initNonce =
  ChainState
    ( NewEpochState
        e
...
instance
  ...
  , Era era
  , STS (ConwayBBODY era)
  , PredicateFailure (EraRule "BBODY" era) ~ ConwayBbodyPredFailure era
  , Event (EraRule "BBODY" era) ~ Event (ConwayBBODY era)
  ) =>
  Embed (ConwayBBODY era) (CHAIN era)
  where
