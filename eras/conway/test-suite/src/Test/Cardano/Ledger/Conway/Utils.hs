...
module Test.Cardano.Ledger.Conway.Utils (
  mkSeedFromWords,
  mkCertifiedVRF,
  epochFromSlotNo,
  evolveKESUntil,
...
import Cardano.Ledger.Conway.API.Validation (ApplyBlock)
...
import Test.Cardano.Ledger.Shelley.Generator.Core (RawSeed (RawSeed))
...
genMultiAsset :: forall c. Crypto c => Integer -> Gen (MultiAsset c)
genMultiAsset amount = do
  MultiAsset <$> genNonEmptyMap arbitrary (genNonEmptyMap arbitrary (pure amount))