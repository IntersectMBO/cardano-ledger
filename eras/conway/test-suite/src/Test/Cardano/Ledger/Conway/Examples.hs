...
import Cardano.Ledger.Conway.Era (ConwayBBODY)
import Cardano.Ledger.Conway.Rules
import Cardano.Protocol.TPraos.BHeader (BHeader)
import Control.State.Transition.Extended hiding (Assertion)
import Data.List.NonEmpty (NonEmpty)
import Test.Cardano.Ledger.Conway.Rules.Chain (
  CHAIN,
...
import Test.Cardano.Ledger.Conway.TreeDiff (expectExprEqual)
import Test.Cardano.Ledger.Conway.Utils (applySTSTest, maxLLSupply, runShelleyBase)
...
type A = ConwayEra TestCrypto

instance Embed (ConwayBBODY A) (CHAIN A) where
...
testCHAINExample :: CHAINExample (BHeader StandardCrypto) (ConwayEra StandardCrypto) -> Assertion
testCHAINExample (CHAINExample initSt block (Right expectedSt)) = do
  ( checkTrace @(CHAIN (ConwayEra StandardCrypto))
      runShelleyBase
      ()
...
testCHAINExample (CHAINExample initSt block predicateFailure@(Left _)) = do
  let st = runShelleyBase $ applySTSTest @(CHAIN (ConwayEra StandardCrypto)) (TRC ((), initSt, block))
  st @?= predicateFailure