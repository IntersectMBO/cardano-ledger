...
noStakeProp =
  prop @((RatifyEnv era, RatifyState era, GovActionState era) -> IO ())
    ...
    ( \(re, rs, gas) ->
        let re' = re {reVotingStakePoolDistr = def & vspdTotalVotingStakeL .~ knownNonZeroCoin @100}
...
genTestData Ratios {yes, no, abstain, alwaysAbstain, noConfidence} = do
  ...
  distr <- do
    ...
    pure $ VotingStakePoolDistr {vspdIndividualStake = distr, vspdTotalVotingStake = totalStake}
