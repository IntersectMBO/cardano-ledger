...
import Test.Cardano.Ledger.Imp.Common (Gen, NonNegative(NonNegative), arbitrary, choose, chooseInt, Positive(Positive))
...
import Lens.Micro ((.~), (<>~))
import Cardano.Ledger.Coin (CompactForm (CompactCoin), Coin (Coin))
...
genAlonzoBasedPParams
  :: forall era. AlonzoEraPParams era
  => Gen (PParams era)
...
genBabbageEraPParams
  :: forall era. BabbageEraPParams era
  => Gen (PParams era)
...
genConwayBasedEraPParams
  :: forall era. ConwayEraPParams era
  => Gen (PParams era)
