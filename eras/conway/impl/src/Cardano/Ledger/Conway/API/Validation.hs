...
import Cardano.Ledger.Conway.Rules.Bbody (ConwayBbodyState (BbodyState))
...
instance
  ( Crypto c
  , DSignable c (Hash c EraIndependentTxBody)
  , STS (EraRule "TICK" (ConwayEra c))
  , STS (EraRule "BBODY" (ConwayEra c))
  ) =>
  ApplyBlock (ConwayEra c)
...
updateNewEpochState ::
  (LedgerState era ~ State (EraRule "ZONES" era), EraGov era) =>
  NewEpochState era ->
  ConwayBbodyState era ->
  NewEpochState era
updateNewEpochState ss (BbodyState ls bcur) =
