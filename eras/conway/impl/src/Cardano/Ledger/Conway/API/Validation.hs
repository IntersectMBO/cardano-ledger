...
-- API.
module Cardano.Ledger.Conway.API.Validation (
-- ApplyBlock (..),
-- applyBlock,
-- applyTick,
-- TickTransitionError (..),
...
-- import Cardano.Ledger.Conway.LedgerState.Types (UTxOStateTemp)
...
-- data LedgerState2 era where
--   LedgerState2 :: forall era. LedgerState era -> LedgerState2 era
--   LedgerState2New ::
--     !(UTxOStateTemp (ConwayEra c)) -> !(CertState (ConwayEra c)) -> LedgerState2 (ConwayEra c)
...
--   , State (EraRule "LEDGERS" era) ~ LedgerState2 (forall c. ConwayEra c)
--   ) =>
--   ApplyBlock era
--   where
