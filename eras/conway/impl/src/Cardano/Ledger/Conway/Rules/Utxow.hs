...
data ConwayUtxowPredFailure era
  ...
  | MissingRequiredDatums
      -- | Set of missing data hashes TODO: Make this NonEmpty #4066
      !(Set (DataHash (EraCrypto era)))
      -- | Set of received data hashes
      !(Set (DataHash (EraCrypto era)))
  | NotAllowedSupplementalDatums
      -- | Set of unallowed data hashes. TODO: Make this NonEmpty #4066
      !(Set (DataHash (EraCrypto era)))
  ...
  decCBOR = decode . Summands "ConwayUtxowPred" $ \case
    ...
    7 ->
      SumD ConflictingMetadataHash
        <! FromGroup (Proxy @(Mismatch 'RelEQ (AuxiliaryDataHash (EraCrypto era))))
    ...
    13 ->
      SumD PPViewHashesDontMatch
        <! FromGroup (Proxy @(Mismatch 'RelEQ (StrictMaybe (ScriptIntegrityHash (EraCrypto era)))))
