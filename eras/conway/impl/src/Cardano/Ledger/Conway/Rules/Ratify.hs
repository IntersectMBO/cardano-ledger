...
reorderActions :: SSeq.StrictSeq (GovActionState era) -> SSeq.StrictSeq (GovActionState era)
reorderActions = SSeq.fromList . sortOn (actionPriority . gasAction) . toList
...
ratifyTransition = do
  TRC
    ...
    judgmentContext
  -- FIXME: `reorderActions` is unnecessary after it is done the first
  -- time. The solution may not be trivial.
...
prevActionAsExpected = \case
  ParameterChange parent _ -> (parent ==) . (^. prevGovActionIdsL . pfPParamUpdateL)
