    ...
    where
      tenOnwards =
        encode @_ @(ConwayGovCertPredFailure era) . \case
          ...
          ConwayDRepIncorrectDeposit mm -> Sum ConwayDRepIncorrectDeposit 2 !> To mm
          ConwayCommitteeHasPreviouslyResigned coldCred -> Sum ConwayCommitteeHasPreviouslyResigned 3 !> To coldCred
          ConwayDRepIncorrectRefund mm -> Sum ConwayDRepIncorrectRefund 4 !> To mm
          ConwayCommitteeIsUnknown coldCred -> Sum ConwayCommitteeIsUnknown 5 !> To coldCred
      beforeTen =
        encode @_ @(ConwayGovCertPredFailure era) . \case
          ConwayDRepAlreadyRegistered cred -> Sum ConwayDRepAlreadyRegistered 0 !> To cred
          ConwayDRepNotRegistered cred -> Sum ConwayDRepNotRegistered 1 !> To cred
          ConwayDRepIncorrectDeposit mm@(Mismatch _ _) ->
            Sum ConwayDRepIncorrectDeposit 2
              !> E (\(Mismatch {..}) -> encCBOR mismatchSupplied <> encCBOR mismatchExpected) mm
          ConwayCommitteeHasPreviouslyResigned coldCred -> Sum ConwayCommitteeHasPreviouslyResigned 3 !> To coldCred
          ConwayDRepIncorrectRefund mm@(Mismatch _ _) ->
            Sum ConwayDRepIncorrectRefund 4
              !> E (\(Mismatch {..}) -> encCBOR mismatchSupplied <> encCBOR mismatchExpected) mm
          ConwayCommitteeIsUnknown coldCred -> Sum ConwayCommitteeIsUnknown 5 !> To coldCred
...
instance
  ...
  decCBOR = ifDecoderVersionAtLeast (natVersion @10) tenOnwards beforeTen
    where
      tenOnwards = decode . Summands "ConwayGovCertPredFailure" $ \case
        0 -> SumD ConwayDRepAlreadyRegistered <! From
        1 -> SumD ConwayDRepNotRegistered <! From
        2 -> SumD ConwayDRepIncorrectDeposit <! From
        3 -> SumD ConwayCommitteeHasPreviouslyResigned <! From
        4 -> SumD ConwayDRepIncorrectRefund <! From
        5 -> SumD ConwayCommitteeIsUnknown <! From
        n -> Invalid n
      beforeTen = decode . Summands "ConwayGovCertPredFailure" $ \case
        0 -> SumD ConwayDRepAlreadyRegistered <! From
        1 -> SumD ConwayDRepNotRegistered <! From
        2 ->
          SumD ConwayDRepIncorrectDeposit
            <! D
              ( do
        ...
        3 -> SumD ConwayCommitteeHasPreviouslyResigned <! From
        4 ->
          SumD ConwayDRepIncorrectRefund
            <! D
              ( do
                  mismatchSupplied <- decCBOR
                  mismatchExpected <- decCBOR
