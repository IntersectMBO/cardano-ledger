...
instance
  ...
  encCBOR =
    encode . \case
      ...
      FeeTooSmallUTxO mm -> Sum FeeTooSmallUTxO 5 !> ToGroup (swapMismatch mm)
      ...
      ExUnitsTooBigUTxO mm -> Sum ExUnitsTooBigUTxO 14 !> ToGroup (swapMismatch mm)
      CollateralContainsNonADA a -> Sum CollateralContainsNonADA 15 !> To a
      WrongNetworkInTxBody mm -> Sum WrongNetworkInTxBody 16 !> ToGroup (swapMismatch mm)
      OutsideForecast a -> Sum OutsideForecast 17 !> To a
      TooManyCollateralInputs mm -> Sum TooManyCollateralInputs 18 !> ToGroup (swapMismatch mm)
...
instance
  ...
  decCBOR = decode . Summands "ConwayUtxoPredFailure" $ \case
    ...
    3 -> SumD MaxTxSizeUTxO <! FromGroup (Proxy @(Mismatch 'RelLTEQ Integer))
    4 -> SumD InputSetEmptyUTxO
    5 -> SumD FeeTooSmallUTxO <! (swapMismatch <$> FromGroup (Proxy @(Mismatch 'RelGTEQ Coin)))
    6 -> SumD ValueNotConservedUTxO <! FromGroup (Proxy @(Mismatch 'RelEQ (Value era)))
    ...
    14 -> SumD ExUnitsTooBigUTxO <! (swapMismatch <$> FromGroup (Proxy @(Mismatch 'RelLTEQ ExUnits)))
    15 -> SumD CollateralContainsNonADA <! From
    16 -> SumD WrongNetworkInTxBody <! (swapMismatch <$> FromGroup (Proxy @(Mismatch 'RelEQ Network)))
    17 -> SumD OutsideForecast <! From
    18 ->
      SumD TooManyCollateralInputs <! (swapMismatch <$> FromGroup (Proxy @(Mismatch 'RelLTEQ Natural)))
