...
instance
  ...
  decCBOR = decode . Summands "ConwayUtxoPredFailure" $ \case
    ...
    5 -> SumD FeeTooSmallUTxO <! (swapMismatch <$> FromGroup)
    ...
    14 -> SumD ExUnitsTooBigUTxO <! (swapMismatch <$> FromGroup)
    15 -> SumD CollateralContainsNonADA <! From
    16 -> SumD WrongNetworkInTxBody <! (swapMismatch <$> FromGroup)
    17 -> SumD OutsideForecast <! From
    18 ->
      SumD TooManyCollateralInputs <! (swapMismatch <$> FromGroup)
