...
epochTransition = do
  ...
  let
    ...
    --
    -- QUESTION: SHould we check here that the @`PrevGovActionIds`@ in
    -- the new enact state match up with the resultant roots of the
    -- @`Proposals`@. How do we handle the case when they dont? For now,
    -- we leave this to the constrained property-tests.
