  ...
  encCBOR cas@(ConwayAccountState _ _ _ _) =
    let ConwayAccountState {..} = cas
     in encodeListLen 4
          <> encCBOR casBalance
          <> encCBOR casDeposit
          <> encCBOR casStakePoolDelegation
          <> encCBOR casDRepDelegation
...
instance ToJSON (ConwayAccountState era) where
  toJSON = object . toConwayAccountStatePairs
  toEncoding = Aeson.pairs . mconcat . toConwayAccountStatePairs

toConwayAccountStatePairs :: Aeson.KeyValue e a => ConwayAccountState era -> [a]
toConwayAccountStatePairs cas@(ConwayAccountState _ _ _ _) =
  let ConwayAccountState {..} = cas
...
instance Typeable era => DecShareCBOR (ConwayAccounts era) where
  ...
  decSharePlusCBOR =
    StateT
      ( \(a, b, c) ->
          decodeRecordNamed "ConwayAccounts" (const 2) $ do
...
class EraAccounts era => ConwayEraAccounts era where
  ...
  -- TODO: rename dRepDelegateeAccountStateL
  --           or accountStateDRepDelegateeL
...
lookupDRepDelegatee :: ConwayEraAccounts era => Credential 'Staking -> Accounts era -> Maybe DRep
lookupDRepDelegatee cred accounts = do
