...
instance HuddleGroup "operational_cert" AlonzoEra where
  huddleGroup = operationalCertGroup @AlonzoEra
...
instance HuddleGroup "protocol_version" AlonzoEra where
  huddleGroup p = "protocol_version" =:~ grp [a (huddleRule @"major_protocol_version" p), a VUInt]

instance HuddleRule "major_protocol_version" AlonzoEra where
  huddleRule _ = "major_protocol_version" =:= (0 :: Integer) ... (7 :: Integer)
  ...
  huddleRule p =
    "protocol_param_update"
      =:= mp
        ...
        , opt (idx 12 ==> huddleRule @"unit_interval" p) //- "d. decentralization constant"
