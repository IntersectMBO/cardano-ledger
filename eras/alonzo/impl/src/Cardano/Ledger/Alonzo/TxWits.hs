...
pattern Redeemers rs <-
  ...
  where
    Redeemers rs' = mkMemoized (eraProtVerLow @era) $ RedeemersRaw rs'
...
pattern TxDats m <- (getMemoRawType -> TxDatsRaw m)
  where
    TxDats m = mkMemoized (eraProtVerLow @era) (TxDatsRaw m)
...
pattern AlonzoTxWits {txwitsVKey, txwitsBoot, txscripts, txdats, txrdmrs} <-
  ...
  where
    AlonzoTxWits witsVKey' witsBoot' witsScript' witsDat' witsRdmr' =
      mkMemoized (eraProtVerLow @era) $ AlonzoTxWitsRaw witsVKey' witsBoot' witsScript' witsDat' witsRdmr'
...
addrAlonzoTxWitsL =
  lensMemoRawType (eraProtVerLow @era) atwrAddrTxWits $
...
bootAddrAlonzoTxWitsL =
  lensMemoRawType (eraProtVerLow @era) atwrBootAddrTxWits $
...
scriptAlonzoTxWitsL =
  lensMemoRawType (eraProtVerLow @era) atwrScriptTxWits $
...
datsAlonzoTxWitsL =
  lensMemoRawType (eraProtVerLow @era) atwrDatsTxWits $
...
rdmrsAlonzoTxWitsL =
  lensMemoRawType (eraProtVerLow @era) atwrRdmrsTxWits $
