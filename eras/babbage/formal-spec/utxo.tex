...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-witG]
    {
      ...
      \var{inputHashes} \subseteq_{\{h \mid (\wcard, \wcard, h)\in\fun{txouts}~{tx} \cup \hldiff{\var{utxo}~(\fun{refInputs}~{tx})}\}} \dom (\fun{txdats}~{txw})  \\~\\
      \\~\\
      ...
      \hldiff{\cup \bigcup_{(\_, \_, d, s) \in \txouts{tx}} \{s, d\} \cup \fun{scripts}~(\fun{auxiliaryData}~tx),} \\
