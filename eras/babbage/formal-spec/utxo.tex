...
\begin{figure*}[htb]
  ...
  %
  \begin{align*}
    & \fun{isTwoPhaseScriptAddress} : \Tx \to \UTxO \to \Addr \to \Bool \\
    & \fun{isTwoPhaseScriptAddress}~tx~utxo~a = \\
    &\quad\begin{cases}
        \True  & a \in \AddrScr \land \fun{validatorHash}~a \mapsto s \in \fun{txscripts}~tx~utxo \land s \in \ScriptPhTwo \\
        \False & \text{otherwise}
      \end{cases}
    \nextdef
...
\begin{figure*}
  \begin{align*}
    ...
    & \fun{getDatum}~{tx}~{utxo}~{sp} =
      ...
      \end{cases}
    \nextdef
    & \fun{txscripts} : \Tx \to \UTxO \to \powerset{\Script} \\
    & \fun{txscripts}~tx~utxo = \fun{txwitscripts}~tx \cup \{ \fun{hash}~s \mapsto s \mid (\_, \_, \_, s) \in \var{utxo}~(\fun{spendInputs}~tx \cup \fun{refInputs}~tx)\}
    \nextdef
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-witG]
    {
      ...
      \forall \var{s} \in \range (\fun{txscripts}~{txw}~utxo) \cap \ScriptPhOne,
      \fun{validateScript}~\var{s}~\var{tx}\\~\\
      \{ h \mid (\_, h) \in \fun{scriptsNeeded}~\var{utxo}~txb\} = \dom (\fun{txscripts}~{txw}~utxo) \\~\\
      ...
      \dom (\fun{txrdmrs}~tx) = \left\{ \fun{rdptr}~txb~sp \,\middle|
        {
          \begin{array}{l}
            ...
            \fun{txscripts}~{txw}~utxo~h \in \ScriptPhTwo
          \end{array}
        } \right\}
      \\~\\
      ...
      \\~\\
      \fun{languages}~tx~utxo \subseteq \dom(\fun{costmdls}~tx) \\
