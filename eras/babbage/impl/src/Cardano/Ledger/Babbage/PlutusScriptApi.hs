...
module Cardano.Ledger.Babbage.PlutusScriptApi
  ( -- Figure 8
    getData,
    evalScripts,
    -- Figure 12
...
import qualified Cardano.Ledger.Babbage.TxBody as Babbage (TxBody (..), TxOut (..), vldt')
...
import Cardano.Ledger.Babbage.TxWitness (TxWitness (txwitsVKey'), txscripts', unTxDats)
...
collectTwoPhaseScriptInputs ::
  forall era tx.
  ( Era era,
    Core.Script era ~ BabbageScript.Script era,
  ...
  tx ->
  UTxO era ->
  Either [CollectError (Crypto era)] [(BabbageScript.Script era, [Data era], ExUnits, CostModel)]
collectTwoPhaseScriptInputs ei sysS pp tx utxo =
  let scriptsUsed = Map.elems . txscripts' $ getField @"wits" tx
      usedLanguages = [lang | (BabbageScript.PlutusScript lang _) <- scriptsUsed]
  ...
  where
    ...
    knownToNotBe1Phase (_, sh) =
      case sh `Map.lookup` txscripts' (getField @"wits" tx) of
        Just (BabbageScript.PlutusScript _ _) -> True
        Just (BabbageScript.TimelockScript _) -> False
        Nothing -> True
    redeemer (sp, _) =
    ...
    apply costs (sp, d, eu) script@(BabbageScript.PlutusScript lang _) =
...
language :: BabbageScript.Script era -> Maybe Language
language (BabbageScript.PlutusScript lang _) = Just lang
language (BabbageScript.TimelockScript _) = Nothing
...
evalScripts ::
  forall era tx.
  ( Era era,
    Babbage.TxBody era ~ Core.TxBody era,
    Show (BabbageScript.Script era),
    HasField "body" tx (Core.TxBody era),
    HasField "wits" tx (TxWitness era)
  ) =>
  tx ->
  [(BabbageScript.Script era, [Data era], ExUnits, CostModel)] ->
  ScriptResult
evalScripts _tx [] = Passes
evalScripts tx ((BabbageScript.TimelockScript timelock, _, _, _) : rest) =
  lift (evalTimelock vhks (Babbage.vldt' (getField @"body" tx)) timelock)
...
evalScripts tx ((BabbageScript.PlutusScript lang pscript, ds, units, cost) : rest) =
  let beginMsg =
        intercalate
          ","
