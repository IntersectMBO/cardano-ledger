...
module Cardano.Ledger.Babbage.Tools
...
import Cardano.Ledger.Babbage.TxWitness (RdmrPtr (..), unRedeemers, unTxDats)
...
evaluateTransactionExecutionUnits pp tx utxo ei sysS costModels = do
  ...
  where
    ...
    ptrToPlutusScript = Map.fromList $ do
      ...
      pointer <- case rdptr @(BabbageEra c) txb sp of
        SNothing -> []
