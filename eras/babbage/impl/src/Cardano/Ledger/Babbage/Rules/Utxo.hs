...
import Cardano.Ledger.Babbage.Data (dataHashSize)
import Cardano.Ledger.Babbage.Rules.Utxos (UTXOS, UtxosPredicateFailure)
import Cardano.Ledger.Babbage.Scripts (ExUnits (..), Prices, pointWiseExUnits)
...
import Cardano.Ledger.Babbage.TxWitness (TxWitness (txrdmrs'), nullRedeemers)
...
import qualified Cardano.Ledger.Mary.Value as Babbage (Value)
...
feesOK ::
  ...
  ( Era era,
    ...
    Core.TxOut era ~ Babbage.TxOut era, -- balance requires this,
...
utxoTransition ::
  ...
  ( Era era,
    ...
    Core.Value era ~ Babbage.Value (Crypto era),
...
instance
  ...
  type PredicateFailure (BabbageUTXO era) = UtxoPredicateFailure era
...
instance
  ...
  Embed (UTXOS era) (BabbageUTXO era)
  where
