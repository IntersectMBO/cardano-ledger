...
import Cardano.Ledger.BaseTypes (Inject, StrictMaybe (..), TxIx (..), inject, natVersion)
...
spec = describe "Plutus V1 with references" $ do
  let inBabbage = eraProtVerHigh @era == natVersion @8
