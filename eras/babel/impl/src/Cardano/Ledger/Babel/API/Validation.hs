...
-- API.
module Cardano.Ledger.Babel.API.Validation (
  ApplyBlock (..),
  applyBlock,
  applyTick,
  TickTransitionError (..),
...
import Cardano.Ledger.Babel.Rules.Bbody (BabelBbodyState (BbodyState))
...
instance
  ( Crypto c
  , DSignable c (Hash c EraIndependentTxBody)
  , STS (EraRule "TICK" (BabelEra c))
  , STS (EraRule "BBODY" (BabelEra c))
  ) =>
