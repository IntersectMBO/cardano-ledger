...
data DijkstraLedgerPredFailure era
  ...
  | DijkstraIncompleteWithdrawals Withdrawals
  deriving (Generic)
  ...
  encCBOR =
    encode . \case
      ...
      DijkstraTreasuryValueMismatch mm ->
        Sum (DijkstraTreasuryValueMismatch @era . unswapMismatch) 5 !> To (swapMismatch mm)
...
instance
  ...
  decCBOR = decode . Summands "DijkstraLedgerPredFailure" $ \case
    ...
    5 -> SumD DijkstraTreasuryValueMismatch <! mapCoder unswapMismatch From
...
instance
  ...
  initialRules = []
  transitionRules = [Conway.ledgerTransition @DijkstraLEDGER]
