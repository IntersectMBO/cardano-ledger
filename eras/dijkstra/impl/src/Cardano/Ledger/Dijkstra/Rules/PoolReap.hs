...
-- The `POOLREAP` rule for the Dijkstra era, mirrored from the Shelley era's `POOLREAP` so
-- that the reference counts in `psVRFKeyHashes` are maintained at the epoch boundary as
-- well. The `POOLREAP` rule type itself is declared in "Cardano.Ledger.Dijkstra.Era".
...
poolReapTransition = do
  ...
  let
    ps0 = cs0 ^. certPStateL
    -- VRF key hashes that lose their active reference at this boundary because the
    -- pool re-registered with a different VRF: the reference moves to the newly
    -- adopted parameters, so the superseded hash is decremented. It may stay in the
    -- map with a reduced count when other pools still hold it.
...
-- | Local copy of Shelley's @potEqualsObligation@, which lives in the
-- @Cardano.Ledger.Shelley.LedgerState.Types@ module that is not exposed by the
-- @cardano-ledger-shelley@ package and therefore cannot be imported here.
potEqualsObligation ::
