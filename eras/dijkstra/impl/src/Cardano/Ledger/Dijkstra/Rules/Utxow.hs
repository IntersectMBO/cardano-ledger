...
deriving instance
  ...
  Ord (DijkstraUtxowPredFailure era)
...
dijkstraUtxowTransition = do
  TRC (UtxoEnv slot pp _pState originalCertState originalUtxo, u, stAnnTx) <- judgmentContext
  ...
  trans @(EraRule "UTXO" era) $
    TRC (UtxoEnv slot pp _pState originalCertState originalUtxo, u, stAnnTx)
