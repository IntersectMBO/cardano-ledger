...
dijkstraUtxoTransition = do
  ...
  -- It is safe to skip the value conservation check here, because we know it already passed
  -- with the flag set to Phase2Valid: a node only flips the flag to False and re-validates
  -- a transaction that is phase-1 valid and whose scripts failed.
