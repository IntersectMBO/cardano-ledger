...
data DijkstraTxBodyRaw l era where
  DijkstraTxBodyRaw ::
    ...
    , dtbrAccountBalanceIntervals :: !(StrictMaybe (AccountBalanceIntervals era))
    } ->
  ...
  DijkstraSubTxBodyRaw ::
    ...
    , dstbrAccountBalanceIntervals :: !(StrictMaybe (AccountBalanceIntervals era))
    } ->
...
encodeTxBodyRaw DijkstraTxBodyRaw {..} =
  let ValidityInterval bot top = dtbrVldt
   in Keyed
        ...
        !> encodeKeyedStrictMaybe 26 dtbrAccountBalanceIntervals
encodeTxBodyRaw DijkstraSubTxBodyRaw {..} =
  let ValidityInterval bot top = dstbrVldt
   in Keyed
        ...
        !> encodeKeyedStrictMaybe 26 dstbrAccountBalanceIntervals
...
pattern DijkstraTxBody ::
  ...
  DirectDeposits ->
  StrictMaybe (AccountBalanceIntervals DijkstraEra) ->
...
accountBalanceIntervalsDijkstraTxBodyRawL ::
  Lens' (DijkstraTxBodyRaw l era) (StrictMaybe (AccountBalanceIntervals era))
accountBalanceIntervalsDijkstraTxBodyRawL =
  lens
    ( \case
...
class ConwayEraTxBody era => DijkstraEraTxBody era where
  ...
  accountBalanceIntervalsTxBodyL :: Lens' (TxBody l era) (StrictMaybe (AccountBalanceIntervals era))
