...
goldenBlockTransaction = do
  it "Deserialize a block body with a valid transaction" $
    expectDecoderResultOn @(BlockBody era)
      version
      (blockBodyEnc True)
      (blockBodyWithFlag (IsValid True))
      id
  it "Deserialize a block body with an invalid transaction" $
    expectDecoderResultOn @(BlockBody era)
      version
      (blockBodyEnc False)
      (blockBodyWithFlag (IsValid False))
      id
  it "Fail to deserialize a block transaction with `isValid` before auxiliary data" $
  ...
  where
    version = eraProtVerLow @era
    blockBodyWithFlag isValid =
      mkBasicBlockBody @era
        & txSeqBlockBodyL
          .~ SSeq.singleton
            (mkBasicTx @era @TopTx (mkBasicTxBody @era @TopTx) & isValidTxL .~ isValid)
    ...
    -- The `isValid` flag comes after the auxiliary data in a block transaction
    txInBlockEnc isValid =
      mconcat
        [ E $ TkListLen 4
        , txBodyEnc
    ...
    -- The mempool position of the `isValid` flag (before the auxiliary data) is
    -- not allowed in a block transaction
    blockBodyLegacyFlagEnc =
      mconcat
        [ E $ TkListLen 3
        , E $ TkListLen 1
