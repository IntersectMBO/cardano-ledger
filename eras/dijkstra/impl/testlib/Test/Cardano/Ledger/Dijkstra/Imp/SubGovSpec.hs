...
spec = describe "SUBGOV" $ do
  ...
  describe "ProposalCantFollow" $ do
    it "a hardfork that cannot follow the current protocol version" $ do
      ...
      submitFailingSubTx
        ...
        [ subGovFailure . ProposalCantFollow SNothing $
    ...
    it "a hardfork that cannot follow its parent" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [ subGovFailure . ProposalCantFollow parentPurposeId $
  ...
  describe "MalformedProposal" $ do
    it "an empty parameter update" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ MalformedProposal govAction]
  ...
  describe "ProposalReturnAccountDoesNotExist" $
    it "an unregistered return account" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ ProposalReturnAccountDoesNotExist account]
  ...
  describe "TreasuryWithdrawalReturnAccountsDoNotExist" $
    it "an unregistered withdrawal account" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ TreasuryWithdrawalReturnAccountsDoNotExist [account]]
  ...
  describe "ProposalDepositIncorrect" $
    it "a deposit below the protocol parameter" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [ subGovFailure . ProposalDepositIncorrect $
  ...
  describe "ProposalProcedureNetworkIdMismatch" $
    it "a return account on the wrong network" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ ProposalProcedureNetworkIdMismatch onWrongNetwork Testnet]
  ...
  describe "InvalidGuardrailsScriptHash" $
    it "a policy that is not the constitution's guardrails script" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ InvalidGuardrailsScriptHash (SJust wrongScriptHash) guardrailsScriptHash]
  ...
  describe "ZeroTreasuryWithdrawals" $
    it "withdrawals that sum to zero" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ ZeroTreasuryWithdrawals govAction]
  ...
  describe "ConflictingCommitteeUpdate" $
    it "a member that is both added and removed" $ do
      ...
      let expiration = addEpochInterval currentEpochNo (EpochInterval 1)
      proposal <- mkProposal $ replaceCommitteeMember committeeCredential expiration
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure . ConflictingCommitteeUpdate $ NES.singleton committeeCredential]

  describe "ExpirationEpochTooSmall" $
    it "an expiration in the current epoch" $ do
      committeeCredential <- KeyHashObj <$> freshKeyHash
      currentEpochNo <- getsNES nesELL
      proposal <- mkProposal $ addCommitteeMember committeeCredential currentEpochNo
      submitFailingSubTx
        (proposeSubTx proposal)
        [ subGovFailure . ExpirationEpochTooSmall $
  ...
  describe "InvalidPrevGovActionId" $ do
    it "a parent of the wrong purpose" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ InvalidPrevGovActionId proposal]
    ...
    it "a parent index that does not exist" $ do
      ...
      govAction <-
        mkParameterChangeGovAction
          (SJust govActionId {gaidGovActionIx = GovActionIx 999})
          minCommitteeSizeUpdate
      proposal <- mkProposal govAction
      submitFailingSubTx
        (proposeSubTx proposal)
        [subGovFailure $ InvalidPrevGovActionId proposal]
  ...
  describe "UnelectedCommitteeVoters" $ do
    it "a member that is proposed but not enacted" $ do
      ...
      submitFailingSubTx
        ...
        [subGovFailure $ UnelectedCommitteeVoters [hotCredential]]
    ...
    it "an unknown hot credential, alongside VotersDoNotExist" $ do
      ...
      submitFailingSubTx
        (voteSubTx VoteYes (CommitteeVoter hotCredential) govActionId)
        [ subGovFailure $ UnelectedCommitteeVoters [hotCredential]
        , subGovFailure $ VotersDoNotExist [CommitteeVoter hotCredential]
        ]
  ...
  describe "VotersDoNotExist" $
    it "every unknown voter is reported" $ do
      ...
      submitFailingSubTx
        subTx
        [subGovFailure $ VotersDoNotExist [DRepVoter drepCredential, StakePoolVoter poolId]]
  ...
  describe "GovActionsDoNotExist" $
    it "a vote on an unknown gov action index" $ do
      ...
      submitFailingSubTx
        ...
        [subGovFailure $ GovActionsDoNotExist [unknownGovActionId]]
  ...
  describe "VotingOnExpiredGovAction" $
    it "a vote on an action past its lifetime" $ do
      ...
      submitFailingSubTx
        ...
        [subGovFailure $ VotingOnExpiredGovAction [(voter, govActionId)]]
  ...
  describe "DisallowedVoters" $ do
    it "a committee vote on NoConfidence" $ do
      ...
      submitFailingSubTx
        (voteSubTx VoteYes voter govActionId)
        [subGovFailure $ DisallowedVoters [(voter, govActionId)]]
  ...
  describe "Accepted at the boundary" $ do
    it "a proposal carrying the constitution's guardrails script" $ do
      useNativeGuardrailsScript
      govAction <- mkParameterChangeGovAction SNothing minCommitteeSizeUpdate
      proposal <- mkProposal govAction
    ...
    it "an expiration epoch one after the current one" $ do
      ...
      currentEpochNo <- getsNES nesELL
      let expiration = addEpochInterval currentEpochNo (EpochInterval 1)
      proposal <- mkProposal $ addCommitteeMember committeeCredential expiration
      submitTx_ . mkTopTxWithSubTxs . pure $ proposeSubTx proposal
  ...
  describe "Composite tests" $ do
    it "failures of several proposals, in the order of the body" $ do
      ...
      submitFailingSubTx
        subTx
        [ subGovFailure $ ProposalReturnAccountDoesNotExist firstAccount
        , subGovFailure $ ProposalReturnAccountDoesNotExist secondAccount
        ]

    it "two failures of one committee update, in the order the rule checks them" $ do
      committeeCredential <- KeyHashObj <$> freshKeyHash
      currentEpochNo <- getsNES nesELL
      proposal <- mkProposal $ replaceCommitteeMember committeeCredential currentEpochNo
      submitFailingSubTx
        (proposeSubTx proposal)
        [ subGovFailure . ConflictingCommitteeUpdate $ NES.singleton committeeCredential
        , subGovFailure . ExpirationEpochTooSmall $
    ...
    it "seven failures of one proposal, in the order the rule checks them" $ do
      ...
      submitFailingSubTx
        (proposeSubTx proposal)
        [ subGovFailure $ ProposalReturnAccountDoesNotExist onWrongNetwork
        , subGovFailure $ TreasuryWithdrawalReturnAccountsDoNotExist [onWrongNetwork]
        , subGovFailure . ProposalDepositIncorrect $
            Mismatch {mismatchSupplied = suppliedDeposit, mismatchExpected = expectedDeposit}
        , subGovFailure $ ProposalProcedureNetworkIdMismatch onWrongNetwork Testnet
        , subGovFailure $
  ...
  describe "Sub-transaction semantics" $ do
    ...
    it "a sub-transaction cannot vote on a proposal made by a later sibling" $ do
      ...
      submitFailingTx
        ...
        [subGovFailure $ GovActionsDoNotExist [govActionId]]
    ...
    it "a sub-transaction proposal is the parent of a later sibling's proposal" $ do
      useNativeGuardrailsScript
      parentAction <- mkParameterChangeGovAction SNothing minCommitteeSizeUpdate
      parentProposal <- mkProposal parentAction
      (parentSubTx, parentGovActionId) <- proposeSubTxWithStableId parentProposal
      childAction <- mkParameterChangeGovAction (SJust parentGovActionId) minCommitteeSizeUpdate
...
subGovFailure ::
  DijkstraEraImp era => DijkstraGovPredFailure era -> PredicateFailure (EraRule "LEDGER" era)
subGovFailure = injectFailure . DijkstraSubGovPredFailure

addCommitteeMember :: Credential ColdCommitteeRole -> EpochNo -> GovAction era
addCommitteeMember committeeCredential expiration =
  UpdateCommittee SNothing mempty (Map.singleton committeeCredential expiration) (0 %! 1)

replaceCommitteeMember :: Credential ColdCommitteeRole -> EpochNo -> GovAction era
replaceCommitteeMember committeeCredential expiration =
  UpdateCommittee
    SNothing
    (Set.singleton committeeCredential)
    (Map.singleton committeeCredential expiration)
    (1 %! 1)

minCommitteeSizeUpdate :: ConwayEraPParams era => PParamsUpdate era
minCommitteeSizeUpdate = emptyPParamsUpdate & ppuCommitteeMinSizeL .~ SJust 1
