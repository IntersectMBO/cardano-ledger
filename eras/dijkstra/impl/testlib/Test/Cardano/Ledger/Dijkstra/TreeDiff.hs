...
instance ToExpr LeiosCert where
  toExpr LeiosCert {leiosCertSigners, leiosCertSignature} =
    Rec "LeiosCert" $
      OMap.fromList
        [ ("leiosCertSigners", toExpr . HexBytes $ serialize' (natVersion @12) leiosCertSigners)
        , ("leiosCertSignature", toExpr . HexBytes $ serialize' (natVersion @12) leiosCertSignature)
        ]
