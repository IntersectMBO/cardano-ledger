...
module Cardano.Ledger.Dijkstra.Rules.Utxo (
  DijkstraUTXO,
  allegraToDijkstraUtxoPredFailure,
  babbageToDijkstraUtxoPredFailure,
  alonzoToDijkstraUtxoPredFailure,
...
import Cardano.Ledger.Dijkstra.Era (DijkstraEra, DijkstraUTXO, DijkstraUTXOS)
...
instance InjectRuleFailure "UTXO" BabbageUtxoPredFailure DijkstraEra where
  injectFailure = babbageToDijkstraUtxoPredFailure

instance InjectRuleFailure "UTXO" AlonzoUtxoPredFailure DijkstraEra where
  injectFailure = alonzoToDijkstraUtxoPredFailure
...
instance InjectRuleFailure "UTXO" Allegra.AllegraUtxoPredFailure DijkstraEra where
  injectFailure = allegraToDijkstraUtxoPredFailure

instance InjectRuleFailure "UTXO" DijkstraUtxosPredFailure DijkstraEra where
...
deriving via
  InspectHeapNamed "DijkstraUtxoPred" (DijkstraUtxoPredFailure era)
  instance
...
instance
  ...
  type Signal (DijkstraUTXO era) = AlonzoTx era
...
instance
  ...
  , Event (EraRule "UTXOS" era) ~ Event (DijkstraUTXOS era)
  ) =>
  Embed (DijkstraUTXOS era) (DijkstraUTXO era)
  where
...
babbageToDijkstraUtxoPredFailure = \case
  Babbage.AlonzoInBabbageUtxoPredFailure a -> alonzoToDijkstraUtxoPredFailure a
...
alonzoToDijkstraUtxoPredFailure ::
  ...
  DijkstraUtxoPredFailure era
alonzoToDijkstraUtxoPredFailure = \case
...
allegraToDijkstraUtxoPredFailure ::
  ...
  DijkstraUtxoPredFailure era
allegraToDijkstraUtxoPredFailure = \case
