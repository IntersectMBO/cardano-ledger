...
import Cardano.Ledger.Dijkstra.Era (DijkstraENACT, DijkstraRATIFY)
...
import Cardano.Ledger.Dijkstra.Rules.Enact (EnactSignal (..), EnactState (..))
...
instance
  ...
  , Embed (EraRule "ENACT" era) (DijkstraRATIFY era)
  ...
  where
  type Environment (DijkstraRATIFY era) = RatifyEnv era
  type PredicateFailure (DijkstraRATIFY era) = Void
  type Signal (DijkstraRATIFY era) = RatifySignal era
  type State (DijkstraRATIFY era) = RatifyState era
  type BaseM (DijkstraRATIFY era) = ShelleyBase
...
spoAccepted ::
  DijkstraEraPParams era => RatifyEnv era -> RatifyState era -> GovActionState era -> Bool
...
ratifyTransition ::
  ...
  TransitionRule (DijkstraRATIFY era)
ratifyTransition = do
  TRC
          ...
          let
            st' =
              st
                ...
                & rsEnactedL %~ (Seq.:|> gas)
          trans @(DijkstraRATIFY era) $ TRC (env, st', RatifySignal sigs)
        else do
          -- This action hasn't been ratified yet. Process the remaining actions.
          st' <- trans @(DijkstraRATIFY era) $ TRC (env, st, RatifySignal sigs)
...
instance EraGov era => Embed (DijkstraENACT era) (DijkstraRATIFY era) where
  wrapFailed = absurd
  wrapEvent = absurd
