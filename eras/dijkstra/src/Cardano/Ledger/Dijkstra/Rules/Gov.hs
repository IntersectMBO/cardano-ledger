...
import Cardano.Ledger.Dijkstra.Core (ppGovActionDepositL, ppGovActionLifetimeL)
...
import Cardano.Ledger.Dijkstra.Governance.Proposals (mapProposals)
...
type instance EraRuleEvent "GOV" DijkstraEra = DijkstraGovEvent DijkstraEra
...
instance EraPParams era => NFData (DijkstraGovEvent era)
...
instance
  ...
  type Event (DijkstraGOV era) = DijkstraGovEvent era
...
checkVotesAreNotForExpiredActions ::
  ...
  Test (DijkstraGovPredFailure era)
...
actionWellFormed ::
  DijkstraEraPParams era => ProtVer -> GovAction era -> Test (DijkstraGovPredFailure era)
