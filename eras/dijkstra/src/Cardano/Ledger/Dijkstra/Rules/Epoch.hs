...
import Cardano.Ledger.Dijkstra.Era (DijkstraEPOCH, DijkstraEra, DijkstraHARDFORK, DijkstraRATIFY)
...
import Cardano.Ledger.Dijkstra.Governance.Procedures (Committee (..))
...
type instance EraRuleEvent "EPOCH" DijkstraEra = DijkstraEpochEvent DijkstraEra
...
instance
  ...
  NFData (DijkstraEpochEvent era)

instance
  ( EraTxOut era
  , RunDijkstraRatify era
  , DijkstraEraCertState era
  , DijkstraEraGov era
  , EraStake era
  , EraCertState era
  , Embed (EraRule "SNAP" era) (DijkstraEPOCH era)
  , Environment (EraRule "SNAP" era) ~ SnapEnv era
  , State (EraRule "SNAP" era) ~ SnapShots
  , Signal (EraRule "SNAP" era) ~ ()
  , Embed (EraRule "POOLREAP" era) (DijkstraEPOCH era)
  ...
  , Embed (EraRule "RATIFY" era) (DijkstraEPOCH era)
  ...
  , Embed (EraRule "HARDFORK" era) (DijkstraEPOCH era)
  ...
  where
  type State (DijkstraEPOCH era) = EpochState era
  type Signal (DijkstraEPOCH era) = EpochNo
  type Environment (DijkstraEPOCH era) = ()
  type BaseM (DijkstraEPOCH era) = ShelleyBase

  -- EPOCH rule can never fail
  type PredicateFailure (DijkstraEPOCH era) = Void
  type Event (DijkstraEPOCH era) = DijkstraEpochEvent era
...
epochTransition ::
  ...
  TransitionRule (DijkstraEPOCH era)
epochTransition = do
  TRC
    ( ()
...
instance
  ...
  Embed (ShelleyPOOLREAP era) (DijkstraEPOCH era)
  where
  wrapFailed = \case {}
...
instance
  ...
  Embed (ShelleySNAP era) (DijkstraEPOCH era)
  where
  wrapFailed = \case {}
  wrapEvent = SnapEvent
...
instance
  ( EraGov era
  , PredicateFailure (DijkstraRATIFY era) ~ Void
  , STS (DijkstraRATIFY era)
  , BaseM (DijkstraRATIFY era) ~ ShelleyBase
  , Event (DijkstraRATIFY era) ~ Void
  ) =>
  Embed (DijkstraRATIFY era) (DijkstraEPOCH era)
  where
  wrapFailed = absurd
  wrapEvent = absurd
...
instance
  ( EraGov era
  , PredicateFailure (DijkstraHARDFORK era) ~ Void
  , STS (DijkstraHARDFORK era)
  , BaseM (DijkstraHARDFORK era) ~ ShelleyBase
  , Event (EraRule "HARDFORK" era) ~ DijkstraHardForkEvent era
  ) =>
  Embed (DijkstraHARDFORK era) (DijkstraEPOCH era)
  where
  wrapFailed = absurd
