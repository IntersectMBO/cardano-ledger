...
addFees newFees cs = cs {chainNes = nes}
  where
    nes =
      chainNes cs
        & nesEsL . esLStateL . lsUTxOStateL . utxosFeesL <>~ newFees
...
newStakeCred cred ptr cs = cs {chainNes = nes}
  where
    deposit = nes ^. nesEsL . curPParamsEpochStateL . ppKeyDepositL
    nes =
      chainNes cs
...
deregStakeCred ::
  (EraCertState era, ShelleyEraAccounts era) =>
  Credential 'Staking ->
  ChainState era ->
  ChainState era
  ...
  where
    ...
    (accounts', unclaimed) =
      case lookupAccountState poolAccountCred accounts of
        ...
        Just accountState ->
          let accountState' = accountState & balanceAccountStateL <>~ compactCoinOrError poolDeposit
