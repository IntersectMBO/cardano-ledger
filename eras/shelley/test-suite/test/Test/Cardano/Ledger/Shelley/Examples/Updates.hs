...
initStUpdates :: forall c hc. (Crypto c, HeaderCrypto hc) => ChainState (ShelleyEra c)
initStUpdates = initSt (Proxy @c) (Proxy @hc) initUTxO
...
collectVotes vote =
  ProposedPPUpdates . Map.fromList . (fmap (\n -> (hashKey $ coreNodeVK (Proxy @c) (Proxy @hc) n, vote)))

ppVotes1 :: forall c hc. (Crypto c, HeaderCrypto hc) => ProposedPPUpdates (ShelleyEra c)
...
txbodyEx1 :: forall c hc. (Crypto c, HeaderCrypto hc) => ShelleyTxBody (ShelleyEra c)
txbodyEx1 =
  ShelleyTxBody
      ...
      { addrWits =
          mkWitnessesVKey
            ...
            ( [asWitness Cast.alicePay]
                <> [ asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 0
                   , asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 3
                   , asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 4
                   ]
            )
      }
    SNothing

blockEx1 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => Block (BHeader c hc) (ShelleyEra c)
blockEx1 =
  mkBlockFakeVRF
    lastByronHeaderHash
...
expectedStEx1 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => ChainState (ShelleyEra c)
expectedStEx1 =
...
updates1 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => CHAINExample (BHeader c hc) (ShelleyEra c) hc
    ...
    mempty
      { addrWits =
          mkWitnessesVKey
            ...
            ( [asWitness Cast.alicePay]
                <> [ asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 1
                   , asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 5
                   ]
            )
      }
    SNothing
...
updates2 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => CHAINExample (BHeader c hc) (ShelleyEra c) hc
...
txEx3 :: forall c hc. (Crypto c, ExMock (EraCrypto (ShelleyEra c)) hc) => ShelleyTx (ShelleyEra c)
txEx3 =
  ShelleyTx
    (txbodyEx3 @c @hc)
    mempty
      { addrWits =
          mkWitnessesVKey
            ...
            [asWitness Cast.alicePay, asWitness . aikCold $ coreNodeIssuerKeys (Proxy @c) (Proxy @hc) 1]
      }
    SNothing
...
updates3 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => CHAINExample (BHeader c hc) (ShelleyEra c) hc
...
expectedStEx4 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => ChainState (ShelleyEra c)
expectedStEx4 =
...
updates4 :: forall c hc. (ExMock (EraCrypto (ShelleyEra c)) hc) => CHAINExample (BHeader c hc) (ShelleyEra c) hc
