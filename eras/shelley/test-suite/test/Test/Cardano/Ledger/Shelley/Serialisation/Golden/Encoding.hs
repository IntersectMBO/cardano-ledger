...
import Cardano.Ledger.Shelley.BlockBody (ShelleyBlockBody (..), shelleyBlockBodyHash)
  ...
  BHBody
    ...
    , bheaderL =
        mkCertifiedVRF
          ...
          (vrfSignKey testVRF)
    , bsize = 0
    , bhash = shelleyBlockBodyHash @era $ ShelleyBlockBody @era StrictSeq.empty
    , bheaderOCert =
        OCert
  ...
  testGroup
    ...
    , -- checkEncodingCBOR "block_header_body"
      let prevhash = BlockHash testHeaderHash
          ...
          size = 0
          blockNo = BlockNo 44
          bbhash = shelleyBlockBodyHash @C $ ShelleyBlockBody StrictSeq.empty
    ...
    , -- checkEncodingCBOR "empty_block"
      let sig :: (SignedKES (KES MockCrypto) (BHBody MockCrypto))
          ...
          txns = ShelleyBlockBody StrictSeq.Empty
       in checkEncodingCBORAnnotated
            shelleyProtVer
            "empty_block"
            (Block @C bh (BodyInline txns))
            ( T (TkListLen 4)
                <> S bh
