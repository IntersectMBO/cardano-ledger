...
shelleyProtocolVersionGroup ::
  forall name era. (KnownSymbol name, HuddleRule "major_protocol_version" era) => Proxy name -> Proxy era -> Named Group
shelleyProtocolVersionGroup pname p = pname =.~ grp [a $ huddleRule @"major_protocol_version" p, a VUInt]

headerRule :: forall name era. (KnownSymbol name, HuddleRule "header_body" era) => Proxy name -> Proxy era -> Rule
headerRule pname p =
  pname
...
proposedProtocolParameterUpdatesRule ::
  forall name era.
  (KnownSymbol name, HuddleRule "genesis_hash" era, HuddleRule "protocol_param_update" era) => Proxy name -> Proxy era -> Rule
proposedProtocolParameterUpdatesRule pname p =
  pname
    =.= mp [0 <+ asKey (huddleRule @"genesis_hash" p) ==> huddleRule @"protocol_param_update" p]

updateRule ::
  forall name era. (KnownSymbol name, HuddleRule "proposed_protocol_parameter_updates" era) => Proxy name -> Proxy era -> Rule
updateRule pname p =
  pname
    =.= arr [a $ huddleRule @"proposed_protocol_parameter_updates" p, a $ huddleRule @"epoch" p]

protocolParamUpdateRule ::
  forall name era. (KnownSymbol name, HuddleGroup "protocol_version" era) => Proxy name -> Proxy era -> Rule
...
vkeywitnessRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
vkeywitnessRule pname p =
  pname
    =.= arr [a $ huddleRule @"vkey" p, a $ huddleRule @"signature" p]

bootstrapWitnessRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
...
shelleyOperationalCertGroup :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Named Group
...
genesisHashRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
genesisHashRule pname p = pname =.= huddleRule @"hash28" p

scriptPubkeyGroup :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Named Group
scriptPubkeyGroup pname p = pname =.~ grp [0, a $ huddleRule @"addr_keyhash" p]

scriptAllGroup :: forall name era. (KnownSymbol name, HuddleRule "native_script" era) => Proxy name -> Proxy era -> Named Group
scriptAllGroup pname p = pname =.~ grp [1, a $ arr [0 <+ a (huddleRule @"native_script" p)]]

scriptAnyGroup :: forall name era. (KnownSymbol name, HuddleRule "native_script" era) => Proxy name -> Proxy era -> Named Group
scriptAnyGroup pname p = pname =.~ grp [2, a $ arr [0 <+ a (huddleRule @"native_script" p)]]

transactionIdRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
transactionIdRule pname p = pname =.= huddleRule @"hash32" p

transactionInputRule :: forall name era. (KnownSymbol name, HuddleRule "transaction_id" era) => Proxy name -> Proxy era -> Rule
...
transactionOutputRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
transactionOutputRule pname p =
  pname
    =.= arr [a $ huddleRule @"address" p, "amount" ==> huddleRule @"coin" p]

shelleyWithdrawalsRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
...
poolMetadataRule :: forall name era. (KnownSymbol name, HuddleRule "url" era) => Proxy name -> Proxy era -> Rule
poolMetadataRule pname p =
  pname
    =.= arr [a $ huddleRule @"url" p, a VBytes]

singleHostAddrGroup :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Named Group
...
singleHostNameGroup :: forall name era. (KnownSymbol name, HuddleRule "dns_name" era) => Proxy name -> Proxy era -> Named Group
...
multiHostNameGroup :: forall name era. (KnownSymbol name, HuddleRule "dns_name" era) => Proxy name -> Proxy era -> Named Group
...
poolRegistrationCertGroup ::
  forall name era. (KnownSymbol name, HuddleGroup "pool_params" era) => Proxy name -> Proxy era -> Named Group
poolRegistrationCertGroup pname p = pname =.~ grp [3, a $ huddleGroup @"pool_params" p]

poolRetirementCertGroup :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Named Group
poolRetirementCertGroup pname p =
  pname
    =.~ grp [4, a $ huddleRule @"pool_keyhash" p, a $ huddleRule @"epoch" p]

genesisDelegateHashRule :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Rule
...
deltaCoinRule :: forall name. KnownSymbol name => Proxy name -> Rule
deltaCoinRule pname =
  comment
...
moveInstantaneousRewardRule ::
  forall name era. (KnownSymbol name, HuddleRule "delta_coin" era) => Proxy name -> Proxy era -> Rule
...
moveInstantaneousRewardsCertGroup ::
  forall name era.
  (KnownSymbol name, HuddleRule "move_instantaneous_reward" era) =>
  Proxy name ->
  Proxy era ->
  Named Group
...
accountRegistrationCertGroup :: forall name era. (KnownSymbol name, Era era) => Proxy name -> Proxy era -> Named Group
...
untaggedSet :: (KnownSymbol name, IsType0 a) => Proxy name -> a -> GRuleCall
