...
import Cardano.Ledger.EpochBoundary (SnapShots (ssStakeMark), ssStakePoolDistr)
...
validatingTickTransitionFORECAST nes slot = do
  ...
  let pd' = ssStakePoolDistr $ ssStakeMark ss
...
bheadTransition = do
  ...
  let !_ = ssStakeMark . esSnapshots . nesEs $ nes'
      !_ = ssStakePoolDistr . ssStakeMark . esSnapshots . nesEs $ nes'
