...
transitionRulesUTXOW ::
  ...
  , Environment (EraRule "UTXO" era) ~ UtxoEnv era
  , State (EraRule "UTXO" era) ~ UTxOState era
  , Signal (EraRule "UTXO" era) ~ Tx FullTx era
...
instance
  ...
  type Signal (ShelleyUTXOW era) = Tx FullTx era
...
{-  ∀ s ∈ range(txscripts txw) ∩ Scriptnative), runNativeScript s tx   -}
validateFailedNativeScripts ::
  EraTx era => ScriptsProvided era -> Tx t era -> Test (ShelleyUtxowPredFailure era)
...
validateVerifiedWits :: EraTx era => Tx t era -> Test (ShelleyUtxowPredFailure era)
...
-- | check metadata hash
--   ((adh = ◇) ∧ (ad= ◇)) ∨ (adh = hashAD ad)
validateMetadata :: EraTx era => PParams era -> Tx t era -> Test (ShelleyUtxowPredFailure era)
