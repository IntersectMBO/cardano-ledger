...
data DState c = DState
  { -- | Unified Reward Maps. This contains the reward map (which is the source
    ...
    dsFGenDelegs :: !(Map (FutureGenDeleg c) (GenDelegPair c)),
    -- | Genesis key delegations
    dsGenDelegs :: !(GenDelegs c),
    -- | Instantaneous Rewards
    dsIrwd :: !(InstantaneousRewards c)
  }
  deriving (Show, Eq, Generic)
...
data PState c = PState
  { -- | The stake pool parameters.
    psPParams :: !(Map (KeyHash 'StakePool c) (PoolParams c)),
    ...
    psFPParams :: !(Map (KeyHash 'StakePool c) (PoolParams c)),
...
instance CC.Crypto c => FromSharedCBOR (PState c) where
  ...
  fromSharedPlusCBOR = decodeRecordNamedT "PState" (const 3) $ do
    psPParams <- fromSharedPlusLensCBOR (toMemptyLens _1 id)
    psFPParams <- fromSharedPlusLensCBOR (toMemptyLens _1 id)
    psRetiring <- fromSharedPlusLensCBOR (toMemptyLens _1 id)
    pure PState {psPParams, psFPParams, psRetiring}
