...
instance ShelleyEraScript ShelleyEra where
  mkRequireSignature kh =
    MultiSigConstr $ memoBytes (eraProtVerLow @ShelleyEra) (Sum RequireSignature' 0 !> To kh)
  getRequireSignature (MultiSigConstr (Memo (RequireSignature' kh) _)) = Just kh
  getRequireSignature _ = Nothing

  mkRequireAllOf ms =
    MultiSigConstr $ memoBytes (eraProtVerLow @ShelleyEra) (Sum RequireAllOf' 1 !> To ms)
  getRequireAllOf (MultiSigConstr (Memo (RequireAllOf' ms) _)) = Just ms
  getRequireAllOf _ = Nothing

  mkRequireAnyOf ms =
    MultiSigConstr $ memoBytes (eraProtVerLow @ShelleyEra) (Sum RequireAnyOf' 2 !> To ms)
  getRequireAnyOf (MultiSigConstr (Memo (RequireAnyOf' ms) _)) = Just ms
  getRequireAnyOf _ = Nothing

  mkRequireMOf n ms =
    MultiSigConstr $ memoBytes (eraProtVerLow @ShelleyEra) (Sum RequireMOf' 3 !> To n !> To ms)
