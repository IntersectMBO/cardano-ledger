...
instance Era era => DecShareCBOR (ShelleyCertState era) where
  type Share (ShelleyCertState era) = (Interns (Credential 'Staking), Interns (KeyHash 'StakePool))
  decSharePlusCBOR = decodeRecordNamedT "CertState" (const 3) $ do
    shelleyCertVState <- lift decNoShareCBOR -- TODO: add sharing of DRep credentials
    shelleyCertPState <- decSharePlusLensCBOR _2
    shelleyCertDState <- decSharePlusCBOR
    pure ShelleyCertState {shelleyCertPState, shelleyCertDState, shelleyCertVState}

instance Era era => Default (ShelleyCertState era) where
