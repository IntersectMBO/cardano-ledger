...
bodyShelleyTxL =
  lens (\(TxConstr (Memo tx _)) -> strBody tx) $
    \(TxConstr (Memo tx _)) txBody ->
      TxConstr $ memoBytes (eraProtVerLow @era) $ encodeShelleyTxRaw $ tx {strBody = txBody}
...
witsShelleyTxL =
  lens (\(TxConstr (Memo tx _)) -> strWits tx) $
    \(TxConstr (Memo tx _)) txWits ->
      TxConstr $ memoBytes (eraProtVerLow @era) $ encodeShelleyTxRaw $ tx {strWits = txWits}
...
mkShelleyTx = TxConstr . memoBytes (eraProtVerLow @era) . encodeShelleyTxRaw
