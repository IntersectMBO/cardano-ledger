...
aggregatedRewards ::
  ...
  pp ->
  Bool
aggregatedRewards pp = pvMajor (getField @"sppProtocolVersion" pp) > 2
...
allowMIRTransfer ::
  ...
  pp ->
  Bool
allowMIRTransfer pp = pvMajor (getField @"sppProtocolVersion" pp) > 4
...
validatePoolRewardAccountNetID ::
  ...
  pp ->
  Bool
validatePoolRewardAccountNetID pp = pvMajor (getField @"sppProtocolVersion" pp) > 4
...
allowScriptStakeCredsToEarnRewards ::
  ...
  pp ->
  Bool
allowScriptStakeCredsToEarnRewards pp = pvMajor (getField @"sppProtocolVersion" pp) > 4
...
translateTimeForPlutusScripts ::
  ...
  pp ->
  Bool
translateTimeForPlutusScripts pp = pvMajor (getField @"sppProtocolVersion" pp) > 5
...
missingScriptsSymmetricDifference ::
  ...
  pp ->
  Bool
missingScriptsSymmetricDifference pp = pvMajor (getField @"sppProtocolVersion" pp) > 6
...
forgoRewardPrefilter ::
  ...
  pp ->
  Bool
forgoRewardPrefilter pp = pvMajor (getField @"sppProtocolVersion" pp) > 6
...
allowOutsideForecastTTL pp =
  let mv = pvMajor (getField @"sppProtocolVersion" pp)
   in mv == 5 || mv == 6
