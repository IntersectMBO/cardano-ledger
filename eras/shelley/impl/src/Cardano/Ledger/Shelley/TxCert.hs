...
import Data.Foldable as Foldable (Foldable (..), foldMap', foldl')
...
shelleyTotalDepositsTxCerts pp isRegPoolRegistered certs =
  ...
  where
    ...
    numNewRegPoolCerts = Set.size (Foldable.foldl' addNewPoolIds Set.empty certs)
...
shelleyTotalRefundsTxCerts ::
  ...
  f (TxCert era) ->
  Coin
shelleyTotalRefundsTxCerts pp lookupDeposit = snd . Foldable.foldl' accum (mempty, Coin 0)
  where
