...
import Cardano.Ledger.State (PoolMetadata (..), ppCostL, ppMetadataL, ppVrfL, spsVrfL)
...
spec = describe "POOL" $ do
  ...
  where
    ...
    expectPool poolKh mbVrf = do
      pps <- (^. psStakePoolsL) <$> getPState
      (^. spsVrfL) <$> Map.lookup poolKh pps `shouldBe` mbVrf
    expectFuturePool poolKh mbVrf = do
      fps <- (^. psFutureStakePoolsL) <$> getPState
      (^. spsVrfL) <$> Map.lookup poolKh fps `shouldBe` mbVrf
    expectRetiring isRetiring poolKh = do
      retiring <- (^. psRetiringL) <$> getPState
      assertBool
