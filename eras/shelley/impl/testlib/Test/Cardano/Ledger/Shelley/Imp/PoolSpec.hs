...
spec = describe "POOL" $ do
  ...
  describe "Retiring pools" $ do
    ...
    it "re-register a retiring pool with an already registered vrf" $ do
      ...
      retirePoolTx kh1 (EpochInterval 10) >>= submitTopTx_
      pps <- poolParams kh1 vrf2
      let
        tx :: forall l. Typeable l => Tx l era
        tx = registerPoolTx pps
    ...
    it "re-register retiring pool with its own VRF" $ do
      (kh, vrf) <- registerNewPool
      retirePoolTx kh (EpochInterval 10) >>= submitTopTx_
    ...
    it "register a pool with the VRF of a retiring pool" $ do
      ...
      (kh, vrf) <- registerNewPool
      let retirement = 1
      retirePoolTx kh (EpochInterval retirement) >>= submitTopTx_
      khNew <- freshKeyHash
      pps <- poolParams khNew vrf
      let
        tx :: forall l. Typeable l => Tx l era
    ...
    it "retiring a pool clears its delegations" $ do
      ...
      retirePoolTx poolKh (EpochInterval retirement) >>= submitTopTx_
  ...
  it "Two Pools" $ do
    ...
    withTxsInBlock_ $ do
      ...
      registerPoolWithZeroPledge acpk carlAccAddr
      registerPoolWithZeroPledge bcpk carlAccAddr
    ...
    -- === Block 2, Epoch 1
    --
    -- Block 2 is produced by Alice's pool — this creates the first
    ...
    -- The reward pulser will use these
    ...
    evs4 <- impEventsFrom $ passNEpochs 2

    let extractRupdMap (SomeSTSEvent ev)
          | Just (TickRupdEvent (RupdEvent _ m)) <- cast ev :: Maybe (ShelleyTickEvent era) =
              m
        extractRupdMap _ = mempty

    let rewardMap =
          foldr (Map.unionWith Set.union . extractRupdMap) Map.empty evs4
    ...
    -- For pv >= 3 all rewards are summed, which is strictly more than
    -- the legacy aggregate; for pv <= 2 the behavior is identical.
    let eraAgg = aggregateRewards (ProtVer (eraProtVerHigh @era) 0) rewardMap
    if eraProtVerHigh @era >= natVersion @3
      then do
        Map.lookup carlSHK eraAgg `shouldBe` Just (foldMap rewardAmount carlRewards)
        (eraAgg Map.! carlSHK) `shouldSatisfy` (> legacyAgg Map.! carlSHK)
      else
        Map.lookup carlSHK eraAgg `shouldBe` Just (rewardAmount (Set.findMin carlRewards))
  where
    registerNewPool = do
      (kh, vrf) <- (,) <$> freshKeyHash <*> freshKeyHashVRF
      submitTopTx_ . registerPoolTx =<< poolParams kh vrf
      expectPool kh (Just vrf)
      pure (kh, vrf)
    registerPoolTx pps =
      mkBasicTx mkBasicTxBody
