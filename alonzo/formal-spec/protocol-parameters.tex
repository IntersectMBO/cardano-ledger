...
The new parameter $\var{maxValPerc}$ replaces the constant $\mathsf{MaxValPerc}$
used (in the ShelleyMA era) to limit the size of the the $\Value$ part of an output in a
serialised transaction.
It is expressed as a percentage, and therefore should not be set to be $>~100$.
...
\begin{figure*}[htb]
  ...
  \begin{equation*}
      \begin{array}{r@{~\in~}l@{\qquad}r}
        ...
        \var{maxValPerc} \mapsto \N & \PParams \\
        % & \text{Max perc. of total tx size a value can be}\\
