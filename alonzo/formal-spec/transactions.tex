...
This section outlines the changes that are needed to the Shelley/Allegra/Mary transaction
...
Shelley specification~\cite{XX}:\khcomment{Now also Allegra/Mary?}

\begin{itemize}
  \item $\ScriptDataHash$ is the type of a hash of script execution
  ...
  expects that all its non-native scripts will be validated.\khcomment{confirm}
  ...
  an optional hash of a data item. Note that \emph{any} output can
  ...
  script, we simply allow any output to contain a data hash.
...
%      & \text{reverse pointer to thing dv is for}
  ...
  \begin{align*}
    ...
    \nextdef
    \fun{hashScriptData}&\in\PParams \to \powerset{\Language} \to (\RdmrPtr \mapsto \Data) \to \ScriptDataHash^? \\
    \fun{hashScriptData}&~{pp}~{langs}~{rdmrs}~=~ \\
                          &\begin{cases}
  ...
  \begin{equation*}
    \begin{array}{r@{~~}l@{~~}l@{\qquad}l}
      ...
      & \times~ \seqof{\DCert} & \fun{txcerts}& \text{Certificates}\\
       ...
       & \times ~\hldiff{\ScriptDataHash^?} & \hldiff{\fun{sdHash}} & \text{Hash of script-related data}\\
...
\begin{itemize}
  ...
  \item a map of scripts, including non-native scripts;
  \item a map of terms of type $\Data$, containing all required datum objects; and
  \item a map of $\Data$ values indexed by $\RdmrPtr$, containing the redeemers.
\end{itemize}
...
\begin{itemize}
  \item We include a term of type $\ExUnits$. This is a budget in terms of the total number of execution units
  that may be used by all the non-native scripts that occur in the transaction.
...
Only the inputs that are locked by VKeys\khcomment{What is a VKey?} or native scripts can
be used to pay fees. As before, spending VKey or native script outputs is
validated as part of a single rule application2, so a transaction is only valid
if all
the required signatures are valid.
% , or else a transaction will be completely
% invalid.
