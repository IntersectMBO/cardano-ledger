...
minting tokens, spending outputs, verifying certificates, and
...
\begin{itemize}
  ...
  \item $\Data$ is a type for communicating data to scripts, kept abstract here.
...
\begin{figure*}[htb]
  ...
  \begin{equation*}
    \begin{array}{r@{~~}l@{~~}l@{\qquad}l}
      ...
      & \hldiff{\powerset{\TxIn}} & \hldiff{\fun{txinputs_{fee}}}& \text{Fee inputs}\\
  ...
  \begin{equation*}
    \begin{array}{r@{~~}l@{~~}l@{\qquad}l}
      ...
      & \hldiff{\powerset{\Data}} & \hldiff{\fun{dats}}& \text{Optional datums}\\
...
\begin{itemize}
  ...
  \item We include a hash of some data relating to scripts, specifically the redeemers and protocol parameters,
...
The transaction body and the UTxO must uniquely determine all data
...
signed their selection of inputs that will be put in the fee pot in case of script validation failure and
this have had the ability to check the outcome of those scripts before signing the body.

\subsection{Fees in the case of script validation failure}
...
be used to pay fees. Spending VKey or native script outputs is
validated as part of a single rule application as before, so either all
