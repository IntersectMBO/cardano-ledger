...
\begin{figure*}[htb]
  ...
  \begin{align*}
    ...
    &~~      \wedge~ (\fun{exunits}~tx \neq (0, 0) \Rightarrow \var{balance} \geq \txfee{txb}) \\
...
This can only be done by actually running the scripts. In order to  avoid over-spending the budget, we run scripts sequentially rather than in parallel,
stopping the validation whenever one does not validate, and charging the transaction the corresponding
fees. From the perspective of the ledger, there is no difference
between a script that exhausts the $\ExUnits$ budget during validation, and one that fails to validate.
If a transaction contains an invalid script, the only change to the ledger that is made
is to charge the fees.

Running scripts sequentially
to verify that they all validate within the total allotted $\ExUnits$ budget requires only
the remaining $\ExUnits$ to be included in the state.
In the environment, we need the protocol parameters and the
transaction that is neing validated. All the other data that is needed
to run the scripts comes from the signal.\khcomment{signal??}

The only state change in this rule is to the $\var{remExU}$  variable, which has the script exection cost deducted.
This variable keeps track of the remaining funds for
script execution. If the transaction overpays the required fee ($\fun{txscriptfee}~{tx}$
...
from \cite{Shelley-MultiAsset-Spec}\khcomment{You need to list these...}):
...
\begin{enumerate}
  ...
  \item The general accounting property holds\khcomment{what is that?};
  ...
  \item The transaction size does not exceed the maximum limit\khcomment{Defined where?};
...
(that all still apply\khcomment{confirm}), we have made the following changes and additions:

\begin{itemize}

\item All the native scripts in the transaction are valid;

    \item The transaction contains exactly those scripts that are required for witnessing and no
    additional ones; % (this includes all languages of scripts, for all purposes); \khcomment{Not clear what this means?}

  \item The transaction contains a redeemer for every data\khcomment{confirm} item that needs to be  validated
      by a Plutus script;

    \item The only certificates that are allowed to have scripts as witnesses are delegation de-registration certificates;

    \item Every output that is spent by a Plutus script has a corresponding data item in the transaction;\khcomment{wasn't clear}
    ...
    \khcomment{There is some weird edge case here where protocol parameters change across major protocol versions - this needs to be sorted out urgently, otherwise we can never retire/depecrate/repurpose protocol parameters...  It suggests that care needs to be taken
    when submitting transactions that will be run after a hard fork.}
...
by the UTXO transition (the application of which is also part of the conditions).\khcomment{Do we assume that transaction validation and execution happen within the same era?}
