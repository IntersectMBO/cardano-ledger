...
\item All the native scripts in the transaction are validate;

  \item For every item that needs to be validated by a non-native script, the transaction contains
    a redeemer and in case it validates spending an ouput also a datum;
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-witG]
    {
      ...
      \hldiff{\{ s \mid (\_, s) \in \fun{scriptsNeeded}~\var{utxo}~\var{tx}\} = \dom (\fun{txwits}~{tx})}
      \\~\\
