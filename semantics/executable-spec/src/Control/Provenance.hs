...
module Control.Provenance
  ( ProvM(..),  -- * Basic Provenance monad transformer
    ...
    BlackBox,   -- * Abstraction barrier to isolate the provenence type from the result type.
...
pushOtherProv key initial other = do
   t <- active
   if t
      then ProvM(do (a,v) <- lift (runWithProvM initial other)
...
observe m = unlines(map f (Map.assocs m))
  where f (key,PObject _ t) = unpack key++" =\n   "++show t
