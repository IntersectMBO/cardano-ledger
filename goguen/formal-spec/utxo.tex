...
\begin{itemize}
  \item the function $\fun{txinputs_{vf}}$ returns only those transaction inputs
    ...
    \begin{note}
      Make it more obvious that we check that the fee inputs only contain Ada, maybe mention it elsewhere
    \end{note}
...
\begin{figure}[htb]
  ...
  \begin{align*}
    ...
    & \fun{findRdmr}~{tx} ~=~ \{ \var{cur} \mapsto \fun{txrdmrs}~(\fun{txwits}~{tx})~ptr \\
      & ~~\where \\
      & ~~~~~~~ \var{txb}~=~\txbody{tx} \\
      & ~~~~~~~ \var{ptr}~=~
      \begin{cases}
