...
\item $\UTxO$ entries are stored in the finite map $\UTxOIn\mapsto \UTxOOut$.
  ...
  \item $\UTxOOut$ is the type of UTxO entries.
  ...
  \item $\CurItem$ is the type of things that scripts can validate.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item[(i)] the fee-marked inputs are not locked by non-native scripts,
    ...
    \item [(iv)] the fee amount the transaction states it is paying suffices to cover
    the minimum fee the transaction is obligated to pay.
  \end{itemize}
  ...
  \item The function $\fun{txins}$ that returns UTxO keys of transaction inputs.
\end{itemize}
...
\begin{itemize}
  ...
  transaction output and adds the slot number of the block in which transaction is
  ...
  for each of the components of $\ExUnits$.

  \item The minimim fee calculation, $\fun{minfee}$, includes the script
  fees the transaction is obligated to pay to run its scripts.

  \item The $\fun{produced}$ calculation requires the current slot number as an argument, which is
...
\begin{figure}[htb]
  \begin{align*}
    & \fun{getCurVals} \in \GoguenTx \to \UTxO \to \powerset{(\CurItem \times \PolicyID \times \seqof{\Data})} \\
