...
%  this type differs from the type of the output of a transaction.
  ...
  This will be used for future functionality. % that will be added in the future.
...
\item
  $\fun{runPLCScript}$ validates Plutus scripts\todo{Why runPLCScript rather than checkPLCScript?}. It takes the following
  arguments:
  \begin{itemize}
...
% This applies only to non-native
% scripts, as native script execution costs are expected to be much smaller
% and can be assesed and limited by the ledger rules directly.
...
the processing is done % by rules in the transition systems
in the following order:
% (each transition calls  the one that follows it below):
...
\begin{enumerate}
  ...
  (either because the transaction ran out of $\ExUnits$ or for any other reason); or\todo{Confirm ``or'' rather than ``and''.}
...
work with Shelley transactions\todo{cross-reference this -- (see explanation about transforming one type into
the other below)}\todo{Clarify - is $\Tx$ projected into $\GoguenTx$?}. This rule has the following preconditions (plus the relevant ones
