...
\begin{figure}[htb]
  \begin{align*}
    ...
    g& \fun{getOut} ~{txout}~= (\fun{getAddr}~\var{txout}, \fun{getValue}~\var{txout})
    \nextdef
...
\begin{itemize}
  ...
  \item $\fun{getCoin}$ suns all the Ada in a given output and returns it as a
  $\Coin$ value
...
\begin{figure}[htb]
  ...
  \begin{align*}
    ...
    & \text{\emph{-- sum total of amount of Ada in an output}}
    \nextdef
    ...
    & \text{\emph{-- returns the outputs in the UTxO with only the Ada coins}} \\
    \nextdef
