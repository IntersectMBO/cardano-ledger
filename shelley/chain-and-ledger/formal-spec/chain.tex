...
entropy for the next epoch nonce if the threshold of genesis keys are in agreement of the value,
and zero otherwise.
...
$\mathsf{BBODY}$ transition are overlay schedule slots.
...
\begin{figure}
  ...
  \begin{equation*}
    ...
    \powerset ((\powerset{\Slot)} \times \BBodyState \times \Block \times \BBodyState)
  \end{equation*}
...
\begin{figure}[htb]
  ...
  \begin{align*}
      ...
      \begin{array}{lr@{~=~}l}
        \where
          ...
          & \var{es}
          \\
          & (\wcard,~((\wcard,~\wcard,~\wcard,~\wcard,~\var{dms}),~\wcard),~\wcard)
          & \var{ls}
      \end{array}
  \end{align*}
  %
  ...
  \begin{align*}
      ...
      & \fun{updateNES}~
      (\var{e_\ell},~\eta_0,~\var{b_{prev}},~\wcard,~(\var{acnt},~\var{ss},~\wcard),~\var{ru},
       ~\var{pd},~\var{osched},~\var{us})
          ~\var{b_{cur}}~\var{ls} = \\
      & ~~~~
      (\var{e_\ell},~\eta_0,~\var{b_{prev}},~\var{b_{cur}},~(\var{acnt},~\var{ss},~\var{ls}),
       ~\var{ru},~\var{pd},~\var{osched},~\var{us})
  \end{align*}
  %
