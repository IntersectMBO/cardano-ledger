...
Note that in addition to the Genesis initial valid ledger state, we may also
consider a state $\fun{toShelley}(s)$ as a valid initial state, where $s \in
\CEState$ is a valid Byron state.

\begin{definition}[\textbf{Valid Ledger State}]
  \[\applyFun{validLedgerState}~\in~\LEnv~\to~\LState~\to~\Bool\]
  \label{def:valid-ledger-state}
\end{definition}

Definition~\ref{def:valid-ledger-state} gives the type of a function that evaluates
to $\mathsf{True}$ for valid ledger states in a given environment. That is, states that are reachable
from the genesis state via valid LEDGER, LEDGERS, NEWEPOCH, EPOCH, SNAP,
NEWPP, and POOLREAP STS transitions.
