...
of witnessing include public-private key pairs, one-way hash functions
...
\begin{figure}[htb]
  ...
  \begin{equation*}
    ...
    \end{array}
  \end{equation*}
  \emph{Genesis Key Notation}
  \begin{equation*}
    \begin{array}{r@{~\subset~}lr}
      \VKeyGen & \VKey & \text{subset of genesis public keys}\\
    \end{array}
  \end{equation*}
  \emph{Derived types}
  \begin{equation*}
...
Note here that we use the $\VKeyGen$ notation to represent the subset of genesis
(public verification) keys. Genesis keys are the keys belonging to the federated
nodes running the Cardano system currently (also referred to as core nodes).
The process of decentralization will
be gradual, and when it is complete, these keys will still have a special
significance to the system. For example, they will be the only ones able to
propose software and protocol updates. For more on the decentralization process,
...
compromised key does make it easier for an adversary to forge a signature that
