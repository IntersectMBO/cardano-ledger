...
\DueDate{10$^{\textrm{th}}$ October 2019}
\SubmissionDate{10$^{\textrm{th}}$ October 2019}{2019/10/09}
...
\hypersetup{
  ...
  \begin{changelog}
        \change{2019/10/10}{Kevin Hammond}{FM (\IOHK)}{Initial Version.}
      \end{changelog}
      \clearpage%
\begin{landscape}
\floatstyle{plain}
...
$D_E$ & \ADA{3.84M} & Distribution per Epoch in the Testnet & $\frac{\large A_r \times \textit{MER}}{\large 365 \div E}$ \\\hline
...
\noindent
The total \ada{} that is distributed per epoch in the Incentivised Testnet ($D_E$) is calculated from the initial
value of the \ada{} reserves, $A_r$ and the fixed monetary expansion rate, \textit{MER}.
...
the current exchange rate, $D$, as shown % in Figure~\ref{fig:monetary}
...
In the Testnet, however, they cannot be used for further staking.
...
To be added in version 0.2 of this document.
