...
The operational certificates $\var{cs}$ in the stake pool state are a tool to
prevent nodes from rejecting blocks signed by a new hot staking key.
These certificates are part of the block header, and allow nodes to process
blocks where the re-delegation information for the new key could be is inside a block
signed by that new key. For a discussion, see \cite{delegation_design}, and for
