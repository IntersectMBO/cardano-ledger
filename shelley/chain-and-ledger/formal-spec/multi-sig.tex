...
\newcommand{\TxInMultiSig}{\type{TxIn_{multi}}}
\newcommand{\TxInSimp}{\type{TxIn_{simple}}}
...
\item Change the type of transaction inputs to optionally include a validator
  script.
...
\item Adapt the functions used in the validation for the extended types of
  addresses and transaction inputs.
\end{itemize}
...
$\fun{validatorHash}$ returns the hash of a script of a script address.
...
\begin{figure*}[hbt]
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad=\qquad}lr}
      (\var{txid}, \var{ix}) & \TxInSimp & \TxId\times\Ix & \text{tx input} \\
      (\var{txid}, \var{ix}, \var{validator}) & \TxInMultiSig &
                                                          \TxId \times \Ix \times

                                                          \Script &
                                                                    \text{tx input
                                                                    for multi-sig} \\
      \var{txin} & \TxIn & \TxInSimp \uniondistinct \TxInMultiSig & \text{tx input}
    \end{array}
  \end{equation*}
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad=\qquad}l}
      \var{txbody}
      & \TxBody
      & \powerset{\TxIn} \times (\Ix \mapsto \TxOut \times \Script^{?}) \times \seqof{\DCert}
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      ...
      \fun{txouts} & \Tx \to (\Ix \mapsto \TxOut \times \Script^{?}) & \text{transaction outputs} \\
      \fun{validatorScript} & \TxInMultiSig \to \Script & \text{validator script}
    \end{array}
  \end{equation*}
...
In Figure~\ref{fig:types_defs_multi} the base types for transaction inputs and
outputs from~\cite{shelley_formal_spec} are extended to include simple scripts.
The type of transaction inputs, $\TxIn$ is split into sub-types, $\TxInSimp$ for
transaction inputs without scripts and $\TxInMultiSig$ for multi-signature.  In
addition to the transaction id and the index, a value of type $\TxInMultiSig$
also carries a validator script.

The transaction itself is extended to optionally carry a validator script for a
transaction output it will create. This validator script will then have to be
validated when the respective output is to be spent later.
...
scripts is the validation of the UTxOW transition of~\cite{shelley_formal_spec}.

The extended transition system is shown in
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-wit]
    {
      ...
      \forall (\wcard, \wcard, \var{script}) \mapsto (a, \wcard) \in \fun{txins}~tx
      \restrictdom (\TxInMultiSig \restrictdom \var{utxo}),\\ \fun{validatorHash}~\var{a}
      = \fun{hashScript}~script \wedge
      \fun{validateMultiSig}~{script}~{(\dom{(\txwits{tx})})}\\~\\
...
Figure~\ref{fig:functions-witnesses-multi-sig}. The functions $\fun{witsNeeded}$
only collected the payment keys for non-script addresses. The function
$\fun{outs}$ calculates the UTxO from transaction outputs, constructing values
...
\begin{figure}[htb]
  \begin{align*}
    ...
    & ~~\{ \fun{paymentHK}~a \mid i \mapsto (a, \wcard) \in
      \var{utxo},~i\in\txins{tx},~\fun{paymentHK}~a \neq \Nothing \}~\cup \\
    ...
    & \fun{outs} ~ \var{tx} =
        \left\{
          ...
          \middle|
          \var{ix} \mapsto (\var{txout}, \Nothing) \in \txouts{tx}
      \right\} & \\
    & \cup
      \left\{
      (\fun{txid} ~ \var{tx}, \var{ix}, \var{script}) \mapsto \var{txout} ~
      \middle|
      \var{ix} \mapsto (\var{txout}, \var{script}) \in \txouts{tx}
      \right\}
  \end{align*}
