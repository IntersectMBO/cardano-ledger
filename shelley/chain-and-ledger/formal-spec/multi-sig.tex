...
\newcommand{\StakeObject}{\type{StakeObj}}
...
\item Adapt the delegation rules in such a way that multi-signature delegation
  is supported.
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      ...
      \fun{stakeHK_{b}} & (\AddrVKeyB \uniondistinct \AddrScrBase) \to
                          \StakeObject & \text{stake object for base
                                      addresses}\\
      \fun{stakeHK_{r}} & \AddrRWD \to \StakeObject & \text{stake object
...
\emph{payment object} and the part used for staking as \emph{staking
  object}. Analogous to the payment object, the staking object for an $\AddrScr$
is also a script, i.e., staking can also require a combination of signatures
which can be different from the combination required for payment.
...
functions $\fun{stakeHK_b}$, $\fun{stakeHK_{r}}$ and $\fun{addrPtr}$ are
extended to also include the respective script address variants. The return
types are adapted to be a stake object.
...
There are two different types of ledger transitions that are impacted by
multi-signature. The first type is changing delegation of stake which requires
multi-signature. This impacts the DELPL transition which calls either its POOL
or DELEG subrule depending on the type of delegation certificate signal. The
second type is spending an element of the UTxO which required a multi-signature
for unlocking. This impacts the UTXOW transition.

\subsection{Delegation Transitions Rules}
\label{sec:deleg-trans-rules}

In order to validate a multi-signature for delegation, each delegation
certificate in a transaction has to be checked for correct signature
combinations. This requires the transaction $\var{tx}$ to be available in the
environment of the DELPL transition rule, as this rule is called with a single
certificate as its signal, this is reflected in the DELPL environment as shown
in Figure~\ref{fig:defs:delpl}.
  ...
  \begin{equation*}
    ...
    \left(
      \begin{array}{r@{~\in~}lr}
        ...
        \var{tx} & \Tx & \text{current transaction}
      \end{array}
    \right)
  \end{equation*}
  %
...
Multi-signature staking also requires a change to the type of stake object from
...
or a script used for staking, as shown in Figure~\ref{fig:delegation-state-type}.

\begin{figure}
  \emph{Delegation Types}
    \begin{equation*}
      \begin{array}{r@{~\in~}l@{\qquad=\qquad}lr}
        \var{stakeObj} & \StakeObject & (\KeyHash_{stake} \uniondistinct
                                            \HashScr) \\
  ...
  \begin{equation*}
    ...
    \left(\begin{array}{r@{~\in~}lr}
      ...
      \var{ptrs} & \Ptr \mapsto \StakeObject & \text{pointer to stake object}\\
...
Before executing a DELPL subrule, it is necessary to check whether the
certificate concerns a multi-signature. This is done via checking whether the
stake object is a hashed script and that the certificate is of one of the types
that do require a witness. This is shown in the extended transition system for
DELPL in Figure~\ref{fig:rules:delpl}, in the format described
in~\cite{small_step_semantics}

If the staking object is a hashed script and the delegation certificate type is
one which requires a witness, the DELEG subrule is only called if

\begin{itemize}
\item There exists a validator script in the set of witnesses of the transaction
  \var{tx} which corresponds to the staking object,
\item the staking object matches the hash of this script, and
\item the valdiator script validates the delegation certificate in the
  transaction.
\end{itemize}
...
\begin{figure}[hbt]
  ...
  \begin{equation}
    ...
    \inference[Delpl-Del]
    {
      ...
      stakeObj \leteq \fun{cwitness}~\var{c} \\
      \\~\\
      stakeObj \in \HashScr \wedge c \in (\DCertRegKey \uniondistinct
      \DCertDeRegKey \uniondistinct \DCertDeleg)\implies\\
      \exists validator,~stakeObj \mapsto validator \in scriptWits,
      \\ \fun{validateScript}~\var{validator}~\var{tx}
         \wedge \var{stakeObj} = \fun{hashScript}~\var{validator}
      \\~\\
      {
        \left(
          \begin{array}{r}
  ...
  \end{equation}
  \caption{Delegation Transition Rule}
  \label{fig:rules:delpl}
\end{figure}

\textbf{Note:} In contrast to stake object delegation, staking pools temselves
cannot use multi-signature schemes. Else, the lightweight certificates for
delegation from the pool to the KES hot key would have to be script
witnesses. These certificates have to be included in each header, but those
headers are required to have a fixed size and be as small as possible.
...
\begin{figure}[htb]
  \begin{equation*}
    \inference[UTxO-wit]
    {
      ...
      \\~\\
      \forall \var{addr^{script}_{rwd}} \in
      \dom (\AddrRWDScr\restrictdom\fun{txwdrls}~tx), \\
      \exists (\fun{stakeHK_{r}}~\var{addr^{script}_{rwd}}) \mapsto validator
      \in \var{scriptWits},\\
      \fun{stakeHK_{r}}~\var{addr^{script}_{rwd}} =
      \fun{hashScript}~\var{validator}
      \wedge \fun{validateScript}~validator~tx
      \\~\\
...
validates the transaction. Because of the changes to the staking object type,
the original function $\fun{witsNeeded}$ is changed as shown in
Figure~\ref{fig:functions-witnesses}.
...
\begin{figure}[htb]
  \begin{align*}
    ...
    & ~~\{\fun{stakeHK_r}~a \mid a\mapsto \wcard \in \AddrRWDVKey
      \restrictdom \txwdrls{tx}\}~\cup \\
...
staking object) in its serialized form. This tag will also be part of the hash
to allow for easy identification of the language type.

As an example, one could tag key hash payment and staking objects with a tag
$0$, native multi-sig payment and staking objects with a tag $1$ and simple
Plutus objects with a tag $2$. Every new language would incur using a new tag,
changes to the details of how payment or staking objects are treated
\emph{within} an existing language framework will be changed via a software
update, not an additional tag.
