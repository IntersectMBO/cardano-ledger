...
or a script used for staking, as shown in Figure~\ref{fig:defs:delpl}.
...
that do require a witness.

\textbf{Note:} pool delegation certificates cannot use multi-signature
schemes. Else, the lightweight certificates for delegation from the pool to the
KES hot key would have to be scripts witnesses. These certificates have to be
included in each header, but those headers are required to have a fixed size and
...
\begin{figure}[hbt]
  ...
  \begin{equation}
    ...
    \inference[Delpl-Del]
    {
      ...
      \exists validator,~stakeObj \mapsto validator \in scriptWits \wedge
...
\begin{figure}[htb]
  \begin{equation*}
    \inference[UTxO-wit]
    {
      ...
      \\~\\
      \forall \var{reward}\mapsto\var{validator} \in
      \AddrRWDScr\restrictdom\fun{txwdrls}~tx,
      \fun{validateScript}~validator~tx
      \\~\\
...
Multi-signature staking also incurs a reward account which is locked with a
multi-signature scheme. This means that in order to allow spending of rewards
accumulated in a multi-signature rewards account, we also need to validate the
required script. This is done in the $\fun{witsNeeded}$ function as shown in
...
update, not an additional, new tag.
