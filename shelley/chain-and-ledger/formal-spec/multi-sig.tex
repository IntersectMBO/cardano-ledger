  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      ...
      \fun{stakeHK_{b}} & (\AddrVKeyB \uniondistinct \AddrScrBase) \to
                          \StakeDelegs & \text{hash of delegator for base
                                         addresses}\\
      \fun{stakeHK_{r}} & \AddrRWD \to \StakeDelegs & \text{hash of delegator
...
\begin{figure}
  \emph{Delegation Types}
    \begin{equation*}
      \begin{array}{r@{~\in~}l@{\qquad=\qquad}lr}
        \var{stakeDelegator} & \StakeDelegs & \KeyHash_{stake} \uniondistinct
                                            \HashScr \\
      \end{array}
    \end{equation*}
    %
...
Multi-signature staking also requires a change to the type of delegators from
...
$\type{StakeKeys}$ type as shown in Figure~\ref{fig:defs:delpl}.
...
certificate concerns a multi-signature. This is checked via checking whether the
delegator is a hashed script and that the certificate is of one of the types
...
\begin{figure}[hbt]
  ...
  \begin{equation}
    ...
    \inference[Delpl-Del]
    {
      ...
      delegator \leteq \fun{cwitness}~\var{c} \\
      \\~\\
      delegator \in \HashScr \wedge c \in (\DCertRegKey \uniondistinct
      \DCertDeRegKey \uniondistinct \DCertDeleg)\implies\\
      \exists validator,~delegator \mapsto validator \in scriptWits \wedge
