...
Multi-signature staking also requires a change to the type of staking reference
from being just a hashed key to being either a hashed key or a hashed
script. This is reflected in the type of $\StakeDelegs$ with replaces the
previous $\type{StakeKeys}$ type and the type $\StakeObject$ which describes
either a key or a script used for staking, as shown in
...
staking reference is a hashed script and that the certificate is of one of the
types that do require a witness. This is shown in the extended transition system
for DELPL in Figure~\ref{fig:rules:delpl}, in the format described
in~\cite{small_step_semantics}

If the staking reference is a hashed script and the delegation certificate type
is one which requires a witness, the DELEG subrule is only called if

\begin{itemize}
\item There exists a validator script in the set of witnesses of the transaction
  \var{tx} which corresponds to the staking reference,
\item the staking reference matches the hash of this script, and
...
\begin{figure}[hbt]
  ...
  \begin{equation}
    ...
    \inference[Delpl-Del]
    {
      ...
      stakeCred \leteq \fun{cwitness}~\var{c} \\
      \\~\\
      stakeCred \in \HashScr \wedge c \in (\DCertRegKey \uniondistinct
      \DCertDeRegKey \uniondistinct \DCertDeleg)\implies\\
      \exists validator,~stakeCred \mapsto validator \in scriptWits,
      \\ \fun{validateScript}~\var{validator}~\var{tx}
         \wedge \var{stakeCred} = \fun{hashScript}~\var{validator}
      \\~\\
      {
        \left(
          \begin{array}{r}
...
temselves cannot use multi-signature schemes. Else, the lightweight certificates
for delegation from the pool to the KES hot key would have to be script
