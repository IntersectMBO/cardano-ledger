...
\begin{figure*}[hbt]
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      \fun{txins} & \Tx \to \powerset{(\TxIn \times \Script^{?})} &
...
are not required here, we use them with the unit type. $\PendingTx$ is a
representation of the transaction that is to be validated. In particular, this
information contains the set of hash keys that did sign the transaction. The
unit return type signifies that there is no data to return, Plutus uses an
``error'' state to represent a failed validation. Despite that, we will use the
validate function as a predicate where \emph{False} corresponds to the Plutus
error state and \emph{True} to the unit return result.
...
as in Figure~\ref{fig:types_defs_multi}.
...
\begin{figure}[htb]
  \begin{equation*}
    \inference[UTxO-wit]
    {
      ...
      \fun{validateMultiSig}~{s}~{(\dom{(\txwitsVKey{tx})})}\\~\\
