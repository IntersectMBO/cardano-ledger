...
\DeliverableTitle{A Formal Specification of a Multi-Signature Scheme \\ using Scripts}
...
\SubmissionDate{11th September 2019}
...
\begin{changelog}
	\change{11/09/19}{Jared Corduan and Matthias G\"udemann}{FM (IOHK)}{Initial version (0.1).}
	\change{11/09/19}{Kevin Hammond}{FM (IOHK)}{Added cover page, applied review comments.}
\end{changelog}
...
%
  For an output that is locked with\khcomment{protected by? mandates? requires?
    depends on? needs to be authorised by?} a ``multi-signature'', the set of all the keys that have signed the
  transaction is passed to a validation script. In this way, the script can
  decide in a single step whether or not it has a combination of keys that
  will permit the output to be spent correctly.  It is possible to supply more signatures than are required
  to authorise the transaction.  In this case, any subset of valid signatures can be used.
\end{abstract}
...
In Figure~\ref{fig:types-scripts} the $\Addr$ type of the Cardano Ledger formal specification~\cite{shelley_spec}
...
In accordance with the extended UTxO
specification~\cite{plutus_eutxo}, the producer signs the script when creating the transaction\khcomment{?}
and the consumer provides its  own signature \khcomment{?} later when spending the output. The  $\Script$ type
is partitioned into subtypes: here,
$\ScriptPlutus$ for Plutus scripts, $\ScriptMSig$ for native
interpreter scripts (see Sections~\ref{sec:plutus-scripts}
...
$\AddrScr$ sub-type of $\Addr$ (Figure~\ref{fig:types-scripts}).
This can therefore be part of a transaction output that consists of a
pair of $\Addr\times\Coin$. Analogously to $\AddrVKey$, $\AddrScr$ also has an
\emph{enterprise} script address sub-type which does not allow
locked funds to be used in staking, as well as \emph{base} and \emph{pointer} script
address sub-types. We will refer to the parts of an address that is used for payment as the
\emph{payment object} and the part that is used for staking as the \emph{staking
  reference}. Analogously to the payment object, the staking reference for an
$\AddrScr$ is also a script.  This means that staking can also require a combination of
signatures and these may be different from the combination of signatures that is required for payment.

The $\fun{hashScript}$ function calculates the hash of the serialized
script. The accessor function $\fun{validatorHash}$ returns the hash
...
include the respective script address variants. The return types are changed to
...
Figure~\ref{fig:types_defs_multi} extends the type of a transaction
...
$\fun{txwitsScript}$ returns a map\khcomment{set?} of script hashes.
In order for a transaction to be accepted, all the corresponding scripts need to validate the transaction.
...
Figure~\ref{fig:defs:functions-txins} shows the
$\fun{txinsVKey}$ and $\fun{txinsScript}$ scripts, which partition the set of transaction
inputs of the transaction into those that are locked with a private key and
those that are locked via a script, respectively.\khcomment{A single function could be used here, taking the type as a parameter.}

\section{Ledger Transitions for Multi-Signature}
\label{sec:ledg-trans-multi}

While spending values\khcomment{?} and altering delegation decisions can both be locked by
multi-signature scripts, script validation can be
treated in the same way for both cases. The validation of all witnesses occurs through the UTXOW
STS rule, using the \fun{witsVKeyNeeded}  function to collect all the necessary key
...
but headers are required to have a minimal and fixed size.\khcomment{What size is acceptable?}

\subsection{UTXOW Transition Rule}
\label{sec:utxow-transition}

The UTXOW extended transition system\khcomment{Refer to location in original document} is shown in
Figure~\ref{fig:rules:utxow-multi-sig}
\khcomment{Something has gone wrong with ``inference''.  Also, the layout is poor - don't split conditions over 2 lines, or lay them out as centred with each other.}. The constraint on the set of required
witnesses is relaxed in such a way that \emph{redundant} signatures can be
supplied in the transaction.\khcomment{redundant meaning ``not required'' or ``duplicate''?} The set of verification keys is passed to the
...
\noindent
We also check that the set of spent outputs that is locked by a script is equal to the
elements of the inputs for which we have a validator script.\khcomment{This sentence is confusing...} This ensures that
there exists a corresponding validation script for each output that is locked by a script.
...
$
    \inference[UTxO-wit]%
    {
%% I'm getting a latex error in this which I can't fix!  KH
 %       \begin{array}{l}
 ...
 %       \fun{scriptsNeeded}~\var{utxo}~\var{tx}   \subseteq \dom (\fun{txwitsScript}~{tx})
 %       \\~\\
...
\khcomment{Use a dependent type :)}
