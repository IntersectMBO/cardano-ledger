...
\begin{itemize}
  ...
  \item A triple of:
    \begin{itemize}
      ...
      \item A finite map from script hashes to scripts. (NSF: Presumably the key of each mapping must be the hash of the value. Seems worth being explicit here.)
...
\begin{figure*}[htb]
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}l@{\qquad=\qquad}l}
      ...
      & \TxBody
      & \begin{array}{l}
        \powerset{\TxIn} \times \seqof{\TxOut} \times \seqof{\DCert}
        \times \Coin \times \Slot \times \Wdrl
        \\ ~~~~\times \Update \times \MetaDataHash^?
        \end{array}
      \\
      \var{wit} & \TxWitness & (\VKey \mapsto \Sig, \HashScr \mapsto \Script) (NSF: Slightly to the scripts being referred to as ``witnesses''.)
      \\
      \var{tx}
      & \Tx
