...
(NSF: The $pay$ and $stake$ subscripts are not defined or discussed. Are those
subscripts just for legibility?)

(NSF: $\fun{addr_{rwd}}$ is used throughout, but it seems to be the only such
``Constructor function made explicit as such. For example, the definitions in
Figure 8 Helper functions for the Protocol Parameters freely conflate Slot
values with Epoch values and with KESPeriod values. Is there a reason for this
special treatment? I feel like I'm maybe missing something.)
