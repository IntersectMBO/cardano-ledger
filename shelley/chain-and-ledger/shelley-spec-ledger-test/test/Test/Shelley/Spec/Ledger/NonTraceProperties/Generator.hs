...
genNonemptyGenesisState :: forall c v. (ExMock c,CV c v) => Proxy c -> Gen (LedgerState c v)
...
genTx :: (ExMock c,CV c v) => KeyPairs c -> UTxO c v -> SlotNo -> Gen (v, Tx c v)
...
genNonEmptyAndAdvanceTx :: forall c v.
  (ExMock c,CV c v) =>
  Proxy c ->
  Gen (KeyPairs c, Natural, v, LedgerState c v, [Tx c v],
...
genValidLedgerState :: forall c v. (ExMock c,CV c v) => Proxy c -> Gen (KeyPairs c, Natural, [Tx c v], LedgerState c v)
...
genStateTx :: forall c v. (ExMock c,CV c v) => Proxy c -> Gen (LedgerState c v, Natural, v, Tx c v, LedgerValidation c v)
genStateTx p = do
  (keyPairs, steps, _, ls) <- genValidLedgerState p
