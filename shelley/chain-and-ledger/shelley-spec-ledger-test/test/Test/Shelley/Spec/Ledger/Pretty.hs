...
import Test.Tasty
import Test.Tasty.QuickCheck (testProperty, (===), generate, Arbitrary(..),Gen, withMaxSuccess)
...
testPP name ppT gen = testProperty name (do t <- gen; pure(withMaxSuccess 25 (toText(ppT t) === toText(ppT t))))
  where toText doc = renderStrict(layoutPretty defaultLayoutOptions doc)

prettyTest :: TestTree
prettyTest =
   testGroup "Pretty printer tests"
