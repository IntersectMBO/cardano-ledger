...
insertOrUpdate :: UTxOOut C -> Map (Addr C) (CompactValue C) -> Map (Addr C) (CompactValue C)
...
propPreserveBalanceInitTx =
  property $ do
    (_, steps, fee, ls, tx, next) <- Hedgehog.forAll (genNonEmptyAndAdvanceTx (Proxy @C)) -- TODO should the forge be added up to over all tx's?!
    ...
    case next of
      ...
      Right ls' -> do
        ...
        (getAdaAmount $ balance ((_utxo . _utxoState) ls)) === (getAdaAmount $ balance ((_utxo . _utxoState) ls') <> (coinToValue fee)) -- TODO check forge side
...
propBalanceTxInTxOut = property $ do
  ...
  ((_forge tx) <> (balance $ eval (inps <| ((_utxo . _utxoState) l)))) === (balance (txouts tx) <> (coinToValue fee))
...
propNonNegativeTxOuts =
  withTests 100000 . property $ do
    (_, _, _, tx, _) <- Hedgehog.forAll (genStateTx (Proxy @C))
...
propBalanceTxInTxOut' =
  withTests 1000 $
    property $ do
      (l, _, fee, txwits, lv) <- Hedgehog.forAll (genStateTx (Proxy @C))
      let tx = _body txwits
      let inps = txins tx
      let getErrors (LedgerValidation valErrors _) = valErrors
      let balanceSource = getAdaAmount $ balance $ eval (inps <| ((_utxo . _utxoState) l))
      let balanceTarget = getAdaAmount $ balance $ txouts tx
...
propPreserveBalance = property $ do
  ...
  let destroyed =
        balance ((_utxo . _utxoState) l)
          <>  (coinToValue $ keyRefunds emptyPParams $ _body tx)
  let created =
        balance ((_utxo . _utxoState) l')
          <> (coinToValue $ fee
