...
will distinguish between \emph{value addresses} that determine how funds can be
spent, and \emph{stake addresses} that define if and how those funds take part
in the PoS protocol. Actual coins will belong to an (overall) \emph{address},
consisting of both a value address and a stake address.
...
addition, a Shelley address also contains a stake credential. When calculating
the stake distribution (see \cref{delegation-relations} for details), the system
uses this stake credential to decide where to count the stake corresponding to
the tokens in this address.

There are three options for the stake credential in a Shelley address: it can be
provided \emph{by value}, i.e., just be the hash of a verification key or
validator script. Shelley addresses that provide their stake credential by value
are sometimes also called \emph{base addresses}.

There is also a more compact way of representing a stake credential: since stake
...
also include them \emph{by reference}, pointing to the certificate that
registered the stake address containing a given stake credential. Since the
...
credential by reference are also called \emph{pointer addresses}.

The third possibility is to not have a stake credential at all. In this case,
...
Allowing the stake credential to be included in a payment address via reference
...
First, we need to consider the case that the reference does not point to an
...
their stake credential bby value, where the stake address can be registered
...
the stake address by reference to that certificate will lose their stake rights.
