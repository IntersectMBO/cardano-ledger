...
\item
  ...
  \end{itemize}
\item
  Pool rewards will slightly increase with the stake their operator contributes to
  the pool. There is no minimal stake required to create a pool -- anybody can
  do this. However, pools led by operators with high stake will get higher
  rewards. (This will discourage pool operators from splitting their stake to
  operate several pools. It will also help preventing Sybil attacks, where an
  attacker with low stake tries to gain control over a majority of stake by
  creating a lot of pools with low costs.)
\end{itemize}
...
\item
  The influence \(a_0\in[0,\infty)\) a pool operator's stake should have
...
from an epoch will actually be distributed during that epoch. The remainder of
an epoch's rewards pot will be moved over to the next epoch's rewards pot.
...
\item[\(r_e\)] is a factor that captures the relation between the relative stake
  ...
  and the total number \(\overline{N}\) of blocks that were added to the chain during
  the epoch. To be precise, we have
  \[r_e = \frac{N}{\sigma \overline{N}}\,.\]
  ...
  \(\overline{N}\) (leader election, forks, performance of the network and other
  pools).
\end{enumerate}

If we insert the definitions of \(p\) and \(r_e\) into \(\beta\), we find that
\[
\beta = \sigma \frac{n}{\max(N,1)} \frac{N}{\sigma \overline{N}}
= \frac{n}{\overline{N}}\,,
\]
...
\item
  \(a_0\in[0,\infty)\) is a parameter determining operator-stake influence
  on pool rewards.
\item
...
\item
  ...
  operator (the amount of ada pledged during pool registration,
...
operator.} For example, let us assume that the total existing supply of
ada is \(T=31,000,000,000\), and consider a pool whose pool operator
...
The \emph{pool operator reward \(r_\mathrm{operator}\)} (in ada) is
calculated as follows (where \(s\in[0,1]\) is the stake of the pool
operator): \[
    r_\mathrm{operator}(\hat{f}, c, m, s, \sigma) :=
    \left\{
    \begin{array}{ll}
        \displaystyle\hat{f} &
        \text{if $\hat{f}\leq c$,} \\
...
First we define the \emph{desirability} of a pool whose operator has stake
...
Consider a pool with operator stake \(s\), total stake \(\sigma\) and rank
...
\(m\), operator stake \(s\), stake \(\sigma\), rank \(r\), and apparent
...
operator stake \(s\), stake \(\sigma\), rank \(r\), and apparent performance
\(\pbar\), for a member contributing member stake \(t\), are
\begin{equation}
...
of the operator's stake and the stake that this member chooses to delegate.
...
and pool operator \emph{stakes}, as well as the (non-myopic) utilities derived
...
pool operator's stake has on pool rewards.
...
\] (where \(\lambda\) is the stake committed by the pool operator and \(c\)
