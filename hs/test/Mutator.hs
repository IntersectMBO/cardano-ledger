...
mutateDCert keys _ (RegPool (PoolParams _ pledge pledges cost margin altacnt rwdacnt)) = do
  key'    <- getAnyStakeKey keys
  ...
  pure $ RegPool (PoolParams key' pledge pledges cost' interval altacnt rwdacnt)
