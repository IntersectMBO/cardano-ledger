...
\khcomment{Difficult question: who controls registration/de-registration of submitter keys.}\pkcomment{How about an on-chain vote by ada holders?}\khcomment{How do we
...
quorum may be a protocol parameter.}\pkcomment{with an enforced sensible range in $(0,1)$}  No fee is charged for proposal
...
\end{verbatim}\pkcomment{Do we need \texttt{Slot}, or \texttt{Epoch}?}
...
Transfers from treasury to treasury or from reserves to reserves are not permitted.  All transfers must be for a positive amount (including zero\pkcomment{Why do we want to allow zero here?}).
