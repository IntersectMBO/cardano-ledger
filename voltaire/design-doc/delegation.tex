...
\begin{tabular}{||l|p{3in}|l||}
  \hline\hline
  Delegate public key hash % \khcomment{The delegate key structure needs to be confirmed}
...
% \khcomment{We could ask for pledge here, but this sounds a bit like vote buying?}  I added it anyway.
...
% \khcomment{This needs to be defined, and the size confirmed above.}
...
% \khcomment{There is an argument that you should  be able to delegate to no-one, but it seems sensible to maintain consistency with the stake delegation mechanism.}
...
%% Something for Shelley - may not matter anyway
%\khcomment{Not completely clear how the payment address is then removed - there's no on-chain registration.  Does it just become useless and a target for GC?}
...
%% Yes, just generated from keys, I believe.
