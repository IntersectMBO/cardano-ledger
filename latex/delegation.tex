...
Delegation, registration and deregistration are all performed via certificates.
These certificates need not be kept track of in any way after they are validated
and applied (i.e. the action it certifies is performed).
...
one should, however, think of it as a type with five initial constructors, one for
...
Delegations are also expressed by a finite map which relates the stake key
...
When registering a new stake key (the Deleg-Reg inference rule),
the key must be
...
When deregistering a key (the Deleg-Dereg rule), we again
...
Finally, for creating a delegation (the Deleg-Deleg rule),
...
necessary to include $cepoch$ in the context for Pool-Reg. However,
...
The first rule, Pool-Reg, can be used to register a new stake pool, register a
...
The second rule, Pool-Retire, starts the pool retirement process. Given an
...
The third rule, Pool-Reap, is used when the planned retirement epoch $e$ for
a given stake pool in the $\fun{retiring}$ set is reached. In this case, the
pair associated with the $stpools$
