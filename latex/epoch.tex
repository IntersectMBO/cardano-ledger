...
in the set of protocol constants needed for performing rewards calculations.
...
following variables (sourced as indicated):
...
\item $\var{R}$ is the total available rewards for the epoch (in ADA) \newline

Where is this from? \\
\item $\overline{N}$, for a given pool, is the expectation value of the number
of slots that the pool will be elected as leader on average \newline

From protocol constants: \\
\item $k$ is the optimal number of saturated stake pools (which the system
tends to by protocol design)
\item $\var{a_0} \in [0, \infty)$ is a parameter determining leader-stake
influence on pool rewards \newline

Calculated by the functions defined below using the variables above: \\
\item $\var{z_0} := 1/k$ is the size of a saturated pool
\item $\sigma' := \fun{min}(\sigma, z_0)$, where $\sigma$ is the relative stake of the pool
\item $s' := \fun{min}(s, z_0)$,
\item $\sigma$ is the average amount of stake (expressed as a fraction of
the total stake) a pool has
\item $s$ is the fraction of the total stake a particular pool has \\
  $s$ is the relative stake of the pool leader (the amount of ADA pledged during pool registration
\end{itemize}

\begin{todo}
  What is s? Is sigma correct?

  Where does N come from?
\end{todo}

The function $\fun{maxPool}$ gives the maximum possible amount of rewards a pool
can receieve in an epoch. For a given pool, it is calculated as a fraction of
the total available rewards. This result depends on the protocol
constants, the average stake distribution, and the amount of stake the pool has.
...
the expectation of the number of slots the pool will be elected to be a leader of.

Next, the function $\fun{poolRew}$ gives the total rewards to be distributed
within the given pool. This value is based on the re-calculated (for this epoch) pool
average and additional protocol constants.
...
\begin{figure}[htb]
  \begin{align*}
      ...
      & \text{max pool reward} \\
      & \fun{maxPool}~\var{pc}~\var{R}~\sigma~\var{s} = \\
      & ~~~\floor*{
           \frac{R}{1 + a_0}
           \cdot
      ...
      & ~~~\to (\HashKey \mapsto \mathbb{R}) \to [0, 1]
      & \text{moving average} \\
      ...
      & ~~~\to (\HashKey \mapsto \mathbb{R}) \to \Coin \to (\Coin \times [0, 1])
      & \text{pool reward} \\
...
the total value is presented above, denoted $\fun{poolRew}$.
...
on pool-specific constants (these come from the ledger state), as well as
the $\sigma$ and $s$ (calculated as part of reward
distribution, see below). The calculation for the pool
...
reward amount for any pool member, using a boolean as an input to indicate
whether a given member is the pool leader or not.

Here we also present the $\fun{groupByPool}$ map, which, given a stake distribution
for all registered stake keys, as well as the current delegations, produces a
finite map indexed by the hash keys of pool leaders. Each leader key is associated
with a subset of the stake distribution finite map. This associated subset
contains only the stake distribution entries where the hash keys delegate to the pool
...
\begin{figure}[htb]
  \begin{align*}
      ...
      & \begin{cases}
        ...
        \end{cases} \\
      & ~~~\where \\
      & ~~~~~~~(c, m) = \fun{poolConsts}~pool \\
      \nextdef
      & \fun{memberRew} \in \Coin \to \StakePool \to [0, 1] \to [0, 1] \to \Coin
      & \text{pool member reward} \\
      & \fun{memberRew}~\hat{f}~\var{pool}~\sigma~\var{t} = \\
      & \begin{cases}
          0 & \hat{f} \leq c\\
          (\hat{f} - c)\cdot(1-m)\cdot\frac{t}{\sigma} &
          \text{otherwise.}
        \end{cases} \\
      & ~~~\where \\
      & ~~~~~~~(c, m) = \fun{poolConsts}~pool \\
      ...
      & \begin{cases}
        ...
        \end{cases} \\
      \nextdef
      & \fun{groupByPool} \in \Distr \to (\HashKey \mapsto \HashKey) \\
      & ~~~\to (\HashKey \mapsto \Distr)
      & \text{pool individual reward} \\
      & \fun{groupByPool}~\var{dstr}~\var{delegs} = \\
      & ~~~\Big\{ leader \mapsto \{hk \mapsto d \in distr \mid (hk \mapsto leader)\in\var{delegs}\}
                  \Big\}
  \end{align*}
...
the stake key $\var{poolHK}$). The pool is passed to $\fun{rewardOnePool}$
via the $\var{distr}$ variable, which is a subset of hash keys in this pool
paired with the stake fractions associated with the keys.

Given, additionally, pool and protocol constants,
as well as the total available rewards, block production,
and the pool averages, this map outputs a finite map and an $\var{avg}$
...
and $\fun{poolReq}$ calculations. The $\var{avg}$ value for the stake pool
(i.e. the pool's ranking) is also calculated by the $\fun{poolRew}$ function.

\begin{todo}
  in rewardOnePool, sigma should be an average, i.e. divided by the size of distr?
  should it be k $\div$ tot?
\end{todo}

The $\fun{reward}$ calculation itself uses the $\fun{rewardOnePool}$
function to reward each pool and calculate the pools' moving averages. In order
...
Within the $\fun{reward}$ calculation, the stake distribution $\var{stDistr}$
is calculated for each stake key by dividing the the active stake associated
with that key by the total active stake. This distribution is then grouped
into subsets by the pool each stake key belongs to using the
$\fun{groupByPool}$ mapping.

Next, a dataset of $\var{results}$ is generated
...
\begin{figure}[htb]
  \begin{align*}
      ...
      & ~~~\to \Distr \to (\HashKey \mapsto \mathbb{R}) \to (\AddrRWD \mapsto \Coin)\times[0, 1]
      & \text{reward one pool} \\
      & \fun{rewardOnePool}~\var{pc}~\var{R}~\var{n}~\var{poolHK}~\var{pool}~\var{distr}~\var{avgs} = \\
      & ~~~(\{ (\AddrRWD~hk)\mapsto ( \fun{indivRew}~\var{poolR}~\var{pool}
                                             ~\sigma~\var{s}~(hk ==\var{poolHK})) \\
      & ~~~~~~~\mid hk\mapsto s\in\var{distr}\},~\var{avg})\\
      & ~~~\where \\
      & ~~~~~~~\sigma = \sum_{\_\mapsto t\in\var{distr}} t\\
      & ~~~~~~~maxP = \fun{maxPool}~\var{pc}~\var{R}~\sigma~\var{s}~\var{poolConsts}\\
      ...
      & ~~~~~~~pdistr = \fun{groupByPool}~\var{stDistr}~\var{delegs} \\
      & ~~~~~~~pdata = \left\{
                         hk\mapsto (pool, n, distr)  \mathrel{\Bigg|}
                          \begin{array}{r@{\mapsto}c@{~\in~}l}
                          hk & \var{pool} & \var{pparams} \\
                          hk & \var{n} & \var{prod} \\
                          hk & \var{distr} & \var{pdistr} \\
                          \end{array}
                       \right\} \\
      & ~~~~~~~results = \{hk \mapsto \fun{rewardOnePool}~\var{R}~\var{pc}~\var{n}~\var{hk}~\var{pool}~\var{distr}~\var{avgs}
                         \mid \\
      &\qquad \qquad \qquad \qquad hk\mapsto(n, pool, distr)\in\var{pdata} \}
  \end{align*}
