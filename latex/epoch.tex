...
\begin{figure}[htb]
  \begin{align*}
      ...
      & \fun{baseStake}~{txouts} = \\
      & ~~ \{(hk, c) \mid a\in\Addr_{base} \land (a, c) \in txouts \land \fun{stakeHK}~a = hk \}
      \nextdef
      & \fun{ptrStake} \in \powerset{\TxOut} \to (\Ptr \mapsto \HashKey)
          \to \powerset{(\HashKey \times \Coin)}
      & \text{base address stake} \\
      & \fun{ptrStake}~{txouts}~{ptrs} = \\
      & ~~ \{(hk, c) \mid a\in\Addr_{ptr} \land (a, c) \in txouts \land (\fun{stakePtr}~a\mapsto hk \in ptrs \}
      \nextdef
      & \fun{stake} \in \powerset{\TxOut} \to (\Ptr \mapsto \HashKey)
          \to \powerset{(\HashKey \times \Coin)}
      & \text{base address stake} \\
      & \fun{stake}~{txouts}~{ptrs} = (\fun{baseStake}~{txouts}) \cup (\fun{ptrStake}~{txouts}~{ptrs})
      \nextdef
      ...
      & \fun{totalStake}~\var{txouts}~\var{ptrs}~\var{stkeys}~\var{delegs}~\var{stpools} =
        \sum_{\_\mapsto c\in \var{active}}c \\
      & \where active = \activeStake{txouts}{ptrs}{stkeys}{delegs}{stpools}
      \nextdef
      & \fun{stakeDistr} \in \powerset{\TxOut} \to (\Ptr \mapsto \HashKey) \to \Allocs\\
      & ~~\to (\HashKey \mapsto \HashKey) \to \Allocs \to (\VKey \mapsto [0, 1])
      & \text{stake distribution} \\
      & \fun{stakeDistr}~\var{txouts}~\var{ptrs}~\var{stkeys}~\var{delegs}~\var{stpools} = \\
  ...
  \begin{equation*}
    ...
    \left(
      \begin{array}{r@{~\in~}ll}
        ...
        \var{pcOld} & \PrtclConsts & \text{old protocol constants}\\
        \var{pcNew} & \PrtclConsts & \text{new protocol constants}\\
        \var{production} & \VKey \mapsto \mathbb{N} & \text{blocks produced in the epoch}\\
      \end{array}
    \right)
  \end{equation*}
  %
