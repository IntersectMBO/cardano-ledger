...
\item The signature \var{sigma} can be verified with the cold verification key
  \var{vk_{cold}}.
\end{itemize}
...
\begin{itemize}
\item The slot \var{slot} of the block.
\item The hash of the block header \var{h} of the previous block.
...
\item The verification key validates the signature \var{\sigma} for the block
  header.
\end{itemize}

The state update finally sets \var{slot} as new block header slot and the hash
of \var{bh} as the new block header hash in the block header state.
...
been signed with the issuer key.
...
\begin{figure}[ht]
  \begin{equation}\label{eq:bheader}
    \inference[BHead]
    {
      ...
      \var{h} = \bprev{bhb}
      \\
      \mathcal{V}_{\var{vk_{hot}}}{\serialised{bhb}}_{\sigma}^{t}
      &
...
nonce \var{\eta} and the block slot \var{s}. The transition rule itself has no
