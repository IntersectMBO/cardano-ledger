...
are presented in Figure~\ref{fig:defs:utxow}. We introducs two new abstract types,
...
be obtained from the the transaction by applying the $\fun{wits}$ function.
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-wit]
    {
      ...
      ~ \\
      & \forall c \in \fun{dcerts}~{tx} \cdot \exists! \dwit{c} = (\var{vk}, \sigma)
      \cdot \\
      \verify{vk}{\serialised{(\dbody{c},~\txbody \var{tx})}}{\sigma}\\
      ~ \\
