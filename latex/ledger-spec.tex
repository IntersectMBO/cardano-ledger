...
The $\fun{created}$ calculation sums up all of the unspent outputs that
...
to this (the LHS, i.e. $\fun{created}$) side of the equation as they are accounted for implicitly (as
...
The $\fun{destroyed}$ calculation, on the other hand, sums up the total
...
\begin{todo}
  Created and destroyed calculations i think are mostly right, the names
  can change, though, if necessary
\end{todo}

\begin{todo}
  How do we make sure the $wdrls$ of a transaction are the same as the
  withdrawals included in the outputs??
\end{todo}

\begin{todo}
Also, how do we know the fee and deposits are individually correct, and not just
