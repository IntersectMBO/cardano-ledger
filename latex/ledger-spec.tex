...
\begin{todo}
Is it necessary to have the releasing c check here? Since the refund calc is
only summed over either keyreg or poolreg certificates?
\end{todo}
...
\begin{todo}
Explain why the decayed value is only for one epoch and why this isnt
an epoch boundary calculation
\end{todo}
...
the retirement is scheduled. For details of this accounting, see
...
Finally, the type $\Wdrl$ is used to store withdrawals
(from the pool of rewards) on the ledger. Note that rewards are not
distributed automatically to their recipients. Instead, the wallet must produce
a withdrawal request and put it inside a transaction, and once the transaction
in processed, the withdrawals posted on the ledger.

The term of type $\Wdrl$ is a pair of a transation ID and a reward addresses
the transaction with this ID requested to collect the rewards for.
...
given transaction, to which this transaction is requesting rewards.
...
\begin{figure*}
  ...
  \begin{equation*}
    \begin{array}{r@{~\in~}lr}
      ...
      %
      \fun{txwdrls} & \Tx \to \powerset{\AddrRWD} & \text{transaction withdrawals}\\
    \end{array}
  \end{equation*}
...
$\DState$, i.e. $\var{stkeys}$. The calculation is a sum of all coin in the inputs of
$\var{tx}$ plus the individual key deregistration refunds of the $\var{tx}$ in this context.
...
\begin{figure}
  \begin{align*}
    ...
    \nextdef
    & \fun{created} \in \PrtclConsts \to \UTxO \to \Allocs \to \Tx \to \Coin
    & \text{value created} \\
    ...
    & \fun{destroyed} ~ pc ~ stpools ~ tx =
      \balance{(\txouts{tx})}  + \txfee{tx} + \deposits{pc}{stpools}{tx}\\
  \end{align*}
...
as well as the value in the deposits pool, the fees pool, and any pending
reward withdrawals (due to previous transactions).
...
UTxO, now associated with the $\var{txid}(\var{tx})$ instead (so they can
...
The pending withdrawals are updated by:

\begin{itemize}
\item pair the signal transaction's $\txid$ with the
reward addresses to which it requested withdrawals, and add these pairs
to the $wdrls$ set on the ledger
\end{itemize}

Note that the type of withdrawals of a transaction contains only the
$\var{txid}$ of the transaction that made the withdrawal request, and the address to which
it requested the withdrawals. Other data
needed to include these withdrawals in the UTxO on the ledger (i.e. $\Ix$
values and the coin value of the reward)
is calculated and incorporated into the ledger UTxO at the epoch boundary
(see Section~\ref{sec:epoch}).
  ...
  \begin{equation}\label{eq:utxo-inductive}
    ...
    { \ttl tx \leq \var{slot}
      ...
      \forall \fun{dcerts}~tx,~\var{c}\in\DCertRetirePool \Rightarrow
          ~\fun{ttl}~{tx} \leq \fun{retire}~{c}
      \\
      \created{pc}{utxo}{stkeys}{tx} = \destroyed{pc}{stpools}{tx}
      \\
      ~
      \\
    ...
    }
    {
      ...
      \begin{array}{r}
        ...
        \var{wdrls} \cup (\{\fun{txid}~{tx},~{a})|\var{a}\in\txwdrls{tx}\})\\
      \end{array}
      \right)
    }
  \end{equation}
...
\begin{itemize}
 ...
 there exists a verifiable witness such the hash of the spending key of the
 withdrawal address is the same as the hash of the verification key of the witness.
\end{itemize}

\begin{todo}
  Why do the reward requests need witnessing?
\end{todo}
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-wit]
    {
      ...
      ~ \\
      & \forall \var{a} \in \txwdrls{tx} \cdot \exists (\var{vk}, \sigma) \in \wits{\var{tx}}
      \cdot \\
...
a transaction $\var{tx}$ such that the (witnessed) ledger state transition in
the antecedent is
triggered by that same transaction, and the the (witnessed) delegation state
transition in the antecedent is triggered by the set of certificates carried by
this transaction.
...
The full ledger state transition combines the UTxO transition
(triggered by the body of a transaction) and delegation transition
(triggered by the certificates of a transaction) into a single transition
