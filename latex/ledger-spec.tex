...
of delegation with witnesses include public-private key pairs, hash
...
In particular, one property we enforce is that serialization must yield
 the same result when applying the serialization process
to parts (subtypes) of the data individually as to the whole. For a given type
$A$, we use the notation $\llbracket A\rrbracket$ to denote the serialized
presentation of this type. For a type $C = (A, B)$, this property can be
expressed as

\[ \llbracket C \rrbracket =
      \llbracket \llbracket A \rrbracket \llbracket B \rrbracket \rrbracket \]

Note also that

\[ \llbracket C \rrbracket =
      \llbracket \llbracket C \rrbracket \rrbracket \]
...
\begin{todo}
  discuss dcerts and go over inference and sequence Rules
\end{todo}

\begin{todo}
  include Allocs in types
...
The $\Allocs$ type pairs a hash key with a slot number and is a generic datatype
which we will use below to represent both individual and pool allocation
...
Is that what $(-)_s$ notation is for? Did I miss where it is defined?
\end{todo}
...
Note that the the stake pool retirement refunds are issued when a certificate
scheduling the retirement is processed, not at the epoch boundary for which
the retirement is scheduled. If the retirement is cancelled (a new registration
certificate is issued for this stake pool), a new deposit must
...
This minimum fee
is found in the context of the transaction (i.e. a collection of
...
\begin{todo}
  Explain exactly how refunds get spend (how the ledger is updated to include
...
\begin{todo}
  change hash to hashkey everywhere
\end{todo}
...
The purpose of witnessing is to provide replay protection. Due to the nature
...
Every trasaction must pay a fee, so its list of inputs can never be empty.
Since all the inputs of an applied transaction have already been included
...
\begin{todo}
The witnessed rule should have all the same env variables as the non-witnessed
one? It seems awkward to move them between environment and state
\end{todo}
...
of $\UTxO$, $\DState$, and $\PState$ triggered by the same signal transaction
in a particular context into a single transition that follows the transition
rules of all three components.

\begin{todo}
  mention how composition works in this case
\end{todo}
