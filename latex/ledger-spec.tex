...
be withdrawn from the rewards awarded to that address.
  ...
  \begin{align*}
    ...
    & \fun{reapRewards} \in (\AddrRWD \mapsto \Coin) \to \Wdrl \to (\AddrRWD \mapsto \Coin) \\
        & \fun{reapRewards}~\var{rewards}~\var{wdrls} =
         \var{rewards} \unionoverrideMinus \var{wdrls} \\
    \nextdef
...
\caption{Delegation Rewards Update Rules}
...
%  \item Each withdrawal address in the transaction withdrawal requests is in
%  the set of withdrawal addresses to which rewards are awarded


%\dom \fun{txwdrls}~{tx} \subseteq \dom \var{rewards}
\end{todo}
