...
deposits it pays (also in unspent outputs)
\end{itemize}
...
accounts, i.e. it can be directed to any address. However, means there is no
direct link between the $wdrls$ requests (similarly, the key deregistration
certificate addresses and refund amounts) and the $outputs$. We verify the
correctness of the included outputs through the preservation of value condition
and the witnesses authorizing them. The combination of the
...
\begin{itemize}
 ...
 \item For each output $ix \mapsto (a,c)$ (including any withdrawal and refund
 outputs included) there exists a (unique) verifiable witness such that the hash of
 the spending key of the
 output address is the same as the hash of the verification key of the witness.
...
Because of witnessing of the outputs, we know that all the value withdrawn from ledger
...
Here, we also enforce the condition that only one witness per each input and each
output, as well as one
witness per withdrawal address is allowed. We do so to prevent transactions
being unnecessarily large and full of redundant witnessing. We do not
discuss certificate witnessing here as this is ensured by the DELEGW rule
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-wit]
    {
      ...
      ~ \\
      & \forall (ix \mapsto (a, c)) \in outputs \cdot \exists! (\var{vk}, \sigma) \in \wits{\var{tx}}
      \cdot \\
      \mathcal{V}_{\var{vk}}{\serialised{(\txbody{tx}, (a,c))}}_{\sigma}
