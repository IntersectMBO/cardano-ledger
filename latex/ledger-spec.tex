...
Figure~\ref{fig:functions:deposits}.
The function
...
\begin{figure}
  \begin{align*}
    ...
    & \fun{deposits}~{allocs}~{pc}~{tx} = \sum\limits_{c \in \fun{dresource}~tx
        \wedge \fun{author}~c \in \dom~\var{allocs}}(\fun{dvalue}~pc~c)
      \nextdef
...
Finally, a term of type $\Wdrl$ is used to represents

\begin{todo}
  explain withdrawals
  Should they be powersets of finite maps??
  rewards not distributed automatically, where are they calculated?
  In order to claim rewards, wallet must actively do so by making special
...
The map $\fun{txwdrls}$ map gives the set of reward withdrawals done by a given
transaction.
...
Here we also remind the reader how domain restriction, domain exclusion, and
range restriction notation is to be read.
...
be applied to a particular $\UTxOState$:
...
\item The inputs of any pending withdrawals on the ledger are distinct from
the inputs in the list of withdrawals of the signal transaction
\end{itemize}
...
carrying. Finally, the ledger and transaction withdrawal inputs must be disjoint
to avoid any withdrawal conflics or insonsistent updates.
...
\begin{todo}
  describe deposits, fees, wdrls update
\end{todo}
...
\begin{itemize}
 ...
 \item For every input of $\var{tx}$, ...
\end{itemize}

\begin{todo}
  Revise the second condition above
\end{todo}
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-wit]
    {
      ...
      ~ \\
      & \forall (txin \mapsto a) \in \txwdrls{tx} \cdot \exists (\var{vk}, \sigma) \in \wits{\var{tx}}
      \cdot \\
