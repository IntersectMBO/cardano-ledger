...
epochIndex file = withBinaryFile file ReadMode $ \h -> do
  headerIsGood <- (indexHeader ==)
    <$> LBS.hGet h (fromIntegral $ LBS.length indexHeader)
  if headerIsGood then pure (Just (EpochIndex file)) else pure Nothing
...
readIndex :: EpochIndex -> Slot -> IO (Maybe Word64)
