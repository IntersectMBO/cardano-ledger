...
mkSalt :: ToCBOR salt => salt -> S.Salt
...
mkPass :: ToCBOR pass => pass -> S.Pass
mkPass = S.Pass . serialize'

encryptPass
  :: (ToCBOR pass, MonadRandom m) => S.ScryptParams -> pass -> m S.EncryptedPass
encryptPass params passphrase =
  genSalt <&> \salt -> encryptPassWithSalt params salt passphrase

encryptPassWithSalt
  :: ToCBOR pass => S.ScryptParams -> S.Salt -> pass -> S.EncryptedPass
encryptPassWithSalt params salt passphrase =
  S.encryptPass params salt (mkPass passphrase)

verifyPass :: ToCBOR pass => S.ScryptParams -> pass -> S.EncryptedPass -> Bool
